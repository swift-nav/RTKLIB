...
/* Subroutine */ int step2diu_(doublereal *xsta, doublereal *fhr, doublereal *t,
                               doublereal *xcorsta) {
  /* Initialized data */

  static doublereal datdi[279] /* was [9][31] */ = {
      -3.,  0.,   2.,   0.,  0.,  -.01, 0.,   0.,  0.,    -3.,  2.,   0.,
      0.,   0.,   -.01, 0.,  0.,  0.,   -2.,  0.,  1.,    -1.,  0.,   -.02,
      0.,   0.,   0.,   -2., 0.,  1.,   0.,   0.,  -.08,  0.,   -.01, .01,
      -2.,  2.,   -1.,  0.,  0.,  -.02, 0.,   0.,  0.,    -1.,  0.,   0.,
      -1.,  0.,   -.1,  0.,  0.,  0.,   -1.,  0.,  0.,    0.,   0.,   -.51,
      0.,   -.02, .03,  -1., 2.,  0.,   0.,   0.,  .01,   0.,   0.,   0.,
      0.,   -2.,  1.,   0.,  0.,  .01,  0.,   0.,  0.,    0.,   0.,   -1.,
      0.,   0.,   .02,  0.,  0.,  0.,   0.,   0.,  1.,    0.,   0.,   .06,
      0.,   0.,   0.,   0.,  0.,  1.,   1.,   0.,  .01,   0.,   0.,   0.,
      0.,   2.,   -1.,  0.,  0.,  .01,  0.,   0.,  0.,    1.,   -3.,  0.,
      0.,   1.,   -.06, 0.,  0.,  0.,   1.,   -2., 0.,    -1.,  0.,   .01,
      0.,   0.,   0.,   1.,  -2., 0.,   0.,   0.,  -1.23, -.07, .06,  .01,
      1.,   -1.,  0.,   0.,  -1., .02,  0.,   0.,  0.,    1.,   -1.,  0.,
      0.,   1.,   .04,  0.,  0.,  0.,   1.,   0.,  0.,    -1.,  0.,   -.22,
      .01,  .01,  0.,   1.,  0.,  0.,   0.,   0.,  12.,   -.8,  -.67, -.03,
      1.,   0.,   0.,   1.,  0.,  1.73, -.12, -.1, 0.,    1.,   0.,   0.,
      2.,   0.,   -.04, 0.,  0.,  0.,   1.,   1.,  0.,    0.,   -1.,  -.5,
      -.01, .03,  0.,   1.,  1.,  0.,   0.,   1.,  .01,   0.,   0.,   0.,
      0.,   1.,   0.,   1.,  -1., -.01, 0.,   0.,  0.,    1.,   2.,   -2.,
      0.,   0.,   -.01, 0.,  0.,  0.,   1.,   2.,  0.,    0.,   0.,   -.11,
      .01,  .01,  0.,   2.,  -2., 1.,   0.,   0.,  -.01,  0.,   0.,   0.,
      2.,   0.,   -1.,  0.,  0.,  -.02, 0.,   0.,  0.,    3.,   0.,   0.,
  ...
  double d_mod(doublereal *, doublereal *), sqrt(doublereal),
      atan2(doublereal, doublereal), sin(doublereal), cos(doublereal);

  /* Local variables */
  static doublereal h__;
  static integer i__, j;
  static doublereal p, s, de, dn, dr, pr, ps, zla, tau, zns, rsta, cosla, sinla,
  ...
  s = ((*t * 1.85139e-6 - .0014663889) * *t + 481267.88194) * *t + 218.31664563;
  tau = *fhr * 15. + 280.4606184 +
        ((*t * -2.58e-8 + 3.8793e-4) * *t + 36000.7700536) * *t + (-s);
  pr = (((*t * 7e-9 + 2.1e-8) * *t + 3.08889e-4) * *t + 1.396971278) * *t;
  s += pr;
  h__ =
      (((*t * -6.54e-9 + 2e-8) * *t + 3.0322222e-4) * *t + 36000.7697489) * *t +
      280.46645;
  p = (((*t * 5.263e-8 - 1.24991e-5) * *t - .01032172222) * *t +
      ...
      83.35324312;
  zns =
      (((*t * 1.65e-8 - 2.13944e-6) * *t - .00207561111) * *t + 1934.13626197) *
          *t +
      234.95544499;
  ps = (((*t * -3.34e-9 - 1.778e-8) * *t + 4.5688889e-4) * *t + 1.71945766667) *
           *t +
       282.93734098;
  ...
  for (j = 1; j <= 31; ++j) {
    ...
    thetaf = (tau + datdi[j * 9 - 9] * s + datdi[j * 9 - 8] * h__ +
              datdi[j * 9 - 7] * p + datdi[j * 9 - 6] * zns +
              datdi[j * 9 - 5] * ps) *
             deg2rad;
    ...
    /* Computing 2nd power */
    d__4 = sinphi;
    dn = datdi[j * 9 - 2] * (d__1 * d__1 - d__2 * d__2) * sin(thetaf + zla) +
         datdi[j * 9 - 1] * (d__3 * d__3 - d__4 * d__4) * cos(thetaf + zla);
    /*      DE=DATDI(8,J)*SINPHI*COS(THETAF+ZLA)+ */
    /*     Modified 20 June 2007 */
    de = datdi[j * 9 - 2] * sinphi * cos(thetaf + zla) -
         datdi[j * 9 - 1] * sinphi * sin(thetaf + zla);
    xcorsta[1] =
