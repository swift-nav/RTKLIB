...
/* Subroutine */ int step2lon_(doublereal *xsta, doublereal *t,
                               doublereal *xcorsta) {
  /* Initialized data */

  static doublereal datdi[45] /* was [9][5] */ = {
      0.,   0.,   0.,   1.,   0.,   .47,  .23,  .16,  .07,  0.,   2.,   0.,
      0.,   0.,   -.2,  -.12, -.11, -.05, 1.,   0.,   -1.,  0.,   0.,   -.11,
      -.08, -.09, -.04, 2.,   0.,   0.,   0.,   0.,   -.13, -.11, -.15, -.07,
  ...
  /* Builtin functions */
  double sqrt(doublereal), d_mod(doublereal *, doublereal *), cos(doublereal),
      sin(doublereal);

  /* Local variables */
  static doublereal h__;
  static integer i__, j;
  static doublereal p, s, de, dn, dr, pr, ps, zns, rsta, cosla, sinla, thetaf,
  ...
  for (j = 1; j <= 5; ++j) {
    thetaf =
        (datdi[j * 9 - 9] * s + datdi[j * 9 - 8] * h__ + datdi[j * 9 - 7] * p +
         datdi[j * 9 - 6] * zns + datdi[j * 9 - 5] * ps) *
        deg2rad;
    /* Computing 2nd power */
    d__1 = sinphi;
    /* Computing 2nd power */
    ...
    dn = datdi[j * 9 - 3] * (cosphi * sinphi * 2.) * cos(thetaf) +
         datdi[j * 9 - 1] * (cosphi * sinphi * 2.) * sin(thetaf);
    de = 0.;
    dr_tot__ += dr;
    dn_tot__ += dn;
    xcorsta[1] =
