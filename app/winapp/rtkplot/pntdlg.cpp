...
//---------------------------------------------------------------------------
__fastcall TPntDialog::TPntDialog(TComponent *Owner) : TForm(Owner) {
  PntList->RowCount = 1;
}
//---------------------------------------------------------------------------
...
void __fastcall TPntDialog::BtnAddClick(TObject *Sender) {
  ...
  for (i = 0; i < PntList->RowCount; i++) {
    if (PntList->Cells[2][i] == "")
      break;
  }
  if (i >= PntList->RowCount)
    return;
  ...
  for (int i = sel; i < PntList->RowCount; i++) {
    for (int j = 0; j < PntList->ColCount; j++) {
      if (i + 1 >= PntList->RowCount)
        ...
        PntList->Cells[j][i] = PntList->Cells[j][i + 1];
    }
  }
  UpdatePoint();
}
//---------------------------------------------------------------------------
void __fastcall TPntDialog::BtnUpdateClick(TObject *Sender) { UpdatePoint(); }
//---------------------------------------------------------------------------
void __fastcall TPntDialog::PntListSetEditText(TObject *Sender, int ACol,
                                               int ARow,
                                               const UnicodeString Value) {
  UpdatePoint();
}
//---------------------------------------------------------------------------
void __fastcall TPntDialog::BtnCloseClick(TObject *Sender) { Close(); }
//---------------------------------------------------------------------------
