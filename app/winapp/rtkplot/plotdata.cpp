...
void __fastcall TPlot::ReadSol(TStrings *files, int sel) {
  ...
  for (i = 0; i < 2; i++) {
    if (SolFiles[i]->Count == 0)
      continue;
    Caption = Caption + SolFiles[i]->Strings[0] +
              (SolFiles[i]->Count > 1 ? "... " : " ");
  }
  BtnSol12->Down = False;
  if (sel == 0)
...
int __fastcall TPlot::ReadObsRnx(TStrings *files, obs_t *obs, nav_t *nav,
  ...
  for (i = 0; i < files->Count; i++) {
    ...
    if (!strcmp(p, ".obs") || !strcmp(p, ".OBS")) {
      ...
      strcpy(p + 3, "N");
      readrnxt(navfile, 1, ts, te, tint, opt, NULL, nav, NULL);
    } else if (!strcmp(p - 1, "O.rnx") &&
               (p = strrchr(navfile, '_'))) { /* rinex 3 */
      *p = '\0';
      if (!(p = strrchr(navfile, '_')))
        continue;
      *p = '\0';
      if (!(p = strrchr(navfile, '_')))
        continue;
...
void __fastcall TPlot::ReadElMaskData(AnsiString file) {
  ...
  while (fgets(buff, sizeof(buff), fp)) {
    ...
    if (buff[0] == '%' || sscanf(buff, "%lf %lf", &az1, &el1) < 2)
      continue;

    if (az0 < az1 && az1 <= 360.0 && 0.0 <= el1 && el1 <= 90.0) {

      for (j = (int)az0; j < (int)az1; j++)
        ElMaskData[j] = el0 * D2R;
...
static void YCrCb(const BYTE *pix, double *Y) {
  ...
  Y[1] =
      (0.500 * pix[0] - 0.419 * pix[1] + 0.081 * pix[2]) / 255; // Cr (-.5-.5)
  Y[2] =
      (-0.169 * pix[0] - 0.331 * pix[1] + 0.500 * pix[2]) / 255; // Cb (-.5-.5)
}
// update sky image ---------------------------------------------------------
void __fastcall TPlot::UpdateSky(void) {
  BITMAP bm1, bm2;
  BYTE *pix;
  double x, y, xp, yp, r, a, b, p[3], q[3], R[9] = {0}, dr, dist, Yz[3] = {0},
                                            Y[3];
  ...
  if (w1 <= 0 || h1 <= 0 || b1 < w1 * 3 || w2 <= 0 || h2 <= 0 || b2 < w2 * 3)
    return;
  ...
  for (j = 0; j < h2; j++)
    for (i = 0; i < w2; i++) {
      ...
      if (SkyDestCorr) {
        ...
        dr = r * 9.0 - k;
        dist =
            k > 8 ? SkyDest[9] : (1.0 - dr) * SkyDest[k] + dr * SkyDest[k + 1];
        xp *= dist / r;
        yp *= dist / r;
      } else {
        xp *= SkyScale;
    ...
    *p = '\0';
    if (strstr(buff, "centx") == buff)
      ...
      sscanf(p + 1, "%lf %lf %lf %lf %lf %lf %lf %lf %lf", SkyDest + 1,
             SkyDest + 2, SkyDest + 3, SkyDest + 4, SkyDest + 5, SkyDest + 6,
             SkyDest + 7, SkyDest + 8, SkyDest + 9);
...
void __fastcall TPlot::ReadGpxFile(AnsiString file) {
  ...
  while (fgets(buff, sizeof(buff), fp) && NWayPnt < MAXWAYPNT) {
    if ((p = strstr(buff, "<wpt "))) {
      if (sscanf(p + strlen("<wpt "), "lat=\"%lf\" lon=\"%lf\"", pos, pos + 1) <
          2)
        continue;
    ...
    } else if ((p = strstr(buff, label1.c_str())) && norm(pos, 3) > 0.0 &&
               !name[0]) {
      sscanf(p + strlen(label1.c_str()), "%[^<]", name);
    } else if ((p = strstr(buff, label2.c_str())) && norm(pos, 3) > 0.0 &&
               !name[0]) {
...
void __fastcall TPlot::ReadPosFile(AnsiString file) {
  ...
  while (fgets(buff, sizeof(buff), fp) && NWayPnt < MAXWAYPNT) {
    ...
    n = sscanf(buff, "%lf %lf %lf %255s %255s", pos, pos + 1, pos + 2, id,
               name);
...
void __fastcall TPlot::ReadWaypoint(AnsiString file) {
  ...
  if ((p = strrchr(file.c_str(), '.')) && !strcmp(p, ".gpx"))
    type = 1;
...
void __fastcall TPlot::ReadStaPos(const char *file, const char *sta,
  ...
  while (fgets(buff, sizeof(buff), fp)) {
    if (strstr(buff, "%=SNX") == buff)
      sinex = 1;
    if (buff[0] == '%' || buff[1] == '#')
      continue;
    if (sinex) {
      if (strlen(buff) < 68 || strncmp(buff + 14, sta, 4))
        continue;
      if (!strncmp(buff + 7, "STAX", 4))
        rr[0] = str2num(buff, 47, 21);
      if (!strncmp(buff + 7, "STAY", 4))
        rr[1] = str2num(buff, 47, 21);
      if (!strncmp(buff + 7, "STAZ", 4)) {
        rr[2] = str2num(buff, 47, 21);
        break;
      }
    } else {
      if (sscanf(buff, "%lf %lf %lf %s", pos, pos + 1, pos + 2, code) < 4)
        continue;
      if (strcmp(code, sta))
        continue;
      pos[0] *= D2R;
...
void __fastcall TPlot::SaveDop(AnsiString file) {
  ...
  tlabel = TimeLabel <= 1 ? "TIME (GPST)"
                          : (TimeLabel <= 2 ? "TIME (UTC)" : "TIME (JST)");

  fprintf(fp, "%% %-*s %6s %8s %8s %8s %8s (EL>=%.0fdeg)\n",
          TimeLabel == 0 ? 13 : 19, tlabel, "NSAT", "GDOP", "PDOP", "HDOP",
          "VDOP", ElMask);

  for (i = 0; i < NObs; i++) {
    ns = 0;
    for (j = IndexObs[i]; j < Obs.n && j < IndexObs[i + 1]; j++) {
      if (SatMask[Obs.data[j].sat - 1])
        continue;
      if (El[j] < ElMask * D2R)
        continue;
...
void __fastcall TPlot::SaveSnrMp(AnsiString file) {
  ...
  fprintf(fp, "%% %-*s %6s %8s %8s %9s %10s\n", TimeLabel == 0 ? 13 : 19,
          tlabel, "SAT", "AZ(deg)", "EL(deg)", "SNR(dBHz)", mp);

  for (i = 0; i < MAXSAT; i++) {
    if (SatMask[i] || !SatSel[i])
      continue;
    satno2id(i + 1, sat);

    for (j = 0; j < Obs.n; j++) {
      if (Obs.data[j].sat != i + 1)
        continue;

      for (k = 0; k < NFREQ + NEXOBS; k++) {
        if (strstr(code2obs(Obs.data[j].code[k]), code))
          break;
      }
      if (k >= NFREQ + NEXOBS)
        continue;
      ...
      fprintf(fp, "%s %6s %8.1f %8.1f %9.2f %10.4f\n", tstr, sat, Az[j] * R2D,
              El[j] * R2D, Obs.data[j].SNR[k] * SNR_UNIT,
              !Mp[k] ? 0.0 : Mp[k][j]);
    }
  }
  fclose(fp);
...
void __fastcall TPlot::Connect(void) {
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (RtStream[i] == STR_FILE || !SolData[i].cyclic ||
        SolData[i].nmax != RtBuffSize + 1) {
      Clear();
      initsolbuf(SolData + i, 1, RtBuffSize + 1);
    }
    if (RtStream[i] == STR_SERIAL)
      mode |= STR_MODE_W;
...
int __fastcall TPlot::CheckObs(AnsiString file) {
  ...
  if (!(p = strrchr(file.c_str(), '.')))
    return 0;
  if (!strcmp(p, ".z") || !strcmp(p, ".gz") || !strcmp(p, ".zip") ||
      !strcmp(p, ".Z") || !strcmp(p, ".GZ") || !strcmp(p, ".ZIP")) {
    return *(p - 1) == 'o' || *(p - 1) == 'O' || *(p - 1) == 'd' ||
           *(p - 1) == 'D';
  }
  return !strcmp(p, ".obs") || !strcmp(p, ".OBS") || !strcmp(p + 3, "o") ||
...
void __fastcall TPlot::UpdateObs(int nobs) {
  ...
  for (int i = 0, j = 0; i < Obs.n; i = j) {
    ...
    for (j = i; j < Obs.n; j++) {
      if (timediff(Obs.data[j].time, time) > TTOL)
        break;
    }
    IndexObs[NObs++] = i;
...
void __fastcall TPlot::UpdateMp(void) {
  ...
  for (i = 0; i < Obs.n; i++) {
    ...
    if (data->L[0] == 0.0 || data->L[1] == 0.0 || freq1 == 0.0 || freq2 == 0.0)
      continue;
    I = -CLIGHT * (data->L[0] / freq1 - data->L[1] / freq2) /
        (1.0 - SQR(freq1 / freq2));

    for (j = 0; j < NFREQ + NEXOBS; j++) {
      freq = sat2freq(data->sat, data->code[j], &Nav);
      if (data->P[j] == 0.0 || data->L[j] == 0.0 || freq == 0.0)
        continue;
      Mp[j][i] =
          data->P[j] - CLIGHT * data->L[j] / freq - 2.0 * SQR(freq1 / freq) * I;
    }
  }
    ...
    for (j = 0; j < NFREQ + NEXOBS; j++) {
      for (i = n = m = 0, B = 0.0; i < Obs.n; i++) {
        ...
        if ((data->LLI[j] & 1) || (data->LLI[0] & 1) || (data->LLI[1] & 1) ||
            fabs(Mp[j][i] - B) > 5.0) {
          for (k = m; k < i; k++) {
            if (Obs.data[k].sat == sat && Mp[j][k] != 0.0)
              Mp[j][k] -= B;
          }
          n = 0;
          m = i;
