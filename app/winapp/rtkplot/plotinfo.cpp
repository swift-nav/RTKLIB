...
void __fastcall TPlot::UpdateTimeObs(void) {
  ...
  UTF8String msgs2[] = {" SNR=...45.", "..40.", "..35.", "..30.",
                        "..25 ",       "",      " <25 "};
  UTF8String msgs3[] = {" SYS=GPS ", "GLO ", "GAL ", "QZS ",
                        "BDS ",      "IRN ", "SBS "};
  UTF8String msgs4[] = {" MP=..0.6", "..0.3", "..0.0..", "-0.3..",
                        "-0.6..",    "",      ""};
...
void __fastcall TPlot::UpdateTimeSol(void) {
  ...
  if ((BtnSol1->Down || BtnSol2->Down || BtnSol12->Down) &&
    ...
    if (PLOT_SOLP <= PlotType && PlotType <= PLOT_SOLA) {
      ...
      msg += s.sprintf("E=%7.4f%s N=%7.4f%s U=%7.4f%s Q=", p->x[0], u, p->y[0],
                       u, p->z[0], u);
      delete p;
...
void __fastcall TPlot::UpdateInfoSol(void) {
  ...
  if (n > 0) {
    ...
    for (i = 1; i <= 6; i++) {
      if (nq[i] <= 0)
        continue;
      msgs[i - 1].sprintf("%d:%d(%.1f%%) ", i, nq[i],
                          (double)nq[i] / n * 100.0);
    }
  }
  ShowMsg(msg);
...
void __fastcall TPlot::UpdatePlotType(void) {
  ...
  for (i = 0; i < PlotTypeS->Items->Count; i++) {
    if (PlotTypeS->Items->Strings[i] != PTypes[PlotType])
      continue;
