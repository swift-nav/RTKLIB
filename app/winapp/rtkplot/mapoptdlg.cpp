...
//---------------------------------------------------------------------------
void __fastcall TMapOptDialog::ScaleXUpDownChangingEx(
...
//---------------------------------------------------------------------------
void __fastcall TMapOptDialog::ScaleYUpDownChangingEx(
    TObject *Sender, bool &AllowChange, int NewValue,
    TUpDownDirection Direction) {
  AnsiString s;
...
//---------------------------------------------------------------------------
void __fastcall TMapOptDialog::ScaleXKeyDown(TObject *Sender, WORD &Key,
                                             TShiftState Shift)
...
//---------------------------------------------------------------------------
void __fastcall TMapOptDialog::ScaleYKeyDown(TObject *Sender, WORD &Key,
                                             TShiftState Shift)
...
//---------------------------------------------------------------------------
void __fastcall TMapOptDialog::LatKeyDown(TObject *Sender, WORD &Key,
                                          TShiftState Shift)

{
  bool allowchange;
  if (Key == VK_UP || Key == VK_DOWN || Key == VK_RIGHT || Key == VK_LEFT) {
    LatUpDownChangingEx(Sender, allowchange, 0,
                        Key == VK_UP || Key == VK_RIGHT ? updUp : updDown);
    Key = 0;
  }
}
//---------------------------------------------------------------------------
void __fastcall TMapOptDialog::LonKeyDown(TObject *Sender, WORD &Key,
                                          TShiftState Shift)

{
  bool allowchange;
  if (Key == VK_UP || Key == VK_DOWN || Key == VK_RIGHT || Key == VK_LEFT) {
    LonUpDownChangingEx(Sender, allowchange, 0,
                        Key == VK_UP || Key == VK_RIGHT ? updUp : updDown);
    Key = 0;
  }
}
