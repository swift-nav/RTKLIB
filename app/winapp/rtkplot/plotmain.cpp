...
void __fastcall TPlot::FormShow(TObject *Sender) {
  ...
  if (*path1 || *path2) {
    ...
    Caption = Title != ""
                  ? Title
                  : s.sprintf("%s ver.%s %s", PRGNAME, VER_RTKLIB, PATCH_LEVEL);
  }
  if (ShapeFile != "") {
...
void __fastcall TPlot::MenuPortClick(TObject *Sender) {
  ...
  if (ConnectDialog->ShowModal() != mrOk)
    return;
...
void __fastcall TPlot::MenuOptionsClick(TObject *Sender) {
  ...
  if (PlotOptDialog->ShowModal() != mrOk)
    return;
  ...
  for (int i = 0; i < RangeList->Count; i++) {
    ...
    if (sscanf(str.c_str(), "%lf%31s", &range, unit) < 1)
      continue;
    if (!strcmp(unit, "cm"))
      range *= 0.01;
...
void __fastcall TPlot::DispGesture(TObject *Sender,
...
void __fastcall TPlot::BtnRangeListClick(TObject *Sender) {
  ...
  RangeList->Left =
      btn->Parent->Left + btn->Left + btn->Width - RangeList->Width;
...
void __fastcall TPlot::RangeListClick(TObject *Sender) {
  ...
  if ((i = RangeList->ItemIndex) < 0)
    return;

  str = RangeList->Items->Strings[i];
  if (sscanf(str.c_str(), "%lf%s", &YRange, unit) < 1)
    return;
  if (!strcmp(unit, "cm"))
    YRange *= 0.01;
  else if (!strcmp(unit, "km"))
...
void __fastcall TPlot::DispMouseMove(TObject *Sender, TShiftState Shift, int X,
                                     int Y) {
...
void __fastcall TPlot::MouseDownSol(int X, int Y) {
  ...
  for (i = 0; i < 3; i++) {
    if (!btn[i]->Down || (i != 1 && PlotType == PLOT_SNR))
      continue;
...
void __fastcall TPlot::UpdateEnable(void) {
  ...
  BtnSol12->Enabled = !ConnectState && PlotType <= PLOT_SOLA &&
                      SolData[0].n > 0 && SolData[1].n > 0;
  ...
  SatList->Visible = PlotType == PLOT_RES || PlotType == PLOT_RESE ||
                     PlotType >= PLOT_OBS || PlotType == PLOT_SKY ||
  ...
  BtnShowImg->Visible =
      PlotType == PLOT_TRK || PlotType == PLOT_SKY || PlotType == PLOT_MPS;
  ...
  MenuAnimStart->Enabled =
      !ConnectState && BtnAnimate->Enabled && !BtnAnimate->Down;
  MenuAnimStop->Enabled =
      !ConnectState && BtnAnimate->Enabled && BtnAnimate->Down;
...
void __fastcall TPlot::LoadOpt(void) {
  ...
  MapStrs[0][1] =
      ini->ReadString("mapview", "mapstrs_0_1",
                      "https://tile.openstreetmap.org/{z}/{x}/{y}.png");
  MapStrs[0][2] =
      ini->ReadString("mapview", "mapstrs_0_2", "https://osm.org/copyright");
  for (int i = 1; i < 6; i++)
    for (int j = 0; j < 3; j++) {
      MapStrs[i][j] =
          ini->ReadString("mapview", s.sprintf("mapstrs_%d_%d", i, j), "");
    }
  for (int i = 0; i < 2; i++) {
...
void __fastcall TPlot::SaveOpt(void) {
  ...
  for (int i = 0; i < 6; i++)
    for (int j = 0; j < 3; j++) {
      ini->WriteString("mapview", s.sprintf("mapstrs_%d_%d", i, j),
                       MapStrs[i][j]);
    }
  for (int i = 0; i < 2; i++) {
