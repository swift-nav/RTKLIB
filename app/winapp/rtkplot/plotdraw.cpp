...
#define P_IN_B(pos, bound)                                                     \
  (pos[0] >= bound[0] && pos[0] <= bound[1] && pos[1] >= bound[2] &&           \
   pos[1] <= bound[3])

#define B_IN_B(bound1, bound2)                                                 \
  (bound1[0] <= bound2[1] && bound1[1] >= bound2[0] &&                         \
   bound1[2] <= bound2[3] && bound1[3] >= bound2[2])
...
void __fastcall TPlot::DrawTrkMap(int level) {
  ...
  for (i = 0; i < 8; i++) {
    ...
    ecef2pos(rr, pos);
    if (pos[0] < bound[0])
      bound[0] = pos[0]; // min lat
    if (pos[0] > bound[1])
      bound[1] = pos[0]; // max lat
    if (pos[1] < bound[2])
      bound[2] = pos[1]; // min lon
    if (pos[1] > bound[3])
      bound[3] = pos[1]; // max lon
  }
...
void __fastcall TPlot::DrawTrkStat(const TIMEPOS *pos, UTF8String header,
                                   int p) {
  UTF8String s[6];
  TPoint p1, p2;
  ...
  if (pos->n > 0) {
    ...
    s[n++].sprintf("RMS=E:%7.4fm N:%7.4fm U:%7.4fm 2D:%7.4fm", rms[0], rms[1],
                   rms[2], 2.0 * rms[3]);
    delete[] d;
  }
  GraphT->GetPos(p1, p2);
...
void __fastcall TPlot::DrawSol(int level, int type) {
  ...
  if (BtnShowTrack->Down &&
      (BtnSol1->Down || BtnSol2->Down || BtnSol12->Down)) {
...
void __fastcall TPlot::DrawSolPnt(const TIMEPOS *pos, int level, int style) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (level && ShowErr && PlotType <= PLOT_SOLA && PlotStyle < 2) {
      ...
      if (ShowErr == 1) {
        for (j = 0; j < pos->n; j++) {
          GraphG[i]->DrawMark(x[j], y[j], 12, CColor[1],
                              (int)(SQRT(s[j]) * 2.0 / ys), 0);
        }
      } else {
        yy = new double[pos->n];
  ...
  if (level && PlotStyle <= 2) {
    ...
    for (i = 0; i < Obs.n; i++) {
      ...
      if (fabs(xt - tt[obs->sat - 1]) / xs > 0.9) {
        GraphR->DrawMark(xt, yp[obs->sat - 1], 0,
...
void __fastcall TPlot::GenObsSlip(int *LLI) {
  ...
  for (int i = 0; i < Obs.n; i++) {
    ...
    LLI[i] = 0;
    if (El[i] < ElMask * D2R || !SatSel[obs->sat - 1])
      continue;
    if (ElMaskP && El[i] < ElMaskData[(int)(Az[i] * R2D + 0.5)])
      continue;
    ...
    if (ShowSlip == 1) { // LG jump
      ...
      if (!strcmp(obstype, "ALL")) {
        if ((freq1 = sat2freq(obs->sat, obs->code[0], &Nav)) == 0.0)
          continue;
        LLI[i] = obs->LLI[0] & 2;
        for (j = 1; j < NFREQ + NEXOBS; j++) {
          LLI[i] |= obs->LLI[j] & 2;
          if ((freq2 = sat2freq(obs->sat, obs->code[j], &Nav)) == 0.0)
            continue;
          gf = CLIGHT * (obs->L[0] / freq1 - obs->L[j] / freq2);
          if (fabs(gfp[obs->sat - 1][j] - gf) > THRESLIP)
            LLI[i] |= 1;
          gfp[obs->sat - 1][j] = gf;
        }
      } else {
        if (sscanf(obstype, "L%d", &k) == 1) {
          ...
          for (j = 0; j < NFREQ + NEXOBS; j++) {
            if (!strcmp(code2obs(obs->code[j]), obstype))
              break;
          }
          if (j >= NFREQ + NEXOBS)
            continue;
        ...
        LLI[i] = obs->LLI[j] & 2;
        k = (j == 0) ? 1 : 0;
        if ((freq1 = sat2freq(obs->sat, obs->code[k], &Nav)) == 0.0)
          continue;
        if ((freq2 = sat2freq(obs->sat, obs->code[j], &Nav)) == 0.0)
          continue;
...
void __fastcall TPlot::DrawObsEphem(double *yp) {
  ...
  for (i = 0; i < MAXSAT; i++) {
    ...
    for (j = 0; j < Nav.n; j++) {
      ...
      for (k = 0; k < 3; k++)
        ps[k].y += MarkSize + 2 + off[Nav.eph[j].sat - 1];
      ps[0].y -= 2;
      ...
      if (in)
        GraphR->DrawMark(ps[2], 0, svh ? MColor[0][5] : CColor[1], svh ? 4 : 3,
                         0);
    }
    for (j = 0; j < Nav.ng; j++) {
      ...
      for (k = 0; k < 3; k++)
        ps[k].y += MarkSize + 2 + off[Nav.geph[j].sat - 1];
      ps[0].y -= 2;

      GraphR->DrawPoly(ps, 3, Nav.geph[j].svh ? MColor[0][5] : CColor[1], 0);

      if (in)
        GraphR->DrawMark(ps[2], 0, Nav.geph[j].svh ? MColor[0][5] : CColor[1],
    ...
    for (j = 0; j < Nav.ns; j++) {
      ...
      for (k = 0; k < 3; k++)
        ps[k].y += MarkSize + 2 + off[Nav.seph[j].sat - 1];
      ps[0].y -= 2;

      GraphR->DrawPoly(ps, 3, Nav.seph[j].svh ? MColor[0][5] : CColor[1], 0);

      if (in)
        GraphR->DrawMark(ps[2], 0, Nav.seph[j].svh ? MColor[0][5] : CColor[1],
...
void __fastcall TPlot::DrawSky(int level) {
  ...
  obsd_t *obs;
  gtime_t t[MAXSAT] = {{0}};
  double p[MAXSAT][2] = {{0}}, gfp[MAXSAT][NFREQ + NEXOBS] = {{0}},
         p0[MAXSAT][2] = {{0}};
  ...
  if (ShowSlip && PlotStyle <= 2) {
    ...
    for (i = 0; i < Obs.n; i++) {
      ...
      if (fabs(dt) > 300.0)
        continue;
      GraphS->DrawMark(x, y, 4, MColor[0][5], MarkSize * 3 + 2,
                       ATAN2(dy, dx) * R2D + 90);
    }
    delete[] LLI;
  }
  ...
  if (BtnShowTrack->Down && 0 <= ind && ind < NObs) {
    ...
    for (i = IndexObs[ind]; i < Obs.n && i < IndexObs[ind + 1]; i++) {
      ...
      GraphS->DrawMark(x, y, 1, col == clBlack ? MColor[0][0] : CColor[2],
                       Disp->Font->Size * 2 + 5, 0);
  ...
  if (ShowStats && BtnShowTrack->Down && 0 <= ind && ind < NObs && !SimObs) {
    ...
    if (!strcmp(obstype, "ALL")) {
      ...
      ustr.sprintf("%3s: %3s  %4s   %3s %3s", "SAT", "SIG", "OBS", "CN0",
                   "LLI");
    }
    ...
    for (i = IndexObs[ind]; i < Obs.n && i < IndexObs[ind + 1]; i++) {
      ...
      if (HideLowSat && El[i] < ElMask * D2R)
        continue;
      if (HideLowSat && ElMaskP && El[i] < ElMaskData[(int)(Az[i] * R2D + 0.5)])
        continue;
      ...
      TColor col = ObsColor(obs, Az[i], El[i]);
      p2.y += hh;
      GraphS->DrawText(p2, ustr, col == clBlack ? MColor[0][0] : col, 2, 2, 0,
                       MS_FONT);
    }
  }
...
void __fastcall TPlot::DrawDop(int level) {
  ...
  if (doptype == 0) {
    ...
    label.sprintf("DOP %s 10 (EL %s %.0f%s)", CHARMUL, CHARGTE, ElMask,
                  CHARDEG);
  }
  ...
  for (i = 0; i < NObs; i++) {
    ...
    for (j = IndexObs[i]; j < Obs.n && j < IndexObs[i + 1]; j++) {
      ...
      if (El[j] < ElMask * D2R)
        continue;
      if (ElMaskP && El[j] < ElMaskData[(int)(Az[j] * R2D + 0.5)])
        continue;
      azel[ns[n] * 2] = Az[j];
      azel[1 + ns[n] * 2] = El[j];
      ns[n]++;
  ...
  if (BtnShowTrack->Down && 0 <= ind && ind < NObs) {
    ...
    for (i = 0; i < 4; i++) {
      if ((doptype != 0 && doptype != i + 2) || dop[i] <= 0.0)
        continue;
      GraphR->DrawMark(xl[0], dop[i] * 10.0, 0, MColor[0][i + 2],
                       MarkSize * 2 + 2, 0);
    }
    if (doptype == 0 || doptype == 1) {
...
void __fastcall TPlot::DrawDopStat(double *dop, int *ns, int n) {
  ...
  if (DopType->ItemIndex == 0 || DopType->ItemIndex >= 2) {
    s2[m++].sprintf("AVE= GDOP:%4.1f PDOP:%4.1f HDOP:%4.1f VDOP:%4.1f", ave[0],
                    ave[1], ave[2], ave[3]);
    s2[m++].sprintf("NDOP=%d(%4.1f%%) %d(%4.1f%%) %d(%4.1f%%) %d(%4.1f%%)",
                    ndop[0], n > 0 ? ndop[0] * 100.0 / n : 0.0, ndop[1],
                    n > 0 ? ndop[1] * 100.0 / n : 0.0, ndop[2],
                    n > 0 ? ndop[2] * 100.0 / n : 0.0, ndop[3],
...
void __fastcall TPlot::DrawSnr(int level) {
  ...
  if (NObs > 0 && BtnSol1->Down) {
    ...
    for (int i = 0, l = 0; i < 3; i++) {
        ...
        for (int j = 0; j < Obs.n; j++) {
          ...
          if (sscanf(obstype, "L%d", &freq) == 1) {
            ...
            for (k = 0; k < NFREQ + NEXOBS; k++) {
              if (!strcmp(code2obs(obs->code[k]), obstype))
                break;
            }
            if (k >= NFREQ + NEXOBS)
              continue;
          }
          if (obs->SNR[k] * SNR_UNIT <= 0.0)
            continue;
      ...
      if (level && i == 1 && nrms > 0 && ShowStats && !BtnShowTrack->Down) {
        ...
        p1.x = p2.x - 8;
        p1.y += 3;
        DrawLabel(GraphG[i], p1, s.sprintf("AVE=%.4fm RMS=%.4fm", ave, rms), 2,
                  2);
      }
      if (BtnShowTrack->Down && 0 <= ObsIndex && ObsIndex < NObs &&
        ...
        p1.x = p2.x - 8;
        p1.y += 3;
        DrawLabel(GraphG[i], p1,
                  s.sprintf("%.*f %s", i == 1 ? 4 : 1, yp[0], unit[i].c_str()),
                  2, 2);
      }
    }
    delete[] x;
  ...
  for (int i = 0; i < 3; i++) {
    ...
    p1.y += 3;
    DrawLabel(GraphG[i], p1,
              s.sprintf("%s (%s)", label[i].c_str(), unit[i].c_str()), 1, 2);
  }
}
...
void __fastcall TPlot::DrawSnrE(int level) {
      ...
      for (int j = 0; j < Obs.n; j++) {
        ...
        if (y[0][n[0]] > 0.0) {
          if (timediff(time, Obs.data[j].time) == 0.0) {
            ...
            if (np[0] < MAXSAT && colp[0][np[0]] != clBlack)
              np[0]++;
          }
          if (n[0] < NObs)
            n[0]++;
        ...
        if (y[1][n[1]] != 0.0) {
          ...
          if (timediff(time, Obs.data[j].time) == 0.0) {
            ...
            if (np[1] < MAXSAT && colp[1][np[1]] != clBlack)
              np[1]++;
          }
          if (n[1] < NObs)
            n[1]++;
    ...
    if (BtnShowTrack->Down && 0 <= ObsIndex && ObsIndex < NObs &&
      ...
      for (int i = 0; i < 2; i++) {
        ...
        for (int j = 0; j < np[i]; j++) {
          GraphE[i]->DrawMark(xp[i][j], yp[i][j], 0, CColor[0],
                              MarkSize * 2 + 8, 0);
          GraphE[i]->DrawMark(xp[i][j], yp[i][j], 1, CColor[2],
                              MarkSize * 2 + 6, 0);
          GraphE[i]->DrawMark(xp[i][j], yp[i][j], 0, colp[i][j],
                              MarkSize * 2 + 2, 0);
        }
      }
    }
  ...
  if (ShowStats) {
    ...
    if (btn[1]->Down && nrms > 0 && !BtnShowTrack->Down) {
      ...
      p1.x = p2.x - 8;
      p1.y += 6;
      DrawLabel(GraphE[1], p1, s.sprintf("AVE=%.4fm RMS=%.4fm", ave, rms), 2,
                2);
    }
  }
}
...
void __fastcall TPlot::DrawMpS(int level) {
  ...
  if (BtnShowTrack->Down && 0 <= ObsIndex && ObsIndex < NObs) {
    for (int i = IndexObs[ObsIndex]; i < Obs.n && i < IndexObs[ObsIndex + 1];
         i++) {
      obsd_t *obs = Obs.data + i;
      int j, freq;
...
void __fastcall TPlot::DrawRes(int level) {
  UTF8String label[] = {"Pseudorange Residuals (m)",
  ...
  if (0 <= ind && ind < SolData[sel].n && BtnShowTrack->Down &&
      BtnFixHoriz->Down) {
    ...
    for (int sat = 1; sat <= MAXSAT; sat++) {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        if (!level || !(PlotStyle % 2)) {
          ...
          if (i == 2)
            DrawPolyS(GraphG[i], x[3], y[3], m[3], CColor[3], 0);
        }
        if (level && PlotStyle < 2) {
          for (int j = 0; j < m[i]; j++) {
