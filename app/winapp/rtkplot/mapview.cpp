...
TMapView *MapView;
//---------------------------------------------------------------------------
__fastcall TMapView::TMapView(TComponent *Owner) : TForm(Owner) {
  MapSel = 0;
  Lat = Lon = 0.0;
  for (int i = 0; i < 2; i++) {
...
//---------------------------------------------------------------------------
void __fastcall TMapView::BtnCloseClick(TObject *Sender) { Close(); }
//---------------------------------------------------------------------------
void __fastcall TMapView::MapSel1Click(TObject *Sender) { SelectMap(0); }
//---------------------------------------------------------------------------
void __fastcall TMapView::MapSel2Click(TObject *Sender) { SelectMap(1); }
//---------------------------------------------------------------------------
void __fastcall TMapView::BtnOptClick(TObject *Sender) {
  ...
  if (MapViewOptDialog->ShowModal() != mrOk)
    return;
...
void __fastcall TMapView::ShowMapLL(void) {
  ...
  while (fgets(buff, sizeof(buff), ifp)) {
    fputs(buff, ofp);
    if (!strstr(buff, "// start map tiles"))
      continue;
    for (i = 0, j = 1; i < 6; i++) {
      if (Plot->MapStrs[i][0] == "")
        continue;
...
void __fastcall TMapView::AddMark(int map, int index, double lat, double lon,
  ...
  func.sprintf("AddMark(%.9f,%.9f,'SOL%d','SOLUTION %d')", lat, lon, index,
               index);
  ExecFunc(map, func);
  if (state)
    func.sprintf("ShowMark('SOL%d')", index);
  else
    func.sprintf("HideMark('SOL%d')", index);
  ExecFunc(map, func);
}
//---------------------------------------------------------------------------
