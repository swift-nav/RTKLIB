...
//---------------------------------------------------------------------------
const char *PTypes[] = {"Gnd Trk",    "Position",  "Velocity",  "Accel",
                        "NSat",       "Residuals", "Resid-EL",  "Sat Vis",
                        "Skyplot",    "DOP/NSat",  "SNR/MP/EL", "SNR/MP-EL",
                        "MP-Skyplot", ""};
...
int __fastcall TPlot::ExecCmd(AnsiString cmd) {
  ...
  if (!CreateProcess(NULL, p, NULL, NULL, false, CREATE_NO_WINDOW, NULL, NULL,
                     &si, &info))
    return 0;
...
int __fastcall TPlot::GetCurrentPos(double *rr) {
  ...
  if (PLOT_OBS <= PlotType && PlotType <= PLOT_DOP)
    return 0;
...
TIMEPOS *__fastcall TPlot::SolToPos(solbuf_t *sol, int index, int qflag,
                                    int type) {
  TIMEPOS *pos, *vel, *acc;
  gtime_t ts = {0};
  sol_t *data;
  ...
  if (index >= 0) {
    if (type == 1 && index > sol->n - 2)
      index = sol->n - 2;
    if (type == 2 && index > sol->n - 3)
      index = sol->n - 3;
  }
  for (i = (index < 0) ? 0 : index; (data = getsol(sol, i)); i++) {
    ...
    if (index < 0 && !screent(data->time, ts, ts, tint))
      continue;
    if (qflag && data->stat != qflag)
      continue;
...
void __fastcall TPlot::PosToXyz(gtime_t time, const double *rr, int type,
  ...
  if (type == 0) { // xyz
    for (i = 0; i < 3; i++) {
      opos[i] = OPos[i];
      if (time.time == 0.0 || OEpoch.time == 0.0)
        continue;
...
void __fastcall TPlot::CalcStats(const double *x, int n, double ref,
                                 double &ave, double &std, double &rms) {
...
TColor __fastcall TPlot::ObsColor(const obsd_t *obs, double az, double el) {
  ...
  if (SimObs) {
    ...
    for (i = 0; i < NFREQ + NEXOBS; i++) {
      if (!strcmp(code2obs(obs->code[i]), obstype))
        break;
    }
    if (i >= NFREQ + NEXOBS) {
      return clBlack;
  ...
  if (el < ElMask * D2R ||
      (ElMaskP && el < ElMaskData[(int)(az * R2D + 0.5)])) {
...
int __fastcall TPlot::SearchPos(int x, int y) {
  ...
  for (i = 0; (data = getsol(SolData + sel, i)); i++) {
    if (QFlag->ItemIndex && data->stat != QFlag->ItemIndex)
      continue;
    ...
    if (SQR(xp - xyz[0]) + SQR(yp - xyz[1]) <= SQR(r))
      return i;
  }
  return -1;
}
...
void __fastcall TPlot::TimeStr(gtime_t time, int n, int tsys, char *str) {
  ...
  if (TimeLabel == 0) { // www/ssss
    ...
    if (!(t = localtime(&time.time)))
      ...
      sprintf(tstr, "%04d/%02d/%02d %02d:%02d:%02d.%0*d", t->tm_year + 1900,
...
UTF8String __fastcall TPlot::LatLonStr(const double *pos, int ndec) {
  ...
  if (LatLonFmt == 0) {
    s.sprintf("%*.*f" CHARDEG "%s %*.*f" CHARDEG "%s", ndec + 4, ndec,
              fabs(pos[0] * R2D), pos[0] < 0.0 ? "S" : "N", ndec + 5, ndec,
              fabs(pos[1] * R2D), pos[1] < 0.0 ? "W" : "E");
  } else {
    deg2dms(pos[0] * R2D, dms1, ndec - 5);
    deg2dms(pos[1] * R2D, dms2, ndec - 5);
    s.sprintf("%3.0f" CHARDEG "%02.0f'%0*.*f\"%s %4.0f" CHARDEG
              "%02.0f'%0*.*f\"%s",
              fabs(dms1[0]), dms1[1], ndec - 2, ndec - 5, dms1[2],
              pos[0] < 0.0 ? "S" : "N", fabs(dms2[0]), dms2[1], ndec - 2,
              ndec - 5, dms2[2], pos[1] < 0.0 ? "W" : "E");
  }
  return s;
}
