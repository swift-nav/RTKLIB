...
void __fastcall TMainWindow::Panel2Resize(TObject *Sender) {
  TButton *btns1[] = {BtnOutFile1, BtnOutFile2, BtnOutFile3,
                      BtnOutFile4, BtnOutFile5, BtnOutFile6,
                      BtnOutFile7, BtnOutFile8, BtnOutFile9};
  ...
  InFile->Width =
      w - BtnInFile->Width - BtnInFileView->Width - 6 - InFile->Left;
...
void __fastcall TMainWindow::BtnPlotClick(TObject *Sender) {
  ...
  TCheckBox *cb[] = {OutFileEna1, OutFileEna2, OutFileEna3, OutFileEna4,
                     OutFileEna5, OutFileEna6, OutFileEna7, OutFileEna8};
  int i, ena[8];

  for (i = 0; i < 8; i++)
    ena[i] = cb[i]->Enabled && cb[i]->Checked;
...
void __fastcall TMainWindow::BtnOptionsClick(TObject *Sender) {
  int rnxfile = RnxFile;
  if (ConvOptDialog->ShowModal() != mrOk)
    return;
  if (RnxFile != rnxfile) {
    SetOutFiles(InFile->Text);
  }
...
void __fastcall TMainWindow::GetTime(gtime_t *ts, gtime_t *te, double *tint,
  ...
  if (TimeUnitF->Checked) {
    if (sscanf(TimeUnit_Text.c_str(), "%lf", tunit) >= 1)
      *tunit *= 3600.0;
  }
}
...
void __fastcall TMainWindow::TimeY1KeyDown(TObject *Sender, WORD &Key,
                                           TShiftState Shift) {
  bool allowchange;
...
void __fastcall TMainWindow::TimeH1KeyDown(TObject *Sender, WORD &Key,
                                           TShiftState Shift)
...
void __fastcall TMainWindow::TimeY2KeyDown(TObject *Sender, WORD &Key,
                                           TShiftState Shift)
...
void __fastcall TMainWindow::TimeH2KeyDown(TObject *Sender, WORD &Key,
                                           TShiftState Shift)
...
void __fastcall TMainWindow::UpdateEnable(void) {
  ...
  TimeUnit->Enabled =
      TimeStartF->Checked && TimeEndF->Checked && TimeUnitF->Checked;
...
void __fastcall TMainWindow::ConvertFile(void) {
  ...
  if (format == STRFMT_RTCM2 || format == STRFMT_RTCM3 ||
    ...
    if (StartDialog->ShowModal() != mrOk)
      return;
    rnxopt.trtcm = StartDialog->Time;
  }
  if (OutFile1->Enabled && OutFileEna1->Checked)
    strcpy(ofile[0], OutFile1_Text.c_str());
  if (OutFile2->Enabled && OutFileEna2->Checked)
    strcpy(ofile[1], OutFile2_Text.c_str());
  ...
  if (OutFile8->Enabled && OutFileEna8->Checked)
    strcpy(ofile[7], OutFile8_Text.c_str());
  if (OutFile9->Enabled && OutFileEna9->Checked)
    strcpy(ofile[8], OutFile9_Text.c_str());
...
void __fastcall TMainWindow::LoadOpt(void) {
  ...
  AnsiString opt, mask = "11111111111111111111111111111111111111111111111111111"
                         "111111111111111";
