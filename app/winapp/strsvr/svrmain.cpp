...
void __fastcall TMainForm::BtnOptClick(TObject *Sender) {
  ...
  if (SvrOptDialog->ShowModal() != mrOk)
    return;
...
void __fastcall TMainForm::BtnCmdClick(TObject *Sender) {
  ...
  if (CmdOptDialog->ShowModal() != mrOk)
    return;
  for (j = 0; j < 3; j++) {
    if (type[i]->Text == "Serial") {
...
void __fastcall TMainForm::BtnOutputClick(TObject *Sender) {
  TButton *btn[] = {BtnOutput1, BtnOutput2, BtnOutput3,
                    BtnOutput4, BtnOutput5, BtnOutput6};
  TComboBox *type[] = {Output1, Output2, Output3, Output4, Output5, Output6};
  int i;

  for (i = 0; i < MAXSTR - 1; i++) {
    if ((TButton *)Sender == btn[i])
      break;
  }
  if (i >= MAXSTR - 1)
    return;
...
void __fastcall TMainForm::Timer1Timer(TObject *Sender) {
  ...
  TPanel *e0[] = {IndInput,   IndOutput1, IndOutput2, IndOutput3,
                  IndOutput4, IndOutput5, IndOutput6};
  ...
  TLabel *e2[] = {InputBps,   Output1Bps, Output2Bps, Output3Bps,
                  Output4Bps, Output5Bps, Output6Bps};
  ...
  t[3] = ctime;
  ConTime->Caption =
      s.sprintf("%.0fd %02.0f:%02.0f:%02.0f", t[0], t[1], t[2], t[3]);
...
void __fastcall TMainForm::SvrStart(void) {
  ...
  char *paths[MAXSTR], *logs[MAXSTR], *cmds[MAXSTR] = {0},
                                      *cmds_periodic[MAXSTR] = {0};
  ...
  strcpy(logs[0],
         type[0]->ItemIndex > 5 || !PathEna[0] ? "" : PathLog[0].c_str());

  for (int i = 1; i < MAXSTR; i++) {
    strs[i] = otype[type[i]->ItemIndex];
    strcpy(paths[i],
           !type[i]->ItemIndex ? "" : Paths[i][type[i]->ItemIndex - 1].c_str());
    strcpy(logs[i], !PathEna[i] ? "" : PathLog[i].c_str());
  }
  for (int i = 0; i < MAXSTR; i++) {
    if (strs[i] == STR_SERIAL) {
      if (CmdEna[i][0])
        cmds[i] = MainForm->Cmds[i][0].c_str();
      if (CmdEna[i][2])
        cmds_periodic[i] = MainForm->Cmds[i][2].c_str();
    } else if (strs[i] == STR_TCPCLI || strs[i] == STR_NTRIPCLI) {
      if (CmdEnaTcp[i][0])
        cmds[i] = MainForm->CmdsTcp[i][0].c_str();
      if (CmdEnaTcp[i][2])
        cmds_periodic[i] = MainForm->CmdsTcp[i][2].c_str();
    }
  }
  for (int i = 0; i < 5; i++) {
    opt[i] = SvrOpt[i];
  ...
  for (int i = 0; i < MAXSTR - 1; i++) { // for each out stream
    if (Input->ItemIndex == 2 || Input->ItemIndex == 4)
      continue;
    if (!ConvEna[i])
      continue;
    if (!(conv[i] = strconvnew(ConvInp[i], ConvOut[i], ConvMsg[i].c_str(),
                               StaId, StaSel, ConvOpt[i].c_str())))
      continue;
  ...
  // stream server start
  if (!strsvrstart(&strsvr, opt, strs, paths, logs, conv, cmds, cmds_periodic,
                   AntPos)) {
    return;
  }
...
void __fastcall TMainForm::SvrStop(void) {
  ...
  for (int i = 0; i < MAXSTR; i++) {
    if (strs[i] == STR_SERIAL) {
      if (CmdEna[i][1])
        cmds[i] = MainForm->Cmds[i][1].c_str();
    } else if (strs[i] == STR_TCPCLI || strs[i] == STR_NTRIPCLI) {
      if (CmdEnaTcp[i][1])
        cmds[i] = MainForm->CmdsTcp[i][1].c_str();
    }
  }
...
void __fastcall TMainForm::Timer2Timer(TObject *Sender) {
  const char *types[] = {"None",         "Serial",     "File",
                         "TCP Server",   "TCP Client", "Ntrip Sever",
                         "Ntrip Client", "FTP",        "HTTP",
                         "Ntrip Cast",   "UDP Sever",  "UDP Client"};
...
void __fastcall TMainForm::SerialOpt(int index, int path) {
  ...
  if (SerialOptDialog->ShowModal() != mrOk)
    return;
...
void __fastcall TMainForm::FileOpt(int index, int path) {
  ...
  if (FileOptDialog->ShowModal() != mrOk)
    return;
...
void __fastcall TMainForm::UpdateEnable(void) {
  ...
  TLabel *label2[] = {Output1Byte, Output2Byte, Output3Byte,
                      Output4Byte, Output5Byte, Output6Byte};
  TLabel *label3[] = {Output1Bps, Output2Bps, Output3Bps,
                      Output4Bps, Output5Bps, Output6Bps};
  TButton *btn1[] = {BtnOutput1, BtnOutput2, BtnOutput3,
                     BtnOutput4, BtnOutput5, BtnOutput6};
  TButton *btn2[] = {BtnCmd1, BtnCmd2, BtnCmd3, BtnCmd4, BtnCmd5, BtnCmd6};
  TButton *btn3[] = {BtnConv1, BtnConv2, BtnConv3,
                     BtnConv4, BtnConv5, BtnConv6};
  ...
  for (int i = 0; i < MAXSTR - 1; i++) {
    ...
    btn2[i]->Enabled = btn1[i]->Enabled &&
                       (type[i]->ItemIndex == 1 || type[i]->ItemIndex == 2);
    btn3[i]->Enabled =
        btn1[i]->Enabled && Input->ItemIndex != 2 && Input->ItemIndex != 4;
    btn4[i]->Enabled = btn1[i]->Enabled &&
                       (type[i]->ItemIndex == 1 || type[i]->ItemIndex == 2);
  }
}
// set task-tray icon -------------------------------------------------------
...
void __fastcall TMainForm::LoadOpt(void) {
  ...
  for (int i = 0; i < MAXSTR; i++)
    for (int j = 0; j < 3; j++) {
      CmdEna[i][j] =
          ini->ReadInteger("serial", s.sprintf("cmdena_%d_%d", i, j), 1);
      CmdEnaTcp[i][j] =
          ini->ReadInteger("tcpip", s.sprintf("cmdena_%d_%d", i, j), 1);
    }
  for (int i = 0; i < MAXSTR; i++)
    for (int j = 0; j < 7; j++) {
...
void __fastcall TMainForm::SaveOpt(void) {
  ...
  for (int i = 0; i < MAXHIST; i++) {
    ini->WriteString("tcpopt", s.sprintf("history%d", i),
...
void __fastcall TMainForm::BtnLogClick(TObject *Sender) {
  TButton *btn[] = {BtnLog,  BtnLog1, BtnLog2, BtnLog3,
                    BtnLog4, BtnLog5, BtnLog6};
  int i;
  ...
  FileOptDialog->Caption =
      (i == 0) ? "Input Log Options" : "Return Log Options";
  FileOptDialog->Opt = 2;
  if (FileOptDialog->ShowModal() != mrOk)
    return;
