...
void __fastcall TMainForm::BtnTestClick(TObject *Sender) {
  TTextViewer *viewer;
  TImage *img[] = {Image1, Image2, Image3, Image4,
...
void __fastcall TMainForm::BtnOptsClick(TObject *Sender) {
  ...
  if (DownOptDialog->ShowModal() != mrOk)
    return;
...
void __fastcall TMainForm::BtnDownloadClick(TObject *Sender) {
  ...
  str = Number->Text;
  if (sscanf(str.c_str(), "%d-%d", &seqnos, &seqnoe) == 1)
    seqnoe = seqnos;
...
void __fastcall TMainForm::BtnStasClick(TObject *Sender) {
  if (StaListDialog->ShowModal() != mrOk)
    return;
  UpdateStaList();
  BtnAll->Caption = "A";
}
...
void __fastcall TMainForm::LoadUrl(AnsiString file) {
  ...
  for (i = 0; i < n; i++) {
    ...
    if (!(p = strchr(urls[i].type, '_')))
      continue;
    *p = '\0';
    for (j = 0; j < DataType->Items->Count; j++) {
      if (DataType->Items->Strings[j] == urls[i].type)
        break;
    }
    ...
    subtype = p + 1;
    if ((p = strchr(subtype, '_')))
      *p = '\0';
    for (j = 0; j < SubType->Items->Count; j++) {
      if (SubType->Items->Strings[j] == subtype)
        break;
    }
...
void __fastcall TMainForm::UpdateType(void) {
  ...
  for (i = 0; i < Types->Count; i++) {
    ...
    if (p && (p = strchr(p + 1, '_')))
      *p = '\0';
    if (DataType->Text != "ALL" && DataType->Text != type)
      continue;
    if (SubType->Text != "" && SubType->Text != subtype)
      continue;
...
void __fastcall TMainForm::UpdateMsg(void) {
  ...
  for (i = 0; i < DataList->Count; i++) {
    ...
    for (j = 0; j < Types->Count; j++) {
      if (DataList->Items->Strings[i] != Types->Strings[j])
        continue;
...
// --------------------------------------------------------------------------
void __fastcall TMainForm::ReadHist(TIniFile *ini, AnsiString key,
                                    TStrings *list) {
  AnsiString s, item;
  int i;
...
// --------------------------------------------------------------------------
void __fastcall TMainForm::WriteHist(TIniFile *ini, AnsiString key,
                                     TStrings *list) {
  AnsiString s;
  int i;

  for (i = 0; i < list->Count; i++) {
    ini->WriteString("history", s.sprintf("%s_%03d", key.c_str(), i),
                     list->Strings[i]);
  }
}
// --------------------------------------------------------------------------
void __fastcall TMainForm::AddHist(TComboBox *combo) {
  ...
  if (i >= 0)
    list->Delete(i);
  list->Insert(0, hist);
  for (int i = list->Count - 1; i >= MAX_HIST; i--)
    list->Delete(i);
  combo->ItemIndex = 0;
}
//---------------------------------------------------------------------------
