...
void __fastcall TTcpOptDialog::FormShow(TObject *Sender) {
  ...
  const char *ti[] = {"TCP Server Options ",  "TCP Client Options",
                      ...
                      "NTRIP Caster Options", "NTRIP Caster Server Options",
  ...
  Caption = ti[Opt];
  LabelAddr->Caption =
      (Opt >= 2 && Opt <= 5) ? "NTRIP Caster Address" : "Server Address";
...
void __fastcall TTcpOptDialog::BtnNtripClick(TObject *Sender) {
  ...
  strinit(&str);
  if (!stropen(&str, STR_NTRIPCLI, STR_MODE_R, path.c_str()))
    return;
...
void __fastcall TTcpOptDialog::BtnMountpClick(TObject *Sender) {
  ...
  if (MntpOptDialog->ShowModal() != mrOk)
    return;
