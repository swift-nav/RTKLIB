...
void TGraph::DrawMark(double x, double y, int mark, TColor color,
  ...
  TPoint p;
  if (ToPoint(x, y, p))
    DrawMark(p, mark, color, bgcolor, size, rot);
}
//---------------------------------------------------------------------------
void TGraph::DrawMarks(const double *x, const double *y, const TColor *color,
  ...
  TPoint p, pp;
  for (int i = 0; i < n; i++) {
    if (!ToPoint(x[i], y[i], p) || (pp.x == p.x && pp.y == p.y))
      continue;
...
//---------------------------------------------------------------------------
void TGraph::DrawText(double x, double y, UTF8String str, TColor color, int ha,
...
//---------------------------------------------------------------------------
void TGraph::DrawCircle(double x, double y, TColor color, double rx, double ry,
                        int style) {
  TPoint p;
  ToPoint(x, y, p);
  DrawCircle(p, color, (int)(rx / XScale + 0.5), (int)(ry / YScale + 0.5),
             style);
}
//---------------------------------------------------------------------------
...
void TGraph::DrawPoly(double *x, double *y, int n, TColor color, int style) {
  ...
  for (int i = 0; i < n; i++) {
    ToPoint(x[i], y[i], p[m]);
    if (m == 0 || p[m - 1].x != p[m].x || p[m - 1].y != p[m].y)
      m++;
  }
...
//---------------------------------------------------------------------------
void TGraph::DrawSkyPlot(TPoint p, TColor color1, TColor color2, TColor bgcolor,
                         int size) {
  TCanvas *c = Canvas;
  c->Pen->Color = color1;
  c->Brush->Style = bsClear;
