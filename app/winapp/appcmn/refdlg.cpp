...
void __fastcall TRefDialog::LoadList(void) {
  ...
  // check format
  if (!(fp = fopen(StaPosFile.c_str(), "r")))
    return;
  ...
  if (n == 0) {
    StaList->RowCount = 2;
    for (int i = 0; i < StaList->ColCount; i++)
      StaList->Cells[i][1] = "";
  }
  fclose(fp);
  UpdateDist();
...
void __fastcall TRefDialog::LoadSinex(void) {
  ...
  while (fgets(buff, sizeof(buff), fp)) {
    if (strstr(buff, "+SOLUTION/ESTIMATE"))
      ...
      sol = 0;
    if (!sol || strlen(buff) < 68)
      continue;
    if (!strncmp(buff + 7, "STAX", 4)) {
      if (sscanf(buff + 47, "%lf", rr) < 1)
        continue;
      strncpy(code, buff + 14, 4);
      code[4] = '\0';
    } else if (!strncmp(buff + 7, "STAY", 4)) {
      if (sscanf(buff + 47, "%lf", rr + 1) < 1)
        continue;
      if (strncmp(code, buff + 14, 4))
        continue;
    } else if (!strncmp(buff + 7, "STAZ", 4)) {
      if (sscanf(buff + 47, "%lf", rr + 2) < 1)
        continue;
      if (strncmp(code, buff + 14, 4))
        continue;
      ecef2pos(rr, pos);
