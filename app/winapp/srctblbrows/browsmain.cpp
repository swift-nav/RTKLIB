...
void __fastcall TMainForm::FormShow(TObject *Sender) {
  AnsiString colw0 =
      "30,74,116,56,244,18,52,62,28,50,50,18,18,120,28,18,18,40,300,";
...
void __fastcall TMainForm::MenuOpenClick(TObject *Sender) {
  ...
  if (!OpenDialog->Execute())
    return;
  SrcTable = "";
  if (!(fp = fopen(OpenDialog_FileName.c_str(), "rb")))
    return;
...
void __fastcall TMainForm::MenuSaveClick(TObject *Sender) {
  ...
  if (!SaveDialog->Execute())
    return;
  if (!(fp = fopen(SaveDialog_FileName.c_str(), "wb")))
    return;
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::Table0SelectCell(TObject *Sender, int ACol, int ARow,
                                            bool &CanSelect) {
  AnsiString title;
  if (0 < ARow && ARow < Table0->RowCount) {
    ...
    GoogleMapView->Caption =
        "NTRIP Data Stream Map: " + Address->Text + "/" + title;
  }
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::Table0MouseDown(TObject *Sender, TMouseButton Button,
  ...
  if (row == 0)
    SortTable(Table0, col);
}
//---------------------------------------------------------------------------
void __fastcall TMainForm::Table1MouseDown(TObject *Sender, TMouseButton Button,
                                           TShiftState Shift, int X, int Y) {
  int col, row;
  Table1->MouseToCell(X, Y, col, row);
  if (row == 0)
    SortTable(Table1, col);
}
//---------------------------------------------------------------------------
void __fastcall TMainForm::Table2MouseDown(TObject *Sender, TMouseButton Button,
                                           TShiftState Shift, int X, int Y) {
  int col, row;
  Table2->MouseToCell(X, Y, col, row);
  if (row == 0)
    SortTable(Table2, col);
}
//---------------------------------------------------------------------------
void __fastcall TMainForm::UpdateCaster(void) {
  ...
  for (p = srctbl; *p; p = q + 1) {
    ...
    if (strncmp(buff, "CAS", 3))
      continue;
    for (i = 0, r = strtok(buff, ";"); i < 3 && p; i++, r = strtok(NULL, ";"))
      item[i] = r;
    Address->AddItem(item[1] + ":" + item[2], NULL);
  }
  if (Address->Items->Count > 1)
    Address->Text = Address->Items->Strings[1];
}
//---------------------------------------------------------------------------
...
void __fastcall TMainForm::ShowTable(void) {
  const char *ti[][19] = {
      {"No", "Mountpoint", "ID", "Format", "Format-Details", "Carrier",
       "Nav-System", "Network", "Country", "Latitude", "Longitude", "NMEA",
       "Solution", "Generator", "Compr-Encrp", "Authentication", "Fee",
       "Bitrate", ""},
      {"No", "Host", "Port", "ID", "Operator", "NMEA", "Country", "Latitude",
       "Longitude", "Fallback_Host", "Fallback_Port", "", "", "", "", "", "",
       "", ""},
      {"No", "ID", "Operator", "Authentication", "Fee", "Web-Net", "Web-Str",
       "Web-Reg", "", "", "", "", "", "", "", "", "", "", ""}};
...
void __fastcall TMainForm::UpdateMap(void) {
  ...
  for (int i = 1; i < Table0->RowCount; i++) {
    ...
    msg = "<b>" + Table0->Cells[1][i] + "</b>: " + Table0->Cells[2][i] + " (" +
          ...
          "Latitude/Longitude: " + Table0->Cells[9][i] + ", " +
          Table0->Cells[10][i] + "<br>" + "Generator: " + Table0->Cells[13][i];
