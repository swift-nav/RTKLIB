...
void __fastcall TInputStrDialog::FormShow(TObject *Sender) {
  ...
  Format3->ItemIndex =
      Format[2] < NRcv ? Format[2] : NRcv + Format[2] - STRFMT_SP3;
...
void __fastcall TInputStrDialog::BtnOkClick(TObject *Sender) {
  ...
  Format[1] = Format2->ItemIndex < NRcv
                  ? Format2->ItemIndex
                  : STRFMT_SP3 + Format2->ItemIndex - NRcv;
  Format[2] = Format3->ItemIndex < NRcv
                  ? Format3->ItemIndex
                  : STRFMT_SP3 + Format3->ItemIndex - NRcv;
...
void __fastcall TInputStrDialog::BtnRcvOpt1Click(TObject *Sender) {
  ...
  if (RcvOptDialog->ShowModal() != mrOk)
    return;
...
void __fastcall TInputStrDialog::FtpOpt(int index, int opt) {
  ...
  if (FtpOptDialog->ShowModal() != mrOk)
    return;
