...
//---------------------------------------------------------------------------
void __fastcall TMonitorDialog::FormClose(TObject *Sender,
                                          TCloseAction &Action) {
  free_rtcm(&rtcm);
  free_raw(&raw);
  Release();
...
void __fastcall TMonitorDialog::ClearTable(void) {
  ...
  SelStr->Left = (TypeF == 14) ? SelSys->Left + SelSys->Width + 1
                               : Type->Left + Type->Width + 1;
}
//---------------------------------------------------------------------------
...
void __fastcall TMonitorDialog::ShowRtk(void) {
  ...
  AnsiString mode[] = {"Single",        "DGPS",        "Kinematic",
  ...
  char file[1024] = "";
  const char *ionoopt[] = {
      "OFF",           "Broadcast", "SBAS",     "Dual-Frequency",
      "Estimate STEC", "IONEX TEC", "QZSS LEX", ""};
  const char *tropopt[] = {"OFF",          "Saastamoinen",      "SBAS",
                           "Estimate ZTD", "Estimate ZTD+Grad", ""};
  const char *ephopt[] = {"Broadcast",
                          "Precise",
                          "Broadcast+SBAS",
                          "Broadcat+SSR APC",
                          "Broadcast+SSR CoM",
  ...
  if (rtk.opt.navsys & SYS_GLO)
    navsys = navsys + "GLONASS ";
  if (rtk.opt.navsys & SYS_GAL)
    navsys = navsys + "Galileo ";
  ...
  Tbl->Cells[1][i++] = s.sprintf("%s, %s", rtk.opt.dynamics ? "ON" : "OFF",
                                 rtk.opt.tidecorr ? "ON" : "OFF");

  Tbl->Cells[0][i] = "Ionosphere/Troposphere Model";
  Tbl->Cells[1][i++] =
      s.sprintf("%s, %s", ionoopt[rtk.opt.ionoopt], tropopt[rtk.opt.tropopt]);
  ...
  Tbl->Cells[1][i++] = s.sprintf(
      "Obs(%d), Nav(%d), Ion(%d), Sbs(%d), Pos(%d), Dgps(%d), Ssr(%d), Err(%d)",
      nmsg[0][0], nmsg[0][1] + nmsg[0][6], nmsg[0][2], nmsg[0][3], nmsg[0][4],
      nmsg[0][5], nmsg[0][7], nmsg[0][9]);

  Tbl->Cells[0][i] = "# of Input Data Base Station";
  Tbl->Cells[1][i++] = s.sprintf(
      "Obs(%d), Nav(%d), Ion(%d), Sbs(%d), Pos(%d), Dgps(%d), Ssr(%d), Err(%d)",
      nmsg[1][0], nmsg[1][1] + nmsg[1][6], nmsg[1][2], nmsg[1][3], nmsg[1][4],
      nmsg[1][5], nmsg[1][7], nmsg[1][9]);

  Tbl->Cells[0][i] = "# of Input Data Ephemeris";
  Tbl->Cells[1][i++] = s.sprintf(
      "Obs(%d), Nav(%d), Ion(%d), Sbs(%d), Pos(%d), Dgps(%d), Ssr(%d), Err(%d)",
      nmsg[2][0], nmsg[2][1] + nmsg[2][6], nmsg[2][2], nmsg[2][3], nmsg[2][4],
      nmsg[2][5], nmsg[2][7], nmsg[2][9]);
  ...
  Tbl->Cells[0][i] =
      "Time Sytem Offset/Receiver Bias (GLO-GPS,GAL-GPS,BDS-GPS,IRN-GPS) (ns)";
  ...
  Tbl->Cells[1][i++] =
      s.sprintf("%.1f, %.1f, %.1f, %.1f", dop[0], dop[1], dop[2], dop[3]);
  ...
  Tbl->Cells[1][i++] = s.sprintf("%.3f, %.3f, %.3f", rtk.sol.rr[0],
  ...
  Tbl->Cells[1][i++] =
      s.sprintf("%.8f, %.8f, %.3f", pos[0] * R2D, pos[1] * R2D, pos[2]);
  ...
  Tbl->Cells[1][i++] =
      s.sprintf("%.3f, %.3f, %.3f, %.3f", norm(rr, 3), enu[0], enu[1], enu[2]);
  ...
  for (j = 0; j < NFREQ; j++) {
    ...
    Tbl->Cells[0][i] =
        s.sprintf("Ant Phase Center L%d E/N/U (m) Base Station", j + 1);
    Tbl->Cells[1][i++] = s.sprintf("%.3f, %.3f, %.3f", off[0], off[1], off[2]);
  }
...
void __fastcall TMonitorDialog::SetSat(void) {
  int i, j = 0;
  AnsiString s,
      label[] = {"SAT",     "Status",   "Azimuth (deg)", "Elevation (deg)",
                 "LG (m)",  "PHW(cyc)", "P1-P2(m)",      "P1-C1(m)",
                 "P2-C2(m)"};
...
void __fastcall TMonitorDialog::ShowSat(void) {
  ...
  for (i = 0, n = 1; i < MAXSAT; i++) {
    ...
    for (k = 0; k < nfreq; k++) {
      fix = ssat->fix[k];
      Tbl->Cells[j++][n] =
          fix == 1 ? "FLOAT" : (fix == 2 ? "FIX" : (fix == 3 ? "HOLD" : "-"));
    }
    for (k = 0; k < nfreq; k++) {
...
void __fastcall TMonitorDialog::SetEst(void) {
  AnsiString label[] = {"State", "Estimate Float", "Std Float",
                        "Estimate Fixed", "Std Fixed"};
...
void __fastcall TMonitorDialog::ShowEst(void) {
  ...
  for (i = 0, n = 1; i < nx; i++) {
    ...
    Tbl->Cells[j++][n] =
        P[i + i * nx] == 0.0 ? s0 : s.sprintf("%.3f", SQRT(P[i + i * nx]));
    Tbl->Cells[j++][n] = i >= na || xa[i] == 0 ? s0 : s.sprintf("%.3f", xa[i]);
    Tbl->Cells[j++][n] = i >= na || Pa[i + i * na] == 0.0
                             ? s0
                             : s.sprintf("%.3f", SQRT(Pa[i + i * na]));
    n++;
  }
  free(x);
  free(P);
...
void __fastcall TMonitorDialog::ShowCov(void) {
  ...
  for (i = 0, n = 1; i < nx; i++) {
    if (SelSat->ItemIndex == 1 && (x[i] == 0.0 || P[i + i * nx] == 0.0))
      continue;
    n++;
  }
  if (n < 2) {
  ...
  for (i = 0, n = 1; i < nx; i++) {
    ...
    for (j = 0, m = 1; j < nx; j++) {
      if (SelSat->ItemIndex == 1 && (x[j] == 0.0 || P[j + j * nx] == 0.0))
        continue;
      Tbl->Cells[m][n] =
...
void __fastcall TMonitorDialog::SetObs(void) {
  ...
  for (i = 0; i < NFREQ + nex; i++) {
    ...
    Tbl->Cells[j][0] =
        i < NFREQ ? s.sprintf("C%d", i + 1) : s.sprintf("CX%d", i - NFREQ + 1);
    Tbl->Cells[j++][1] = "";
  }
  for (i = 0; i < NFREQ + nex; i++) {
    Tbl->ColWidths[j] = 30 * FontScale / 96;
    Tbl->Cells[j][0] =
        i < NFREQ ? s.sprintf("S%d", i + 1) : s.sprintf("SX%d", i - NFREQ + 1);
    Tbl->Cells[j++][1] = "";
  }
  for (i = 0; i < NFREQ + nex; i++) {
    Tbl->ColWidths[j] = 80 * FontScale / 96;
    Tbl->Cells[j][0] = i < NFREQ ? s.sprintf("P%d (m)", i + 1)
                                 : s.sprintf("PX%d (m)", i - NFREQ + 1);
    Tbl->Cells[j++][1] = "";
  }
  for (i = 0; i < NFREQ + nex; i++) {
    Tbl->ColWidths[j] = 85 * FontScale / 96;
    Tbl->Cells[j][0] = i < NFREQ ? s.sprintf("L%d (cycle)", i + 1)
                                 : s.sprintf("LX%d (cycle)", i - NFREQ + 1);
    Tbl->Cells[j++][1] = "";
  }
  for (i = 0; i < NFREQ + nex; i++) {
    Tbl->ColWidths[j] = 60 * FontScale / 96;
    Tbl->Cells[j][0] = i < NFREQ ? s.sprintf("D%d (Hz)", i + 1)
                                 : s.sprintf("DX%d (Hz)", i - NFREQ + 1);
    Tbl->Cells[j++][1] = "";
  }
  for (i = 0; i < NFREQ + nex; i++) {
...
void __fastcall TMonitorDialog::ShowObs(void) {
  ...
  int i, j, k, n = 0, nex = ObsMode ? NEXOBS : 0,
               sys = sys_tbl[SelSys->ItemIndex];

  rtksvrlock(&rtksvr);
  for (i = 0; i < rtksvr.obs[0][0].n && n < MAXOBS * 2; i++) {
    if (!(satsys(rtksvr.obs[0][0].data[i].sat, NULL) & sys))
      continue;
    obs[n++] = rtksvr.obs[0][0].data[i];
  }
  for (i = 0; i < rtksvr.obs[1][0].n && n < MAXOBS * 2; i++) {
    if (!(satsys(rtksvr.obs[1][0].data[i].sat, NULL) & sys))
      continue;
...
void __fastcall TMonitorDialog::SetNav(void) {
  ...
  int i, width[] = {25, 25, 30, 30, 30, 25, 25, 115, 115, 115, 80,
                    80, 60, 60, 60, 60, 70, 70, 70,  70,  50,  50,
                    50, 50, 70, 70, 50, 50, 70, 70,  30,  30};
  Tbl->ColCount = 32;
  Tbl->RowCount = 2;
  for (i = 0; i < Tbl->ColCount; i++) {
    Tbl->ColWidths[i] = width[i] * FontScale / 96;
...
void __fastcall TMonitorDialog::ShowIonUtc(void) {
  ...
  for (i = 0; i < 8; i++)
    utc_cmp[i] = rtksvr.nav.utc_cmp[i];
  for (i = 0; i < 9; i++)
    utc_irn[i] = rtksvr.nav.utc_irn[i];
  for (i = 0; i < 8; i++)
  ...
  for (i = 0; i < 8; i++)
    ion_cmp[i] = rtksvr.nav.ion_cmp[i];
  for (i = 0; i < 8; i++)
    ion_irn[i] = rtksvr.nav.ion_irn[i];
  rtksvrunlock(&rtksvr);
  ...
  Tbl->Cells[1][i++] = s.sprintf("%.0f, %.0f, %.0f, %.0f", utc_gps[4],
                                 utc_gps[5], utc_gps[6], utc_gps[7]);

  Tbl->Cells[0][i] = "GPST-UTC Ref Week, Time(s), A0(ns), A1(ns/s)";
  Tbl->Cells[1][i++] =
      s.sprintf("%.0f, %.0f, %.3f, %.5E", utc_gps[3], utc_gps[2],
  ...
  Tbl->Cells[1][i++] =
      s.sprintf("%.0f, %.0f, %.3f, %.5E", utc_gal[3], utc_gal[2],
                utc_gal[0] * 1E9, utc_gal[1] * 1E9);

  Tbl->Cells[0][i] = "QZSST-UTC Ref Week, Time(s), A0(ns), A1(ns/s)";
  Tbl->Cells[1][i++] =
      s.sprintf("%.0f, %.0f, %.3f, %.5E", utc_qzs[3], utc_qzs[2],
                utc_qzs[0] * 1E9, utc_qzs[1] * 1E9);

  Tbl->Cells[0][i] = "BDST-UTC Ref Week, Time(s), A0(ns), A1(ns/s)";
  Tbl->Cells[1][i++] =
      s.sprintf("%.0f, %.0f, %.3f, %.5E", utc_cmp[3], utc_cmp[2],
                utc_cmp[0] * 1E9, utc_cmp[1] * 1E9);

  Tbl->Cells[0][i] = "IRNT-UTC Ref Week,Time(s), A0(ns), A1(ns/s), A2(ns/s2)";
  Tbl->Cells[1][i++] =
      s.sprintf("%.0f, %.0f, %.3f, %.5E, %.5E", utc_irn[3], utc_irn[2],
                utc_irn[0] * 1E9, utc_irn[1] * 1E9, utc_irn[8] * 1E9);

  Tbl->Cells[0][i] = "GPS Iono Parameters Alpha0-3";
  Tbl->Cells[1][i++] = s.sprintf("%.5E, %.5E, %.5E, %.5E", ion_gps[0],
                                 ion_gps[1], ion_gps[2], ion_gps[3]);

  Tbl->Cells[0][i] = "GPS Iono Parameters Beta0-3";
  Tbl->Cells[1][i++] = s.sprintf("%.5E, %.5E, %.5E, %.5E", ion_gps[4],
                                 ion_gps[5], ion_gps[6], ion_gps[7]);

  Tbl->Cells[0][i] = "Galileo Iono Parameters 0-2";
  Tbl->Cells[1][i++] =
      s.sprintf("%.5E, %.5E, %.5E", ion_gal[0], ion_gal[1], ion_gal[2]);

  Tbl->Cells[0][i] = "QZSS Iono Parameters Alpha0-3";
  Tbl->Cells[1][i++] = s.sprintf("%.5E, %.5E, %.5E, %.5E", ion_qzs[0],
                                 ion_qzs[1], ion_qzs[2], ion_qzs[3]);

  Tbl->Cells[0][i] = "QZSS Iono Parameters Beta0-3";
  Tbl->Cells[1][i++] = s.sprintf("%.5E, %.5E, %.5E, %.5E", ion_qzs[4],
                                 ion_qzs[5], ion_qzs[6], ion_qzs[7]);

  Tbl->Cells[0][i] = "BDS Iono Parameters Alpha0-3";
  Tbl->Cells[1][i++] = s.sprintf("%.5E, %.5E, %.5E, %.5E", ion_cmp[0],
                                 ion_cmp[1], ion_cmp[2], ion_cmp[3]);

  Tbl->Cells[0][i] = "BDS Iono Parameters Beta0-3";
  Tbl->Cells[1][i++] = s.sprintf("%.5E, %.5E, %.5E, %.5E", ion_cmp[4],
                                 ion_cmp[5], ion_cmp[6], ion_cmp[7]);

  Tbl->Cells[0][i] = "NavIC Iono Parameters Alpha0-3";
  Tbl->Cells[1][i++] = s.sprintf("%.5E, %.5E, %.5E, %.5E", ion_irn[0],
                                 ion_irn[1], ion_irn[2], ion_irn[3]);

  Tbl->Cells[0][i] = "NavIC Iono Parameters Beta0-3";
  Tbl->Cells[1][i++] = s.sprintf("%.5E, %.5E, %.5E, %.5E", ion_irn[4],
                                 ion_irn[5], ion_irn[6], ion_irn[7]);
}
//---------------------------------------------------------------------------
void __fastcall TMonitorDialog::SetStr(void) {
  AnsiString label[] = {"STR",          "Stream",     "Type",
                        "Format",       "Mode",       "State",
                        "Input(bytes)", "Input(bps)", "Output(bytes)",
                        "Output(bps)",  "Path",       "Message"};
...
void __fastcall TMonitorDialog::ShowStr(void) {
  ...
  AnsiString type[] = {"-",
                       ...
                       "UDP Client",
                       ""};
  AnsiString outformat[] = {"Lat/Lon/Height", "X/Y/Z-ECEF", "E/N/U-Baseline",
                            "NMEA-0183"};
...
void __fastcall TMonitorDialog::SetSbsLong(void) {
  AnsiString label[] = {"SAT",       "Status",    "IODE",        "dX (m)",
                        "dY (m)",    "dZ (m)",    "dVX (m/s)",   "dVY (m/s)",
                        "dVZ (m/s)", "daf0 (ns)", "daf1 (ns/s)", "T0"};
...
void __fastcall TMonitorDialog::ShowSbsLong(void) {
  ...
  Label->Caption =
      s.sprintf("IODP:%2d  System Latency:%2d s", sbssat.iodp, sbssat.tlat);
...
void __fastcall TMonitorDialog::SetSbsIono(void) {
  AnsiString label[] = {"IODI",  "Lat (deg)", "Lon (deg)",
                        "GIVEI", "Delay (m)", "T0"};
...
void __fastcall TMonitorDialog::ShowSbsIono(void) {
  ...
  for (i = 0; i < MAXBAND + 1; i++) {
    ...
    for (j = 0; j < ion->nigp; j++) {
      ...
      Tbl->Cells[k++][n + 1] =
          ion->igp[j].give ? s.sprintf("%d", ion->igp[j].give - 1) : s0;
...
void __fastcall TMonitorDialog::SetSbsFast(void) {
  AnsiString label[] = {"SAT",  "Status", "PRC (m)", "RRC (m)",
                        "IODF", "UDREI",  "AI",      "Tof"};
...
void __fastcall TMonitorDialog::ShowSbsFast(void) {
  ...
  Label->Caption = "";
  Tbl->RowCount = sbssat.nsat <= 0 ? 1 : sbssat.nsat + 1;
  // Label->Caption=s.sprintf("IODP:%2d  System Latency:%2d
  // s",sbssat.iodp,sbssat.tlat);
  for (i = 0; i < sbssat.nsat; i++) {
    j = 0;
    satp = sbssat.sat + i;
    valid = fabs(timediff(time, satp->fcorr.t0)) <= MAXSBSAGEF &&
    ...
    Tbl->Cells[j++][i + 1] =
        satp->fcorr.udre ? s.sprintf("%d", satp->fcorr.udre - 1) : s0;
...
void __fastcall TMonitorDialog::ShowRtcm(void) {
  ...
  double pos[3] = {0};
  int i = 1, j, format;
  char tstr[64] = "-", mstr1[1024] = "", mstr2[1024] = "", *p1 = mstr1,
       *p2 = mstr2;
  ...
  for (j = 1; j < 300; j++) {
    if (rtcm.nmsg3[j] == 0)
      continue;
    p2 +=
        sprintf(p2, "%s%d(%d)", p2 > mstr2 ? "," : "", j + 1000, rtcm.nmsg3[j]);
  }
  for (j = 300; j < 399; j++) {
    if (rtcm.nmsg3[j] == 0)
      continue;
    p2 +=
        sprintf(p2, "%s%d(%d)", p2 > mstr2 ? "," : "", j + 3770, rtcm.nmsg3[j]);
  }
  if (rtcm.nmsg3[0] > 0) {
...
void __fastcall TMonitorDialog::SetRtcmSsr(void) {
  AnsiString s,
      label[] = {"SAT",        "Status",     "UDI(s)",       "UDHR(s)",
                 "IOD",        "URA",        "Datum",        "T0",
                 "D0-A(m)",    "D0-C(m)",    "D0-R(m)",      "D1-A(mm/s)",
                 "D1-C(mm/s)", "D1-R(mm/s)", "C0(m)",        "C1(mm/s)",
                 "C2(mm/s2)",  "C-HR(m)",    "Code Bias(m)", "Phase Bias(m)"};
  int i, width[] = {25, 30, 30, 30, 30, 25, 15, 115, 50,  50,
                    50, 50, 50, 50, 50, 50, 50, 50,  180, 180};
  char *code;
...
void __fastcall TMonitorDialog::ShowRefSta(void) {
  ...
  Tbl->Cells[1][i++] =
      s.sprintf("%.3f,%.3f,%.3f", sta.pos[0], sta.pos[1], sta.pos[2]);
  ...
  Tbl->Cells[1][i++] =
      s.sprintf("%.3f,%.3f,%.3f", sta.del[0], sta.del[1], sta.del[2]);
  ...
  Tbl->Cells[1][i++] =
      s.sprintf("%.2f, %.2f, %.2f, %.2f", sta.glo_cp_bias[0],
                sta.glo_cp_bias[1], sta.glo_cp_bias[2], sta.glo_cp_bias[3]);
}
//---------------------------------------------------------------------------
