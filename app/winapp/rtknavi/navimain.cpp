...
int __fastcall TMainForm::ExecCmd(AnsiString cmd, int show) {
  ...
  si.cb = sizeof(si);
  char *p = cmd.c_str();
  if (!CreateProcess(NULL, p, NULL, NULL, false, show ? 0 : CREATE_NO_WINDOW,
                     NULL, NULL, &si, &info))
    return 0;
...
__fastcall TMainForm::TMainForm(TComponent *Owner) : TForm(Owner) {
  ...
  SolBuffSize = 1000;
  for (int i = 0; i < 8; i++) {
    StreamC[i] = Stream[i] = Format[i] = CmdEna[i][0] = CmdEna[i][1] =
        CmdEna[i][2] = 0;
  }
...
void __fastcall TMainForm::BtnInputStrClick(TObject *Sender) {
  ...
  if (InputStrDialog->ShowModal() != mrOk)
    return;
...
void __fastcall TMainForm::BtnOutputStrClick(TObject *Sender) {
  int otype[] = {STR_SERIAL,   STR_TCPCLI,   STR_TCPSVR,
  ...
  if (OutputStrDialog->ShowModal() != mrOk)
    return;
...
void __fastcall TMainForm::BtnLogStrClick(TObject *Sender) {
  ...
  if (LogStrDialog->ShowModal() != mrOk)
    return;
...
void __fastcall TMainForm::SvrStart(void) {
  ...
  if ((RovAntPcvF || RefAntPcvF) && !readpcv(AntPcvFileF.c_str(), &pcvr)) {
    Message->Caption =
        s.sprintf("rcv ant file read error %s", AntPcvFileF.c_str());
    Message->Hint = Message->Caption;
    return;
  }
  ...
  if (PrcOpt.sateph == EPHOPT_PREC || PrcOpt.sateph == EPHOPT_SSRCOM) {
    if (!readpcv(SatPcvFileF.c_str(), &pcvs)) {
      Message->Caption =
          s.sprintf("sat ant file read error %s", SatPcvFileF.c_str());
      Message->Hint = Message->Caption;
      return;
    }
    for (i = 0; i < MAXSAT; i++) {
  ...
  for (i = 0; i < 3; i++) {
    if (strs[i] == STR_SERIAL) {
      if (CmdEna[i][0])
        cmds[i] = Cmds[i][0].c_str();
      if (CmdEna[i][2])
        cmds_periodic[i] = Cmds[i][2].c_str();
    } else if (strs[i] == STR_TCPCLI || strs[i] == STR_TCPSVR ||
               strs[i] == STR_NTRIPCLI) {
      if (CmdEnaTcp[i][0])
        cmds[i] = CmdsTcp[i][0].c_str();
      if (CmdEnaTcp[i][2])
        cmds_periodic[i] = CmdsTcp[i][2].c_str();
    }
  ...
  if (!rtksvrstart(&rtksvr, SvrCycle, SvrBuffSize, strs, paths, Format,
                   ...
                   nmeapos, &PrcOpt, solopt, &monistr, errmsg)) {
...
void __fastcall TMainForm::SvrStop(void) {
  ...
  if (DebugStatusF > 0)
    rtkclosestat();
  if (OutputGeoidF > 0 && GeoidDataFileF != "")
    closegeoid();
}
...
void __fastcall TMainForm::TimerTimer(TObject *Sender) {
  ...
  if (update) {
    ...
    if (++TimerInact * Timer->Interval > TIMEOUT)
      SolCurrentStat = 0;
  }
  if (SolCurrentStat) {
    Svr->Color = clLime;
...
void __fastcall TMainForm::UpdateLog(int stat, gtime_t time, double *rr,
                                     float *qr, double *rb, int ns, double age,
                                     double ratio) {
  int i, ena;
...
void __fastcall TMainForm::UpdateTime(void) {
  ...
  if (TimeSys == 0)
    ...
    if (!(t = localtime(&time.time)))
      ...
      sprintf(tstr, "%04d/%02d/%02d %02d:%02d:%02d.%d", t->tm_year + 1900,
...
void __fastcall TMainForm::UpdatePos(void) {
  TLabel *label[] = {Plabel1, Plabel2, Plabel3,  Pos1,
                     Pos2,    Pos3,    LabelStd, LabelNSat};
  UTF8String sol[] = {"----", "FIX", "FLOAT", "SBAS", "DGPS", "SINGLE", "PPP"};
  UTF8String s[9], ext = "";
  TColor color[] = {clSilver, clGreen, CLORANGE, clFuchsia,
                    clBlue,   clRed,   clTeal};
  gtime_t time;
  ...
  if (SolType == 0) {
    ...
    s[3].sprintf("%.0f%s %02.0f' %07.4f\"", fabs(dms1[0]), CHARDEG, dms1[1],
                 dms1[2]);
    s[4].sprintf("%.0f%s %02.0f' %07.4f\"", fabs(dms2[0]), CHARDEG, dms2[1],
                 dms2[2]);
    s[5].sprintf("%.3f m", pos[2]);
    s[6].sprintf("N:%6.3f E:%6.3f U:%6.3f m", SQRT(Qe[4]), SQRT(Qe[0]),
                 SQRT(Qe[8]));
  } else if (SolType == 1) {
    if (norm(rr, 3) > 0.0) {
      ecef2pos(rr, pos);
    ...
    s[5].sprintf("%.3f m", pos[2]);
    s[6].sprintf("E:%6.3f N:%6.3f U:%6.3f m", SQRT(Qe[0]), SQRT(Qe[4]),
                 SQRT(Qe[8]));
  } else if (SolType == 2) {
    s[0] = "X:";
    s[1] = "Y:";
    ...
    s[6].sprintf("X:%6.3f Y:%6.3f Z:%6.3f m", SQRT(qr[0]), SQRT(qr[4]),
                 SQRT(qr[8]));
  } else if (SolType == 3) {
    if (len > 0.0) {
      ecef2pos(rb, pos);
  ...
  s[7].sprintf("Age:%4.1f s Ratio:%4.1f #Sat:%2d", Age[PSol], Ratio[PSol],
               Nvsat[PSol]);
  if (Ratio[PSol] > 0.0)
    s[8].sprintf(" R:%4.1f", Ratio[PSol]);

  for (i = 0; i < 8; i++)
    label[i]->Caption = s[i];
  for (i = 3; i < 6; i++) {
    label[i]->Font->Color =
        PrcOpt.mode == PMODE_MOVEB && SolType <= 2 ? clGray : clBlack;
  }
  IndQ->Color = IndSol->Color;
  SolS->Caption = Solution->Caption;
  SolS->Font->Color = Solution->Font->Color;
  SolQ->Caption = ext + L" " + label[0]->Caption + L" " + label[3]->Caption +
                  L" " + label[1]->Caption + L" " + label[4]->Caption + L" " +
...
void __fastcall TMainForm::UpdatePlot(void) {
  if (Panel22->Visible) {
    ...
    Disp1->Canvas->CopyRect(Panel22->ClientRect, Plot1->Canvas,
                            Panel22->ClientRect);
  }
  if (Panel23->Visible) {
    DrawPlot(Plot2, PlotType2, FreqType2);
    Disp2->Canvas->CopyRect(Panel23->ClientRect, Plot2->Canvas,
                            Panel23->ClientRect);
  }
  if (Panel24->Visible) {
    DrawPlot(Plot3, PlotType3, FreqType3);
    Disp3->Canvas->CopyRect(Panel24->ClientRect, Plot3->Canvas,
                            Panel24->ClientRect);
  }
  if (Panel25->Visible) {
    DrawPlot(Plot4, PlotType4, FreqType4);
    Disp4->Canvas->CopyRect(Panel25->ClientRect, Plot4->Canvas,
                            Panel25->ClientRect);
  }
}
// snr color ----------------------------------------------------------------
...
void __fastcall TMainForm::DrawSnr(TCanvas *c, int w, int h, int x0, int y0,
                                   int index, int freq) {
  static const TColor color[] = {(TColor)0x00008000, (TColor)0x00008080,
                                 (TColor)0x00A000A0, (TColor)0x00800000,
                                 (TColor)0x00000080, (TColor)0x00808000,
                                 (TColor)0x00808080};
  static const TColor color_sys[] = {clGreen, (TColor)0xAAFF, clFuchsia, clBlue,
                                     clRed,   clTeal,         clGray};
  UTF8String s;
  int i, j, k, l, n, x1, x2, y1, y2, y3, k1, tm, bm, hh, ww, www,
      snr[NFREQ + 1], mask[7] = {0};
  ...
  for (i = j = 0; i < 7; i++) {
    if (!mask[i])
      continue;
    sprintf(id, "%c", sys[i]);
    DrawText(c, x0 + w - tm * 3 / 2 + PanelFont->Size * 9 / 8 * (-n + j++),
             y0 + tm, (s = id), color[i], 0, 2);
  }
}
...
void __fastcall TMainForm::DrawSat(TCanvas *c, int w, int h, int x0, int y0,
  ...
  for (i = 0, k = Nsat[index] - 1; i < Nsat[index] && i < MAXSAT; i++, k--) {
    ...
    c->Brush->Color = !Vsat[index][k] ? clSilver
                                      : (freq < NFREQ + 1 ? SnrColor(snr[freq])
                                                          : color_sys[l]);
    c->Brush->Style = bsSolid;
    c->Pen->Color = clGray;
    color_text = clWhite;
  ...
  DrawText(c, x0 + 3, y0 + h, s.sprintf("#Sat:%2d/%2d", ns, Nsat[index]),
           clGray, 1, 1);
  DrawText(c, x0 + w - 3, y0 + h, s.sprintf("GDOP: %.1f", dop[0]), clGray, 2,
           1);
}
// draw baseline plot -------------------------------------------------------
void __fastcall TMainForm::DrawBL(TImage *plot, int w, int h) {
  ...
  if (pitch < 0.0) {
    c->Brush->Color = clWhite;
    c->Ellipse(pp.x - d2 / 2, pp.y - d2 / 2, pp.x + d2 / 2 + 1,
               pp.y + d2 / 2 + 1);
    DrawArrow(c, p.x + sya, p.y - cya, d3, (int)((yaw - az) * R2D), clSilver);
  }
  c->Brush->Color = col;
  c->Ellipse(pp.x - d2 / 2 + 2, pp.y - d2 / 2 + 2, pp.x + d2 / 2 - 1,
             pp.y + d2 / 2 - 1);
  for (a = 0; a < 360; a += 5) {
    q = a % 90 == 0
            ? 0
  ...
  DrawText(c, w - 3, h, s.sprintf("P: %.1f%s", pitch * R2D, CHARDEG), clGray, 2,
           1);
}
// draw track plot ----------------------------------------------------------
void __fastcall TMainForm::DrawTrk(TImage *plot) {
  TColor mcolor[] = {clSilver, clGreen, CLORANGE, clFuchsia,
                     clBlue,   clRed,   clTeal};
  ...
  if (norm(ref, 3) > 1E-6) {
    p1.x += 2;
    p1.y = p2.y + 11;
    label.sprintf("%.9f%s %.9f%s", pos[0] * R2D, CHARDEG, pos[1] * R2D,
                  CHARDEG);
...
void __fastcall TMainForm::DrawSky(TCanvas *c, int w, int h, int x0, int y0) {
  ...
  for (a = 0; a < 360; a += 45) {
    ...
    if (a % 90 == 0)
      DrawText(c, p.x + x, p.y - y, label[a / 90], clGray, 0, 0);
  }
}
// draw text ----------------------------------------------------------------
...
// draw arrow ---------------------------------------------------------------
void __fastcall TMainForm::DrawArrow(TCanvas *c, int x, int y, int siz, int ang,
                                     TColor color) {
  TPoint p1[4], p2[4];
  int i;
  ...
  for (i = 0; i < 4; i++) {
    p2[i].x =
        x + (int)(p1[i].x * cos(-ang * D2R) - p1[i].y * sin(-ang * D2R) + 0.5);
    p2[i].y =
        y - (int)(p1[i].x * sin(-ang * D2R) + p1[i].y * cos(-ang * D2R) + 0.5);
  }
  c->Brush->Style = bsSolid;
  c->Brush->Color = color;
  c->Pen->Color = color;
...
void __fastcall TMainForm::OpenMoniPort(int port) {
  ...
  for (i = 0; i <= MAXPORTOFF; i++) {
    ...
    if (stropen(&monistr, STR_TCPSVR, STR_MODE_RW, path)) {
      ...
      if (i > 0)
        Caption = s.sprintf("%s ver.%s %s (%d)", PRGNAME, VER_RTKLIB,
                            PATCH_LEVEL, i + 1);
      OpenPort = port + i;
      return;
    }
  ...
  ShowMessage(
      s.sprintf("monitor port %d-%d open error", port, port + MAXPORTOFF));
  OpenPort = 0;
}
...
void __fastcall TMainForm::SaveLog(void) {
  ...
  sprintf(file, "rtk_%04.0f%02.0f%02.0f%02.0f%02.0f%02.0f.txt", ep[0], ep[1],
          ep[2], ep[3], ep[4], ep[5]);
...
void __fastcall TMainForm::LoadNav(nav_t *nav) {
  ...
  for (i = 0; i < MAXSAT * 2; i++) {
    if ((str = ini->ReadString("navi", s.sprintf("eph_%02d", i), "")) == "")
      continue;
    nav->eph[i] = eph0;
    ...
    *p = '\0';
    ...
    sscanf(p + 1,
           ...
           "lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%d,%d,%lf",
           &nav->eph[i].iode, &nav->eph[i].iodc, &nav->eph[i].sva,
           &nav->eph[i].svh, &toe_time, &toc_time, &ttr_time, &nav->eph[i].A,
  ...
  for (i = 0; i < 8; i++)
    nav->ion_gps[i] = 0.0;
  sscanf(str.c_str(), "%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf", nav->ion_gps,
         nav->ion_gps + 1, nav->ion_gps + 2, nav->ion_gps + 3, nav->ion_gps + 4,
         nav->ion_gps + 5, nav->ion_gps + 6, nav->ion_gps + 7);

  str = ini->ReadString("navi", "utc", "");
  for (i = 0; i < 8; i++)
    nav->utc_gps[i] = 0.0;
  sscanf(str.c_str(), "%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf", nav->utc_gps,
         nav->utc_gps + 1, nav->utc_gps + 2, nav->utc_gps + 3, nav->utc_gps + 4,
         nav->utc_gps + 5, nav->utc_gps + 6, nav->utc_gps + 7);
...
void __fastcall TMainForm::SaveNav(nav_t *nav) {
  ...
  str = "";
  for (i = 0; i < 8; i++)
    str = str + s.sprintf("%.14E,", nav->ion_gps[i]);
  ini->WriteString("navi", "ion", str);

  str = "";
  for (i = 0; i < 8; i++)
    str = str + s.sprintf("%.14E,", nav->utc_gps[i]);
...
void __fastcall TMainForm::LoadOpt(void) {
  ...
  for (i = 0; i < 8; i++) {
    ...
    for (j = 0; j < 4; j++) {
      Paths[i][j] =
          ini->ReadString("stream", s.sprintf("path_%d_%d", no, j), "");
    }
  }
  for (i = 0; i < 3; i++) {
  ...
  for (i = 0; i < 3; i++)
    for (j = 0; j < 3; j++) {
      ...
      CmdEna[i][j] =
          ini->ReadInteger("serial", s.sprintf("cmdena_%d_%d", i, j), 0);
  ...
  for (i = 0; i < 3; i++)
    for (j = 0; j < 3; j++) {
      CmdsTcp[i][j] =
          ini->ReadString("tcpip", s.sprintf("cmd_%d_%d", i, j), "");
      CmdEnaTcp[i][j] =
          ini->ReadInteger("tcpip", s.sprintf("cmdena_%d_%d", i, j), 0);
  ...
  for (i = 0; i < NFREQ; i++)
    for (j = 0; j < 9; j++) {
      PrcOpt.snrmask.mask[i][j] = ini->ReadFloat(
          "prcopt", s.sprintf("snrmask_%d_%d", i + 1, j + 1), 0.0);
    }
  ...
  for (i = 0; i < NMapPnt; i++) {
    ...
    AnsiString pos =
        ini->ReadString("mapopt", s.sprintf("pntpos%d", i + 1), "0,0,0");
  ...
  PanelFont->Color =
      (TColor)ini->ReadInteger("setting", "panelfontcolor", (int)clBlack);
  PosFont->Name = ini->ReadString("setting", "posfontname", POSFONTNAME);
  PosFont->Size = ini->ReadInteger("setting", "posfontsize", POSFONTSIZE);
  PosFont->Color =
      (TColor)ini->ReadInteger("setting", "posfontcolor", (int)clBlack);
  if (ini->ReadInteger("setting", "posfontbold", 0))
    PosFont->Style = PosFont->Style << fsBold;
  if (ini->ReadInteger("setting", "posfontitalic", 0))
    PosFont->Style = PosFont->Style << fsItalic;
  PanelFont->Charset = ANSI_CHARSET;
  PosFont->Charset = ANSI_CHARSET;

  TTextViewer::Color1 =
      (TColor)ini->ReadInteger("viewer", "color1", (int)clBlack);
  TTextViewer::Color2 =
      (TColor)ini->ReadInteger("viewer", "color2", (int)clWhite);
  TTextViewer::FontD = new TFont;
  TTextViewer::FontD->Name =
      ini->ReadString("viewer", "fontname", "Courier New");
...
void __fastcall TMainForm::SaveOpt(void) {
  ...
  for (i = 0; i < 3; i++)
    for (j = 0; j < 3; j++) {
      ...
      ini->WriteInteger("serial", s.sprintf("cmdena_%d_%d", i, j),
                        CmdEna[i][j]);
    }
  for (i = 0; i < 3; i++)
    for (j = 0; j < 3; j++) {
      ...
      ini->WriteInteger("tcpip", s.sprintf("cmdena_%d_%d", i, j),
                        CmdEnaTcp[i][j]);
    }
  ...
  for (i = 0; i < NMapPnt; i++) {
    ...
    ini->WriteString(
        "mapopt", s1.sprintf("pntpos%d", i + 1),
  ...
  ini->WriteInteger("setting", "posfontitalic",
                    PosFont->Style.Contains(fsItalic));
