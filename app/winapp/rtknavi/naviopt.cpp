...
__fastcall TOptDialog::TOptDialog(TComponent *Owner) : TForm(Owner) {
  ...
  if (nglo <= 0)
    NavSys2->Enabled = false;
  if (ngal <= 0)
    NavSys3->Enabled = false;
  if (nqzs <= 0)
    NavSys4->Enabled = false;
  if (ncmp <= 0)
    NavSys6->Enabled = false;
  if (nirn <= 0)
    NavSys7->Enabled = false;
  UpdateEnable();
}
//---------------------------------------------------------------------------
...
void __fastcall TOptDialog::BtnSnrMaskClick(TObject *Sender) {
  ...
  if (MaskOptDialog->ShowModal() != mrOk)
    return;
...
void __fastcall TOptDialog::BtnLocalDirClick(TObject *Sender) {
  ...
  if (!SelectDirectory(L"FTP/HTTP Local Directory", L"", dir, opt))
    return;
  LocalDir->Text = dir;
}
//---------------------------------------------------------------------------
void __fastcall TOptDialog::BtnFont1Click(TObject *Sender) {
  ...
  FontLabel1->Caption =
      FontLabel1->Font->Name + s.sprintf(" %dpt", FontLabel1->Font->Size);
}
//---------------------------------------------------------------------------
void __fastcall TOptDialog::BtnFont2Click(TObject *Sender) {
  ...
  FontLabel2->Caption =
      FontLabel2->Font->Name + s.sprintf(" %dpt", FontLabel2->Font->Size);
}
//---------------------------------------------------------------------------
...
void __fastcall TOptDialog::LoadOpt(AnsiString file) {
  ...
  resetsysopts();
  if (!loadopts(file.c_str(), sysopts) || !loadopts(file.c_str(), rcvopts))
    return;
...
void __fastcall TOptDialog::SaveOpt(AnsiString file) {
  ...
  prcopt.navsys =
      (NavSys1->Checked ? SYS_GPS : 0) | (NavSys2->Checked ? SYS_GLO : 0) |
  ...
  if (RovAntPcv->Checked)
    strcpy(prcopt.anttype[0], RovAnt_Text.c_str());
  if (RefAntPcv->Checked)
    strcpy(prcopt.anttype[1], RefAnt_Text.c_str());
  ...
  if (!saveopts(file.c_str(), "w", comment, sysopts) ||
      !saveopts(file.c_str(), "a", "", rcvopts))
    return;
}
//---------------------------------------------------------------------------
void __fastcall TOptDialog::UpdateEnable(void) {
  int rel =
      PMODE_DGPS <= PosMode->ItemIndex && PosMode->ItemIndex <= PMODE_FIXED;
  int rtk =
      PMODE_KINEMA <= PosMode->ItemIndex && PosMode->ItemIndex <= PMODE_FIXED;
  ...
  BaselineLen->Enabled =
      BaselineConst->Checked && PosMode->ItemIndex == PMODE_MOVEB;
  BaselineSig->Enabled =
      BaselineConst->Checked && PosMode->ItemIndex == PMODE_MOVEB;
  ...
  OutputGeoid->Enabled =
      SolFormat->ItemIndex == 0 && OutputHeight->ItemIndex == 1;
  ...
  LabelRovAntD->Enabled =
      (rel || ppp) && RovAntPcv->Checked && RovAnt->Text != "*";
  ...
  RovPosTypeP->Enabled = PosMode->ItemIndex == PMODE_FIXED ||
                         PosMode->ItemIndex == PMODE_PPP_FIXED;
...
void __fastcall TOptDialog::SetPos(int type, TEdit **edit, double *pos) {
  ...
  if (type == 1) { /* lat/lon/height dms/m */
    ...
    edit[0]->Text =
        s.sprintf("%.0f %02.0f %09.6f", s1 * dms1[0], dms1[1], dms1[2]);
    edit[1]->Text =
        s.sprintf("%.0f %02.0f %09.6f", s2 * dms2[0], dms2[1], dms2[2]);
