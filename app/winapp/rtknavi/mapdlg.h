...
private:
  ...
  void __fastcall DrawVertGraph(const double *sol, const int *stat, int psol,
                                int psols, int psole, int nsol,
                                int currentstat);
  TPoint __fastcall PosToPoint(const double *pos);
  TPoint __fastcall PosToGraphP(const double *pos, const double *ref, int index,
                                int npos, TRect rect);
  void __fastcall DrawPoint(const double *pos, AnsiString name, TColor color);
  void __fastcall DrawVel(const double *vel);
  void __fastcall DrawScale(void);
  void __fastcall DrawCircle(TPoint p, int r, TColor color1, TColor color2);
  void __fastcall DrawGrid(TPoint p, int gint, int ng, TColor color1,
                           TColor color2);
  void __fastcall DrawText(int x, int y, AnsiString s, TColor color, int align);
...
public:
  ...
  void __fastcall UpdateMap(const double *sol, const double *solref,
                            ...
                            int psols, int psole, int nsol, AnsiString *solstr,
                            int currentstat);
