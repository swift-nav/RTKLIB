...
void __fastcall TOptDialog::LoadOpt(AnsiString file) {
  ...
  RefPosType->ItemIndex = prcopt.refpos == 0 ? 0
                          : prcopt.refpos == POSOPT_RINEX_DYN
                              ? 6
                              : prcopt.refpos + 2;
  RovPosTypeP = RovPosType->ItemIndex;
  RefPosTypeP = RefPosType->ItemIndex;
...
void __fastcall TOptDialog::SaveOpt(AnsiString file) {
  ...
  prcopt.refpos = RefPosType->ItemIndex < 3    ? 0
                  : RefPosType->ItemIndex == 6 ? POSOPT_RINEX_DYN
                                               : RefPosType->ItemIndex - 2;
  if (prcopt.rovpos == 0)
...
void __fastcall TOptDialog::UpdateEnable(void) {
  ...
  RovPosType->Enabled = PosMode->ItemIndex == PMODE_FIXED ||
