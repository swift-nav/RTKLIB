...
//---------------------------------------------------------------------------
__fastcall TConvDialog::TConvDialog(TComponent *Owner) : TForm(Owner) {
  UpdateEnable();
}
//---------------------------------------------------------------------------
...
void __fastcall TConvDialog::BtnGoogleClick(TObject *Sender) {
  ...
  char cmd[1024];
  sprintf(cmd, "\"%s\" \"%s\"", MainForm->GoogleEarthFile.c_str(),
          OutputFile_Text.c_str());
  if (!ExecCmd(cmd))
    ShowMsg("error : google earth execution");
}
//---------------------------------------------------------------------------
void __fastcall TConvDialog::BtnConvertClick(TObject *Sender) {
  ...
  double offset[3] = {0}, es[6] = {1970, 1, 1}, ee[6] = {2038, 1, 1},
         tint = 0.0;
  gtime_t ts = {0}, te = {0};
...
void __fastcall TConvDialog::UpdateOutFile(void) {
  ...
  if (!(p = strrchr(file, '.')))
    p = file + strlen(file);
  strcpy(p,
         FormatGPX->Checked ? ".gpx" : (Compress->Checked ? ".kmz" : ".kml"));
