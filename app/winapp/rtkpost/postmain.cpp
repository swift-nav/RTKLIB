...
extern void settime(gtime_t time) {
  ...
  if (tend_.time != 0 && tstart_.time != 0 &&
    ...
    MainForm->Progress->Position =
        (int)(timediff(time, tstart_) / tt * 100.0 + 0.5);
  }
  if (i++ % 23 == 0)
    Application->ProcessMessages();
}
...
void __fastcall TMainForm::DropFiles(TWMDropFiles msg) {
  ...
  if (!DragQueryPoint((HDROP)msg.Drop, &point))
    return;
...
void __fastcall TMainForm::BtnExecClick(TObject *Sender) {
  ...
  if (InputFile2->Text == "" && PMODE_DGPS <= PosMode &&
      PosMode <= PMODE_FIXED) {
  ...
  if ((p = strrchr(OutputFile_Text.c_str(), '.'))) {
    if (!strcmp(p, ".obs") || !strcmp(p, ".OBS") || !strcmp(p, ".nav") ||
        ...
        !strcmp(p, ".gz") || !strcmp(p, ".Z") || !strcmp(p + 3, "o") ||
        !strcmp(p + 3, "O") || !strcmp(p + 3, "d") || !strcmp(p + 3, "D") ||
        !strcmp(p + 3, "n") || !strcmp(p + 3, "N") || !strcmp(p + 3, "g") ||
        !strcmp(p + 3, "G")) {
...
void __fastcall TMainForm::BtnInputPlot1Click(TObject *Sender) {
  ...
  if (files[2] == "") {
    if (ObsToNav(files[0].c_str(), navfile))
      files[2] = navfile;
  }
...
void __fastcall TMainForm::BtnOutDirClick(TObject *Sender) {
  ...
  if (!SelectDirectory(L"Output Directory", L"", dir, opt))
    return;
  OutDir->Text = dir;
#endif
}
...
void __fastcall TMainForm::TimeH1UDChangingEx(TObject *Sender,
  ...
  if (sec < 0)
    ...
    sec -= 86400;
  TimeH1->Text =
      s.sprintf("%02d:%02d:%02d", sec / 3600, (sec % 3600) / 60, sec % 60);
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::TimeY1KeyDown(TObject *Sender, WORD &Key,
                                         TShiftState Shift)
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::TimeH1KeyDown(TObject *Sender, WORD &Key,
                                         TShiftState Shift)
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::TimeY2KeyDown(TObject *Sender, WORD &Key,
                                         TShiftState Shift)
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::TimeH2KeyDown(TObject *Sender, WORD &Key,
                                         TShiftState Shift)
...
int __fastcall TMainForm::ExecProc(void) {
  AnsiString InputFile1_Text = InputFile1->Text,
             InputFile2_Text = InputFile2->Text;
  AnsiString InputFile3_Text = InputFile3->Text,
             InputFile4_Text = InputFile4->Text;
  AnsiString InputFile5_Text = InputFile5->Text,
             InputFile6_Text = InputFile6->Text;
  AnsiString OutputFile_Text = OutputFile->Text;
  FILE *fp;
  ...
  if (TimeEnd->Checked)
    te = GetTime2();
  if (TimeIntF->Checked)
    ti = str2dbl(TimeInt->Text);
  if (TimeUnitF->Checked)
    tu = str2dbl(TimeUnit->Text) * 3600.0;

  if (!GetOption(prcopt, solopt, filopt))
    return 0;
...
int __fastcall TMainForm::GetOption(prcopt_t &prcopt, solopt_t &solopt,
  ...
  if (PosMode == PMODE_SINGLE || PosMode == PMODE_MOVEB) {
    ...
    prcopt.refpos = RefPosType == 6 ? 6 : RefPosType - 2;
...
void __fastcall TMainForm::WriteList(TIniFile *ini, AnsiString cat,
  ...
  for (i = 0; i < list->Count; i++) {
    ini->WriteString(cat, s.sprintf("%s_%03d", key.c_str(), i),
                     list->Strings[i]);
  }
}
// add history --------------------------------------------------------------
void __fastcall TMainForm::AddHist(TComboBox *combo) {
  ...
  if (i >= 0)
    list->Delete(i);
  list->Insert(0, hist);
  for (int i = list->Count - 1; i >= MAXHIST; i--)
    list->Delete(i);
  combo->ItemIndex = 0;
}
// execute command ----------------------------------------------------------
...
void __fastcall TMainForm::SaveOpt(void) {
  ...
  for (int i = 0; i < 10; i++) {
    ini->WriteString("opt", s.sprintf("rovlist%d", i + 1),
                     RovList.SubString(i * 2000, 2000));
  }
  for (p = BaseList.c_str(); *p; p++) {
    if ((p = strstr(p, "\r\n")))
      strncpy(p, "@@", 2);
  ...
  for (int i = 0; i < 10; i++) {
    ini->WriteString("opt", s.sprintf("baselist%d", i + 1),
                     BaseList.SubString(i * 2000, 2000));
  }
...
void __fastcall TMainForm::Panel4Resize(TObject *Sender) {
  ...
  TComboBox *boxes[] = {InputFile1, InputFile2, InputFile3,
                        InputFile4, InputFile5, InputFile6};
  int w = Panel4->Width;
  ...
  BtnInputView1->Left =
      InputFile1->Left + InputFile1->Width - BtnInputView1->Width;
  BtnInputPlot1->Left = BtnInputView1->Left - BtnInputPlot1->Width;
  BtnInputView2->Left =
      InputFile2->Left + InputFile2->Width - BtnInputView2->Width;
  BtnInputPlot2->Left = BtnInputView2->Left - BtnInputPlot2->Width;
  BtnInputView6->Left =
      InputFile3->Left + InputFile3->Width - BtnInputView6->Width;
