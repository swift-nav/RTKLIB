...
static const char *help[] = {
    ...
    "",
    " Read RINEX OBS/NAV/GNAV/HNAV/CLK, SP3, SBAS message log files and "
    "compute ",
    ...
    " file shall be included in input files. To use SP3 precise ephemeris, "
    "specify",
    " the path in the files. The extension of the SP3 file shall be .sp3 or "
    ".eph.",
    ...
    " -sys s[,s...] nav system(s) (s=G:GPS,R:GLO,E:GAL,J:QZS,C:BDS,I:IRN) "
    "[G|R]",
    " -f freq   number of frequencies for relative mode "
    "(1:L1,2:L1+L2,3:L1+L2+L5) [2]",
    ...
    " -g        output latitude/longitude in the form of ddd mm ss.ss' "
    "[ddd.ddd]",
    ...
    " -rb mode  reference (base) receiver position mode (1:average of single "
    "pos,",
    "           2:read from file,3:read from RINEX header,6:read from RINEX "
    "header",
    "           and subsequent site occupancy events) [1]",
    " -l lat lon hgt reference (base) receiver latitude/longitude/height "
    "(deg/m)",
...
int main(int argc, char **argv) {
  ...
  ret = postpos(ts, te, tint, 0.0, &prcopt, &solopt, &filopt, infile, n,
                outfile, "", "");
