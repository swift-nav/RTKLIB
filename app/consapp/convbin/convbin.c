...
static const char *help[] = {
    "",
    " Converts Swift Navigation receiver SBP binary and SBP JSON raw data logs "
    "to RINEX files.",
    "",
    " Usage:",
    "",
    ...
    "                  BASEPOS   - determine base station location from input "
    "file",
    ...
    "     -hr rec      rinex header: receiver number, type and version "
    "separated by /",
    ...
    "     -mask   [sig[,...]] signal mask(s) "
    "(sig={G|R|E|J|S|C|I}L{1C|1P|1W|...})",
    "     -nomask [sig[,...]] signal no mask (same as above)",
    "     -x sat       exclude satellite",
    "     -y sys       exclude systems "
    "(G:GPS,R:GLO,E:GAL,J:QZS,S:SBS,C:BDS,I:IRN)",
    ...
    "",
    " If log format type is not specified, format type is recognized by the "
    "input",
...
static int convbin(int format, rnxopt_t *opt, const char *ifile, char **file,
  ...
  def = !file[0] && !file[1] && !file[2] && !file[3] && !file[4] && !file[5] &&
  ...
  if (*ofile[3])
    fprintf(stderr, "->rinex hnav: %s\n", ofile[3]);
  if (*ofile[4])
    fprintf(stderr, "->rinex qnav: %s\n", ofile[4]);
  if (*ofile[5])
    fprintf(stderr, "->rinex lnav: %s\n", ofile[5]);
  if (*ofile[6])
    fprintf(stderr, "->rinex cnav: %s\n", ofile[6]);
  if (*ofile[7])
    fprintf(stderr, "->rinex inav: %s\n", ofile[7]);
  if (*ofile[8])
    fprintf(stderr, "->sbas log  : %s\n", ofile[8]);
...
static int cmdopts(int argc, char **argv, rnxopt_t *opt, char **ifile,
  ...
  for (i = 1; i < argc; i++) {
    if (!strcmp(argv[i], "-ts") && i + 2 < argc) {
      ...
      strcpy(buff, argv[++i]);
      for (j = 0, p = strtok(buff, "/"); j < 2 && p;
           j++, p = strtok(NULL, "/")) {
        strcpy(opt->name[j], p);
      }
    } else if (!strcmp(argv[i], "-hr") && i + 1 < argc) {
      strcpy(buff, argv[++i]);
      for (j = 0, p = strtok(buff, "/"); j < 3 && p;
           j++, p = strtok(NULL, "/")) {
        strcpy(opt->rec[j], p);
      }
  ...
  if (nf >= 1)
    opt->freqtype |= FREQTYPE_L1;
  if (nf >= 2)
    opt->freqtype |= FREQTYPE_L2;
  if (nf >= 3)
    opt->freqtype |= FREQTYPE_L3;
  if (nf >= 4)
    opt->freqtype |= FREQTYPE_L4;
  if (nf >= 5)
    opt->freqtype |= FREQTYPE_L5;
  ...
  if (*fmt) {
    ...
    paths[0] = path;
    if (!expath(*ifile, paths, 1) || !(p = strrchr(path, '.')))
      return -1;
    if (!strcmp(p, ".rtcm2"))
      format = STRFMT_RTCM2;
...
int main(int argc, char **argv) {
  ...
  sprintf(opt.prog, "%s %s (RTKLIB %s%s)", PRGNAME, SWIFT_VER_RTKLIB,
