...
static const char *help[] = {
    ...
    "    ntrip caster : ntripc_c://[user:passwd@][:port]/mntpnt[:srctbl] (only "
    "out)",
...
int main(int argc, char **argv) {
  ...
  char *cmdfile[MAXSTR] = {"", "", "", "", "", "", "", "", ""}, *cmds[MAXSTR],
       *cmds_periodic[MAXSTR];
  char *local = "", *proxy = "", *msg = "1004,1019", *opt = "", buff[256], *p;
  char strmsg[MAXSTRMSG] = "", *antinfo = "", *rcvinfo = "";
  char *ant[] = {"", "", ""}, *rcv[] = {"", "", ""}, *logfile = "";
  int i, j, n = 0, dispint = 5000, trlevel = 0,
            opts[] = {10000, 10000, 2000, 32768, 10, 0, 30, 0};
  int types[MAXSTR] = {STR_FILE, STR_FILE}, stat[MAXSTR] = {0},
      log_stat[MAXSTR] = {0};
  ...
  for (i = 0; i < n; i++) {
    ...
    if (!(conv[i] =
              strconvnew(fmts[0], fmts[i + 1], msg, sta, sta != 0, opt))) {
