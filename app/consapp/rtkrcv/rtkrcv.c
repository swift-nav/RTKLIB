 ...
 *virtual console to vt.c 2015/05/22 1.12 fix bug on sp3 id in inpstr*-format
 *options 2015/07/31 1.13 accept 4:stat for outstr1-format or outstr2-format add
 *reading satellite dcb 2015/12/14 1.14 add option -sta for station name (#339)
...
static double nmeapos[] = {0, 0,
                           0}; /* nmea position (lat/lon/height) (deg,m) */
...
static const char *usage[] = {
    "usage: rtkrcv [-s][-p port][-d dev][-o file][-w pwd][-r level][-t "
    "level][-sta sta]",
    "options",
...
static int startsvr(vt_t *vt) {
  ...
  if (!rtksvrstart(&svr, svrcycle, buffsize, strtype, paths, strfmt, navmsgsel,
                   cmds, cmds_periodic, ropts, nmeacycle, nmeareq, npos,
                   &prcopt, solopt, &moni, errmsg)) {
...
static void prstream(vt_t *vt) {
  ...
  const char *type[] = {"-",      "serial", "file",     "tcpsvr",
                        "tcpcli", "udp",    "ntrips",   "ntripc",
                        "ftp",    "http",   "ntripc_s", "ntripc_c"};
  const char *fmt[] = {"rtcm2", "rtcm3", "oem4",    "oem3", "ubx",
                       "ss2",   "hemis", "skytreq", "gw10", "javad",
                       "nvs",   "binex", "rt17",    "sbf",  "cmr",
                       "",      "",      "sp3",     ""};
  ...
  vt_printf(vt, "\n%s%-12s %-8s %-5s %s %10s %7s %10s %7s %-24s %s%s\n",
            ESC_BOLD, "Stream", "Type", "Fmt", "S", "In-byte", "In-bps",
            "Out-byte", "Out-bps", "Path", "Message", ESC_RESET);
  for (i = 0; i < 9; i++) {
...
static void prssr(vt_t *vt) {
  ...
  p += sprintf(p,
               ...
               "%6s %6s %6s%s\n",
               ESC_BOLD, "SAT", "S", "UDI", "IOD", "URA", "T0", "D0-A", "D0-C",
               "D0-R", "D1-A", "D1-C", "D1-R", "C0", "C1", "C2", "C-HR",
               ESC_RESET);
  for (i = 0; i < MAXSAT; i++) {
    if (!ssr[i].t0[0].time)
      continue;
    ...
    p += sprintf(p,
                 ...
                 id, valid ? "OK" : "-", ssr[i].udi[0], ssr[i].iode, ssr[i].ura,
                 tstr, ssr[i].deph[0], ssr[i].deph[1], ssr[i].deph[2],
                 ssr[i].ddeph[0] * 1E3, ssr[i].ddeph[1] * 1E3,
                 ssr[i].ddeph[2] * 1E3, ssr[i].dclk[0], ssr[i].dclk[1] * 1E3,
...
static void cmd_save(char **args, int narg, vt_t *vt) {
  ...
  sprintf(comment, "%s options (%s, v.%s %s)", PRGNAME, s, VER_RTKLIB,
          PATCH_LEVEL);
...
static void *con_thread(void *arg) {
  const char *cmds[] = {
      "start",  "stop",     "restart", "solution", "status", "satellite",
 ...
 *     multiple telnet console logins are allowed. The initial processing
 *options are loaded from default file rtkrcv.conf. To change the file, use -o
