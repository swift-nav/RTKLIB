...
static const char *help[] = {
    ...
    "",
    " Read solution file(s) and convert it to Google Earth KML file or GPX "
    "file.",
    " Each line in the input file shall contain fields of time, position "
    "fields ",
    " (latitude/longitude/height or x/y/z-ecef), and quality flag(option). The "
    "line",
    " started with '%', '#', ';' is treated as comment. Command options are "
    "as ",
    " follows. ([]:default)",
    "",
    " -h        print help",
    " -o file   output file [infile + .kml]",
    " -c color  track color (0:off,1:white,2:green,3:orange,4:red,5:yellow) "
    "[5]",
    " -p color  point color (0:off,1:white,2:green,3:orange,4:red,5:by qflag) "
    "[5]",
...
int main(int argc, char **argv) {
  ...
  double offset[3] = {0.0}, tint = 0.0, es[6] = {2000, 1, 1},
         ee[6] = {2000, 1, 1};
  gtime_t ts = {0}, te = {0};
  ...
  for (i = 0; i < n; i++) {
    if (gpx) {
      stat = convgpx(infile[i], outfile, ts, te, tint, qflg, offset, tcolor,
                     pcolor, outalt, outtime);
    } else {
      stat = convkml(infile[i], outfile, ts, te, tint, qflg, offset, tcolor,
                     pcolor, outalt, outtime);
    }
    switch (stat) {
    case -1:
