...
void TextViewer::BtnReadClick() {
  if (BtnRead->text() == tr("Save...")) {
    Save(QDir::toNativeSeparators(
        QFileDialog::getSaveFileName(this, QString(), File)));
  } else {
    Read(QDir::toNativeSeparators(
        QFileDialog::getOpenFileName(this, QString(), File)));
  }
}
//---------------------------------------------------------------------------
void TextViewer::BtnOptClick() {
  ...
  viewerOptDialog->move(
      this->size().width() / 2 - viewerOptDialog->size().width() / 2,
