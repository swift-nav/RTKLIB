...
void FileOptDialog::showEvent(QShowEvent *event) {
  ...
  if (!Opt) {
    ...
    foreach (token, tokens) {
      if (token == "T")
        ChkTimeTag->setChecked(true);
      if (token.contains("+"))
        start = token.toDouble();
      if (token.contains("x"))
        speed = token.mid(1).toDouble();
    }
...
void FileOptDialog::BtnOkClick() {
  ...
  if (!Opt) {
    ...
    if (ChkTimeTag->isChecked()) {
      Path = Path + "::T" + "::" + TimeSpeed->currentText() + "::+" +
             TimeStart->text();
    }
  } else {
    Path = FilePath->text();
...
void FileOptDialog::BtnFilePathClick() {
  if (!Opt) {
    FilePath->setText(QDir::toNativeSeparators(
        QFileDialog::getOpenFileName(this, QString(), FilePath->text())));
  } else {
    FilePath->setText(QDir::toNativeSeparators(
        QFileDialog::getSaveFileName(this, QString(), FilePath->text())));
  }
}
//---------------------------------------------------------------------------
