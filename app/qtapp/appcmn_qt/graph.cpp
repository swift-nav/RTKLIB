...
int Graph::ToPoint(double x, double y, QPoint &p) {
  ...
  p.setX((int)floor(x + 0.5));
  p.setY((int)floor(y + 0.5));
  return (X - xt < x) && (x < X + Width - 1 + xt) && (Y - xt < y) &&
         (y < Y + Height - 1 + xt);
}
//---------------------------------------------------------------------------
void Graph::resize() {
...
void Graph::GetTick(double &xt, double &yt) {
  xt = XTick > 0.0 ? XTick
                   : (XLPos == 5 || XLPos == 6 ? AutoTickTime(XScale)
                                               : AutoTick(XScale));
...
double Graph::AutoTickTime(double scale) {
  double t[] = {0.1,          0.2,          0.5,         1.0,
                3.0,          6.0,          12.0,        30.0,
                60.0,         300.0,        900.0,       1800.0,
                3600.0,       7200.0,       10800.0,     21600.0,
                43200.0,      86400.0,      86400.0 * 2, 86400.0 * 7,
                86400.0 * 14, 86400.0 * 35, 86400.0 * 70};
...
void Graph::DrawGridLabel(QPainter &c, double xt, double yt) {
  ...
  if (XLPos) {
    for (int i = (int)ceil(xl[0] / xt); i * xt <= xl[1]; i++) {
      if (XLPos <= 4) {
        ...
        if (XLPos == 1)
          p.setY(p.y() - 1);
        int ha = XLPos <= 2 ? 0 : (XLPos == 3 ? 2 : 1),
            va = XLPos >= 3 ? 0 : (XLPos == 1 ? 2 : 1);
        DrawText(c, p, NumText(i * xt, xt), Color[2], ha, va,
                 XLPos >= 3 ? 90 : 0);
      } else if (XLPos == 6) {
        ToPoint(i * xt, yl[0], p);
  ...
  if (YLPos) {
    for (int i = (int)ceil(yl[0] / yt); i * yt <= yl[1]; i++) {
      if (YLPos <= 4) {
        ToPoint(xl[0], i * yt, p);
        int ha = YLPos >= 3 ? 0 : (YLPos == 1 ? 2 : 1),
            va = YLPos <= 2 ? 0 : (YLPos == 3 ? 1 : 2);
        DrawText(c, p, NumText(i * yt, yt), Color[2], ha, va,
                 YLPos >= 3 ? 90 : 0);
      } else if (YLPos == 7) {
        if (i == 0)
          continue;
...
void Graph::RotPoint(QPoint *ps, int n, const QPoint &pc, int rot, QPoint *pr) {
  for (int i = 0; i < n; i++) {
    pr[i].setX(pc.x() + (int)floor(ps[i].x() * cos(rot * D2R) -
                                   ps[i].y() * sin(rot * D2R) + 0.5));
    pr[i].setY(pc.y() - (int)floor(ps[i].x() * sin(rot * D2R) +
                                   ps[i].y() * cos(rot * D2R) + 0.5));
  }
}
//---------------------------------------------------------------------------
void Graph::DrawMark(QPainter &c, const QPoint &p, int mark,
...
//---------------------------------------------------------------------------
void Graph::DrawMark(QPainter &c, double x, double y, int mark,
                     const QColor &color, int size, int rot) {
  QPoint p;
  if (ToPoint(x, y, p))
...
//---------------------------------------------------------------------------
void Graph::DrawMark(QPainter &c, const QPoint &p, int mark,
                     const QColor &color, const QColor &bgcolor, int size,
                     int rot) {
  QPoint p1;
...
void Graph::DrawMark(QPainter &c, double x, double y, int mark,
  ...
  QPoint p;
  if (ToPoint(x, y, p))
    DrawMark(c, p, mark, color, bgcolor, size, rot);
}
//---------------------------------------------------------------------------
void Graph::DrawMarks(QPainter &c, const double *x, const double *y,
                      const QVector<QColor> &colors, int n, int mark, int size,
                      int rot) {
  QPoint p, pp;
  for (int i = 0; i < n; i++) {
    if (!ToPoint(x[i], y[i], p) || (pp == p))
      continue;
...
//---------------------------------------------------------------------------
void Graph::DrawText(QPainter &c, const QPoint &p, const QString &str,
                     const QColor &color, int ha, int va, int rot) {
  // str = UTF-8 string
...
//---------------------------------------------------------------------------
void Graph::DrawText(QPainter &c, double x, double y, const QString &str,
                     const QColor &color, int ha, int va, int rot) {
  QPoint p;
...
//---------------------------------------------------------------------------
void Graph::DrawText(QPainter &c, const QPoint &p, const QString &str,
                     const QColor &color, const QColor &bgcolor, int ha, int va,
                     int rot) {
  QPoint p1;
...
//---------------------------------------------------------------------------
void Graph::DrawCircle(QPainter &c, const QPoint &p, const QColor &color,
                       int rx, int ry, int style) {
  Qt::PenStyle ps[] = {Qt::SolidLine, Qt::DotLine, Qt::DashLine,
...
//---------------------------------------------------------------------------
void Graph::DrawCircle(QPainter &c, double x, double y, const QColor &color,
                       double rx, double ry, int style) {
  QPoint p;

  ToPoint(x, y, p);
  DrawCircle(c, p, color, (int)(rx / XScale + 0.5), (int)(ry / YScale + 0.5),
             style);
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void Graph::DrawPoly(QPainter &c, QPoint *p, int n, const QColor &color,
                     int style) {
  Qt::PenStyle ps[] = {Qt::SolidLine, Qt::DotLine, Qt::DashLine,
                       Qt::DashDotLine, Qt::DashDotDotLine};
  QPen pen = c.pen();
  ...
  for (i = j = 0; j < n; j++, area0 = area1) {
    ...
    if (area0 && !ClipPoint(pc, area0, p + j))
      continue;
    if (area1 && !ClipPoint(pc + 1, area1, p + j - 1))
      continue;
...
//---------------------------------------------------------------------------
void Graph::DrawPoly(QPainter &c, double *x, double *y, int n,
...
//---------------------------------------------------------------------------
void Graph::DrawPatch(QPainter &c, QPoint *p, int n, const QColor &color1,
...
//---------------------------------------------------------------------------
void Graph::DrawPatch(QPainter &c, double *x, double *y, int n,
                      const QColor &color1, const QColor &color2, int style) {
  QPoint *p = new QPoint[n];
...
//---------------------------------------------------------------------------
void Graph::DrawSkyPlot(QPainter &c, const QPoint &p, const QColor &color1,
                        const QColor &color2, int size) {
  QPen pen = c.pen();
  pen.setColor(color1);
  c.setPen(pen);
  c.setBrush(Qt::NoBrush);
...
//---------------------------------------------------------------------------
void Graph::DrawSkyPlot(QPainter &c, double x, double y, const QColor &color1,
                        const QColor &color2, double size) {
  QPoint p;
...
//---------------------------------------------------------------------------
void Graph::DrawSkyPlot(QPainter &c, const QPoint &p, const QColor &color1,
                        const QColor &color2, const QColor &bgcolor, int size) {
  QPen pen = c.pen();
  pen.setColor(color1);
  c.setPen(pen);
  c.setBrush(Qt::NoBrush);
...
//---------------------------------------------------------------------------
void Graph::DrawSkyPlot(QPainter &c, double x, double y, const QColor &color1,
                        const QColor &color2, const QColor &bgcolor,
                        double size) {
  QPoint p;
  ToPoint(x, y, p);
