...
RefDialog::RefDialog(QWidget *parent) : QDialog(parent) {
  ...
  connect(StaList, SIGNAL(cellDoubleClicked(int, int)), this,
          SLOT(StaListDblClick(int, int)));
...
void RefDialog::showEvent(QShowEvent *event) {
  ...
  QStringList columns;
  columns << tr("No") << tr("Latitude(%1)").arg(degreeChar)
          << tr("Longitude(%1)").arg(degreeChar) << tr("Height(m)") << tr("Id")
          << tr("Name") << tr("Dist(km)");
...
void RefDialog::BtnLoadClick() {
  StaPosFile = QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("Load Station List..."), StaPosFile,
      tr("Position File (*.pos *.snx);;All (*.*)")));
...
void RefDialog::LoadSinex(void) {
  ...
  while (!file.atEnd()) {
    ...
    if (buff.contains("+SOLUTION/ESTIMATE"))
      sol = 1;
    if (buff.contains("-SOLUTION/ESTIMATE"))
      sol = 0;
...
void RefDialog::AddRef(int n, double *pos, const QString code,
  ...
  StaList->setItem(row - 1, 1,
                   new QTableWidgetItem(QString::number(pos[0], 'f', 9)));
  StaList->setItem(row - 1, 2,
                   new QTableWidgetItem(QString::number(pos[1], 'f', 9)));
  StaList->setItem(row - 1, 3,
                   new QTableWidgetItem(QString::number(pos[2], 'f', 4)));
...
void RefDialog::UpdateDist(void) {
  ...
  for (int i = 1; i < StaList->rowCount(); i++) {
    ...
    StaList->setItem(
        i, 6, new QTableWidgetItem(QString::number(norm(rr, 3) / 1E3, 'f', 1)));
  }
}
//---------------------------------------------------------------------------
