...
extern int showmsg(char *format, ...) {
  ...
  va_end(arg);
  QMetaObject::invokeMethod(mainWindow->Message, "setText",
                            Qt::QueuedConnection,
...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent) {
  ...
  connect(BtnOutFileView1, SIGNAL(clicked(bool)), this,
          SLOT(BtnOutFileView1Click()));
  connect(BtnOutFileView2, SIGNAL(clicked(bool)), this,
          SLOT(BtnOutFileView2Click()));
  connect(BtnOutFileView3, SIGNAL(clicked(bool)), this,
          SLOT(BtnOutFileView3Click()));
  connect(BtnOutFileView4, SIGNAL(clicked(bool)), this,
          SLOT(BtnOutFileView4Click()));
  connect(BtnOutFileView5, SIGNAL(clicked(bool)), this,
          SLOT(BtnOutFileView5Click()));
  connect(BtnOutFileView6, SIGNAL(clicked(bool)), this,
          SLOT(BtnOutFileView6Click()));
  connect(BtnOutFileView7, SIGNAL(clicked(bool)), this,
          SLOT(BtnOutFileView7Click()));
  ...
  connect(BtnInFileView, SIGNAL(clicked(bool)), this,
          SLOT(BtnInFileViewClick()));
...
void MainWindow::showEvent(QShowEvent *event) {
  ...
  QCommandLineOption titleOption(
      QStringList() << "t", QCoreApplication::translate("main", "window title"),
...
void MainWindow::SetOutFiles(const QString &infile) {
  QLineEdit *edit[] = {OutFile1, OutFile2, OutFile3, OutFile4,
                       OutFile5, OutFile6, OutFile7};
  ...
  if (!RnxFile) {
    ...
    if (RnxVer >= 3 && NavSys &&
        (NavSys != SYS_GPS)) { /* ver.3 and mixed system */
...
// read history -------------------------------------------------------------
void MainWindow::ReadList(QComboBox *combo, QSettings *ini,
                          const QString &key) {
  QString item;
  int i;
...
// write history ------------------------------------------------------------
void MainWindow::WriteList(QSettings *ini, const QString &key,
...
void MainWindow::BtnPlotClick() {
  ...
  QString cmd1 = "rtkplot_Qt", cmd2 = "..\\..\\..\\bin\\rtkplot_qt",
          opts = " -r";
  QCheckBox *cb[] = {OutFileEna1, OutFileEna2, OutFileEna3,
                     OutFileEna4, OutFileEna5, OutFileEna6};
  int i, ena[6];

  for (i = 0; i < 6; i++)
    ena[i] = cb[i]->isEnabled() && cb[i]->isChecked();

  for (i = 0; i < 6; i++) {
    if (ena[i])
      opts = opts + " \"" + RepPath(file[i]) + "\"";
  }
  if (opts == " -r")
    return;
...
void MainWindow::BtnPostClick() {
  ...
  if (TimeStartF->isChecked())
    opts = opts + " -ts " + TimeY1->text() + " " + TimeH1->text();
  if (TimeEndF->isChecked())
    opts = opts + " -te " + TimeY2->text() + " " + TimeH2->text();
  if (TimeIntF->isChecked())
    opts = opts + " -ti " + TimeInt->currentText();
  if (TimeUnitF->isChecked())
    opts = opts + " -tu " + TimeUnit->text();
...
void MainWindow::BtnOptionsClick() {
  int rnxfile = RnxFile;
  convOptDialog->exec();
  if (convOptDialog->result() != QDialog::Accepted)
    return;
  if (RnxFile != rnxfile) {
    SetOutFiles(InFile->currentText());
  }
...
void MainWindow::BtnInFileClick() {
  InFile->setCurrentText(QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("Input RTCM, RCV RAW or RINEX OBS File"), QString(),
      tr("All (*.*);;RTCM 2 (*.rtcm2);;RTCM 3 (*.rtcm3);;NovtAtel "
         "(*.gps);;ublox (*.ubx);;Swiftnav (*.sbp);;"
         "Hemisphere (*.bin);;Javad (*.jps);;RINEX OBS (*.obs *.*O);Septentrio "
         "(*.sbf)"))));
  SetOutFiles(InFile->currentText());
}
...
void MainWindow::BtnOutFile1Click() {
  ...
  OutFile1->setText(QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("Output RINEX OBS File"), QString(),
      tr("All (*.*);;RINEX OBS (*.obs *.*O);;RINEX NAV (*.nav *.*N "
         "*.*P);;RINEX GNAV (*.gnav *.*G);;RINEX HNAV (*.hnav *.*H);;"
         "RINEX QNAV (*.qnav *.*Q);;RINEX LNAV (*.lnav *.*L);;SBAS Log "
         "(*.sbs);;LEX Log (*.lex)"),
      &selectedFilter)));
}
...
void MainWindow::BtnOutFile2Click() {
  ...
  OutFile2->setText(QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("Output RINEX NAV File"), QString(),
      tr("All (*.*);;RINEX OBS (*.obs *.*O);;RINEX NAV (*.nav *.*N "
         "*.*P);;RINEX GNAV (*.gnav *.*G);;RINEX HNAV (*.hnav *.*H);;"
         "RINEX QNAV (*.qnav *.*Q);;RINEX LNAV (*.lnav *.*L);;SBAS Log "
         "(*.sbs);;LEX Log (*.lex)"),
...
void MainWindow::BtnOutFile3Click() {
  ...
  OutFile3->setText(QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("Output RINEX GNAV File"), QString(),
      tr("All (*.*);;RINEX OBS (*.obs *.*O);;RINEX NAV (*.nav *.*N "
         "*.*P);;RINEX GNAV (*.gnav *.*G);;RINEX HNAV (*.hnav *.*H);;"
         "RINEX QNAV (*.qnav *.*Q);;RINEX LNAV (*.lnav *.*L);;SBAS Log "
         "(*.sbs);;LEX Log (*.lex)"),
...
void MainWindow::BtnOutFile4Click() {
  ...
  OutFile4->setText(QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("Output RINEX HNAV File"), QString(),
      tr("All (*.*);;RINEX OBS (*.obs *.*O);;RINEX NAV (*.nav *.*N "
         "*.*P);;RINEX GNAV (*.gnav *.*G);;RINEX HNAV (*.hnav *.*H);;"
         "RINEX QNAV (*.qnav *.*Q);;RINEX LNAV (*.lnav *.*L);;SBAS Log "
         "(*.sbs);;LEX Log (*.lex)"),
...
void MainWindow::BtnOutFile5Click() {
  ...
  OutFile5->setText(QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("Output RINEX QNAV File"), QString(),
      tr("All (*.*);;RINEX OBS (*.obs *.*O);;RINEX NAV (*.nav *.*N "
         "*.*P);;RINEX GNAV (*.gnav *.*G);;RINEX HNAV (*.hnav *.*H);;"
         "RINEX QNAV (*.qnav *.*Q);;RINEX LNAV (*.lnav *.*L);;SBAS Log "
         "(*.sbs);;LEX Log (*.lex)"),
...
void MainWindow::BtnOutFile6Click() {
  ...
  OutFile6->setText(QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("Output RINEX LNAV File"), QString(),
      tr("All (*.*);;RINEX OBS (*.obs *.*O);;RINEX NAV (*.nav *.*N "
         "*.*P);;RINEX GNAV (*.gnav *.*G);;RINEX HNAV (*.hnav *.*H);;"
         "RINEX QNAV (*.qnav *.*Q);;RINEX LNAV (*.lnav *.*L);;SBAS Log "
         "(*.sbs);;LEX Log (*.lex)"),
...
void MainWindow::BtnOutFile7Click() {
  ...
  OutFile7->setText(QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("Output SBAS/LEX Log File"), QString(),
      tr("All (*.*);;RINEX OBS (*.obs *.*O);;RINEX NAV (*.nav *.*N "
         "*.*P);;RINEX GNAV (*.gnav *.*G);;RINEX HNAV (*.hnav *.*H);;"
         "RINEX QNAV (*.qnav *.*Q);;RINEX LNAV (*.lnav *.*L);;SBAS Log "
         "(*.sbs);;LEX Log (*.lex)"),
...
void MainWindow::BtnInFileViewClick() {
  ...
  if ((ext == "obs") || (ext == "OBS") || (ext == "nav") || (ext == "NAV") ||
      (ext.mid(1) == "nav") || (ext.mid(1) == "NAV") || (ext.at(2) == 'o') ||
      (ext.at(2) == 'O') || (ext.at(2) == 'n') || (ext.at(2) == 'N') ||
      (ext.at(2) == 'p') || (ext.at(2) == 'P') || (ext.at(2) == 'g') ||
      (ext.at(2) == 'G') || (ext.at(2) == 'h') || (ext.at(2) == 'H') ||
      (ext.at(2) == 'q') || (ext.at(2) == 'Q') || (ext.at(2) == 'l') ||
      (ext.at(2) == 'L')) {
    viewer->show();
...
QString MainWindow::RepPath(const QString &File) {
  QString Path;
  char path[1024];
  reppath(qPrintable(File), path, timeadd(RnxTime, TSTARTMARGIN),
          qPrintable(RnxCode), "");
  return Path = path;
}
// execute command ----------------------------------------------------------
...
void MainWindow::UpdateEnable(void) {
  ...
  TimeUnit->setEnabled(TimeStartF->isChecked() && TimeEndF->isChecked() &&
                       TimeUnitF->isChecked());
  ...
  OutFile3->setEnabled(OutFileEna3->isChecked() && RnxVer < 3 &&
                       (NavSys & SYS_GLO));
  OutFile4->setEnabled(OutFileEna4->isChecked() && RnxVer < 3 &&
                       (NavSys & SYS_SBS));
  OutFile5->setEnabled(OutFileEna5->isChecked() && RnxVer < 3 &&
                       (NavSys & SYS_QZS));
  OutFile6->setEnabled(OutFileEna6->isChecked() && RnxVer < 3 &&
                       (NavSys & SYS_GAL));
...
void MainWindow::ConvertFile(void) {
  ...
  if (Format->currentIndex() == 0) { // auto
    ...
    for (i = 0; formatstrs[i]; i++) {
      if (Format->currentText() == formatstrs[i])
        break;
    }
    if (formatstrs[i])
  ...
  if (conversionThread->format == STRFMT_RTCM2 ||
      conversionThread->format == STRFMT_RTCM3 ||
      conversionThread->format == STRFMT_RT17 ||
      conversionThread->format == STRFMT_CMR) {
  ...
  for (i = 0; i < 6; i++) {
    if (!QFile(conversionThread->ofile[i]).exists())
      continue;
    if (QMessageBox::question(
            this, tr("Overwrite"),
            QString(tr("%1 exists. Do you want to overwrite?"))
                .arg(conversionThread->ofile[i])) != QMessageBox::Yes)
      return;
  }
  ...
  sprintf(conversionThread->rnxopt.prog, "%s %s %s", PRGNAME, VER_RTKLIB,
          PATCH_LEVEL);
  ...
  for (i = 0; i < 2; i++)
    strncpy(conversionThread->rnxopt.name[i], qPrintable(Name[i]), 31);
  for (i = 0; i < 3; i++)
    strncpy(conversionThread->rnxopt.rec[i], qPrintable(Rec[i]), 31);
  for (i = 0; i < 3; i++)
    strncpy(conversionThread->rnxopt.ant[i], qPrintable(Ant[i]), 31);
  if (AutoPos) {
    for (i = 0; i < 3; i++)
      conversionThread->rnxopt.apppos[i] = AppPos[i];
  }
  for (i = 0; i < 3; i++)
    conversionThread->rnxopt.antdel[i] = AntDel[i];
  ...
  if (*conversionThread->rnxopt.rcvopt)
    sprintf(p, ", option: %s", conversionThread->rnxopt.rcvopt);
  for (i = 0; i < 2; i++)
    strncpy(conversionThread->rnxopt.comment[i + 2], qPrintable(Comment[i]),
            63);
  for (i = 0; i < 6; i++)
    strcpy(conversionThread->rnxopt.mask[i], qPrintable(CodeMask[i]));
  ...
  foreach (const QString &sat, exsatsLst) {
    int satid;
    if (!(satid = satid2no(qPrintable(sat))))
      continue;
  ...
  connect(conversionThread, SIGNAL(finished()), this,
          SLOT(ConversionFinished()));
