...
extern int showmsg(char *format, ...) {
  ...
  if ((p = strstr(buff, "STAT="))) {
    ...
    if (buff2.endsWith('_'))
      buff2.remove(buff2.length() - 1, 1);
    buff2.append(p + 5);
    QMetaObject::invokeMethod(mainForm->MsgLabel3, "setText",
                              Qt::QueuedConnection,
                              Q_ARG(QString, QString(buff2)));
  } else if ((p = strstr(buff, "->"))) {
    *p = '\0';
    QMetaObject::invokeMethod(mainForm->MsgLabel1, "setText",
                              Qt::QueuedConnection,
                              Q_ARG(QString, QString(buff)));
    QMetaObject::invokeMethod(mainForm->MsgLabel2, "setText",
                              Qt::QueuedConnection,
...
public:
  ...
  explicit DownloadThread(QObject *parent, const QString lf, bool a, bool t)
      : QThread(parent) {
    seqnos = 0;
    seqnoe = 0;
    opts = 0;
    ...
    for (int i = 0; i < MAX_URL_SEL; i++)
      memset(&urls[i], 0, sizeof(url_t));
...
protected:
  void run() {
    ...
    if (test)
      ...
      dl_exec(ts, te, ti, seqnos, seqnoe, urls, nurl, stas, nsta, dir,
              qPrintable(usr), qPrintable(pwd), qPrintable(proxy), opts, msg,
              fp);
    if (fp)
      fclose(fp);
  }
...
MainForm::MainForm(QWidget *parent) : QWidget(parent) {
  ...
  connect(DataType, SIGNAL(currentIndexChanged(int)), this,
          SLOT(DataTypeChange()));
  connect(SubType, SIGNAL(currentIndexChanged(int)), this,
          SLOT(DataTypeChange()));
...
void MainForm::FormCreate() {
  ...
  setWindowTitle(
      QString("%1 v.%2 %3").arg(PRGNAME).arg(VER_RTKLIB).arg(PATCH_LEVEL));
...
void MainForm::BtnFileClick() {
  ...
  GetTime(&ts, &te, &ti);
  if (str != "")
    reppath(qPrintable(str), path, ts, "", "");
...
void MainForm::BtnOptsClick() {
  ...
  if (downOptDialog.result() != QDialog::Accepted)
    return;
...
void MainForm::BtnDownloadClick() {
  ...
  if (!SkipExist->isChecked())
    thread->opts |= DLOPT_FORCE;
  if (!UnZip->isChecked())
    thread->opts |= DLOPT_KEEPCMP;
  if (HoldErr)
    thread->opts |= DLOPT_HOLDERR;
  if (HoldList)
    thread->opts |= DLOPT_HOLDLST;
...
void MainForm::BtnStasClick() {
  ...
  if (staListDialog.result() != QDialog::Accepted)
    return;
  UpdateStaList();
  BtnAll->setText("A");
}
...
void MainForm::BtnTime1Click() {
  ...
  gtime_t t1;
  t1.time = static_cast<time_t>(time.toTime_t());
...
void MainForm::BtnTime2Click() {
  ...
  gtime_t t2;
  t2.time = static_cast<time_t>(time.toTime_t());
...
void MainForm::LoadOpt(void) {
  ...
  TimeInt->insertItem(0, setting.value("opt/timeint", "24 H").toString());
  TimeInt->setCurrentIndex(0);
  ...
  for (int i = 0; i < 10; i++) {
    stas = setting.value(QString("sta/station%1").arg(i), "")
               .toString()
               .split(",");
  ...
  TextViewer::Color1 =
      setting.value("viewer/color1", QColor(Qt::black)).value<QColor>();
  TextViewer::Color2 =
      setting.value("viewer/color2", QColor(Qt::white)).value<QColor>();
  TextViewer::FontD.setFamily(
      setting.value("viewer/fontname", "Courier New").toString());
...
void MainForm::SaveOpt(void) {
  ...
  for (int i = 0, j = 0; i < 10; i++) {
    for (int k = 0; k < 256 && j < StaList->count(); k++) {
      sta.append(k == 0 ? QString("")
                        : QString(",") + StaList->item(i)->text());
    }
...
void MainForm::LoadUrl(QString file) {
  ...
  for (i = 0; i < n; i++) {
    ...
    if ((p = subtype.indexOf('_')) != -1)
      subtype = subtype.mid(0, p);
...
void MainForm::GetTime(gtime_t *ts, gtime_t *te, double *ti) {
  ...
  eps[0] = TimeY1->date().year();
  eps[1] = TimeY1->date().month();
  eps[2] = TimeY1->date().day();
  eps[3] = TimeH1->time().hour();
  eps[4] = TimeH1->time().minute();
  epe[0] = TimeY2->date().year();
  epe[1] = TimeY2->date().month();
  epe[2] = TimeY2->date().day();
  epe[3] = TimeH2->time().hour();
  epe[4] = TimeH2->time().minute();
...
int MainForm::SelectUrl(url_t *urls) {
  ...
  for (i = 0; i < MAX_URL_SEL; i++)
    types[i] = new char[64];

  for (i = 0; i < DataList->count() && nurl < MAX_URL_SEL; i++) {
    if (!DataList->item(i)->isSelected())
      continue;
...
void MainForm::UpdateType(void) {
  ...
  for (i = 0; i < Types.size(); i++) {
    ...
    if (DataType->currentText() != tr("ALL") && DataType->currentText() != type)
      continue;
    if (SubType->currentText() != "" && SubType->currentText() != subtype)
      continue;
...
void MainForm::UpdateMsg(void) {
  ...
  for (i = 0; i < DataList->count(); i++) {
    ...
    for (j = 0; j < Types.count(); j++) {
      if (DataList->item(i)->text() != Types.at(j))
        continue;
      MsgLabel1->setText(Urls.at(j));
      MsgLabel2->setText(LocalDir->isChecked() ? Dir->currentText()
                                               : Locals.at(j));
...
void MainForm::ReadHist(QSettings &setting, QString key, QComboBox *combo) {
  ...
  for (i = 0; i < MAX_HIST; i++) {
    item = setting.value(QString("history/%1_%2").arg(key).arg(i, 3), "")
               .toString();
    if (item != "")
      combo->addItem(item);
  }
...
void MainForm::WriteHist(QSettings &setting, QString key, QComboBox *combo) {
  ...
  for (i = 0; i < combo->count(); i++) {
    setting.setValue(QString("history/%1_%2").arg(key).arg(i),
                     combo->itemText(i));
  }
}
// --------------------------------------------------------------------------
void MainForm::AddHist(QComboBox *combo) {
  ...
  for (int i = combo->count() - 1; i >= MAX_HIST; i--)
    combo->removeItem(i);
  combo->setCurrentIndex(0);
}
//---------------------------------------------------------------------------
int MainForm::ExecCmd(QString cmd) { return QProcess::startDetached(cmd); }
//---------------------------------------------------------------------------
