...
MainForm::MainForm(QWidget *parent) : QDialog(parent) {
  ...
  QCoreApplication::setApplicationVersion(QString(VER_RTKLIB) + " " +
                                          PATCH_LEVEL);
  ...
  connect(Output1, SIGNAL(currentIndexChanged(int)), this,
          SLOT(Output1Change()));
  connect(Output2, SIGNAL(currentIndexChanged(int)), this,
          SLOT(Output2Change()));
  connect(Output3, SIGNAL(currentIndexChanged(int)), this,
          SLOT(Output3Change()));
...
void MainForm::FormCreate() {
  ...
  parser.addPositionalArgument(
      "source", QCoreApplication::translate("main", "Source file to copy."));
  parser.addPositionalArgument(
      "destination",
  ...
  QCommandLineOption windowTitleOption(
      QStringList() << "t",
...
void MainForm::Output1Change() { UpdateEnable(); }
// callback on output2 type change ------------------------------------------
void MainForm::Output2Change() { UpdateEnable(); }
// callback on output3 type change ------------------------------------------
void MainForm::Output3Change() { UpdateEnable(); }
// callback on interval timer -----------------------------------------------
void MainForm::Timer1Timer() {
  QColor color[] = {Qt::red, Qt::gray, CLORANGE, Qt::green,
  ...
  for (int i = 0; i < 4; i++) {
    ...
    num2cnum(bps[i], s2);
    e0[i]->setStyleSheet(
        QString("background-color: %1").arg(color2String(color[stat[i] + 1])));
    e1[i]->setText(s1);
    e2[i]->setText(s2);
  }
  Progress->setValue(!stat[0] ? 0
                              : MIN(100, (int)(fmod(byte[0] / 500.0, 110.0))));
...
void MainForm::SvrStart(void) {
  ...
  int otype[] = {STR_NONE,   STR_SERIAL,   STR_TCPCLI,
                 STR_TCPSVR, STR_NTRIPSVR, STR_FILE};
  ...
  strcpy(paths[1], !Output1->currentIndex()
                       ? ""
                       : qPrintable(Paths[1][ip[Output1->currentIndex() - 1]]));
  strcpy(paths[2], !Output2->currentIndex()
                       ? ""
                       : qPrintable(Paths[2][ip[Output2->currentIndex() - 1]]));
  strcpy(paths[3], !Output3->currentIndex()
                       ? ""
                       : qPrintable(Paths[3][ip[Output3->currentIndex() - 1]]));

  if (Input->currentIndex() == 0) {
    if (CmdEna[0])
      strncpy(cmds[0], qPrintable(Cmds[0]), 1024);
  } else if (Input->currentIndex() == 1 || Input->currentIndex() == 3) {
    if (CmdEnaTcp[0])
      strncpy(cmds[0], qPrintable(CmdsTcp[0]), 1024);
  }
  for (int i = 0; i < 5; i++) {
    opt[i] = SvrOpt[i];
  }
  ...
  // stream server start
  if (!strsvrstart(&strsvr, opt, strs, paths, conv, cmds, cmds_periodic,
                   AntPos))
    return;
...
void MainForm::SvrStop(void) {
  ...
  if (Input->currentIndex() == 0) {
    if (CmdEna[1])
      strncpy(cmds[0], qPrintable(Cmds[1]), 1024);
  } else if (Input->currentIndex() == 1 || Input->currentIndex() == 3) {
    if (CmdEnaTcp[1])
      strncpy(cmds[0], qPrintable(CmdsTcp[1]), 1024);
  }
...
void MainForm::UpdateEnable(void) {
  ...
  LabelOutput1->setStyleSheet(
      QString("color :%1")
          .arg(color2String(Output1->currentIndex() > 0 ? Qt::black
                                                        : Qt::gray)));
  LabelOutput2->setStyleSheet(
      QString("color :%1")
          .arg(color2String(Output2->currentIndex() > 0 ? Qt::black
                                                        : Qt::gray)));
  LabelOutput3->setStyleSheet(
      QString("color :%1")
          .arg(color2String(Output3->currentIndex() > 0 ? Qt::black
                                                        : Qt::gray)));
  Output1Byte->setStyleSheet(
      QString("color :%1")
          .arg(color2String(Output1->currentIndex() > 0 ? Qt::black
                                                        : Qt::gray)));
  Output2Byte->setStyleSheet(
      QString("color :%1")
          .arg(color2String(Output2->currentIndex() > 0 ? Qt::black
                                                        : Qt::gray)));
  Output3Byte->setStyleSheet(
      QString("color :%1")
          .arg(color2String(Output3->currentIndex() > 0 ? Qt::black
                                                        : Qt::gray)));
  Output1Bps->setStyleSheet(
      QString("color :%1")
          .arg(color2String(Output1->currentIndex() > 0 ? Qt::black
                                                        : Qt::gray)));
  Output2Bps->setStyleSheet(
      QString("color :%1")
          .arg(color2String(Output2->currentIndex() > 0 ? Qt::black
                                                        : Qt::gray)));
  Output3Bps->setStyleSheet(
      QString("color :%1")
          .arg(color2String(Output3->currentIndex() > 0 ? Qt::black
                                                        : Qt::gray)));
...
void MainForm::SetTrayIcon(int index) {
  QString icon[] = {":/icons/tray0.bmp", ":/icons/tray1.bmp",
                    ":/icons/tray2.bmp"};
...
void MainForm::LoadOpt(void) {
  ...
  for (int i = 0; i < 6; i++) {
    SvrOpt[i] =
        settings.value(QString("set/svropt_%1").arg(i), optdef[i]).toInt();
  }
  for (int i = 0; i < 3; i++) {
  ...
  for (int i = 0; i < 4; i++)
    for (int j = 0; j < 4; j++) {
      Paths[i][j] = settings.value(QString("path/path_%1_%2").arg(i).arg(j), "")
                        .toString();
    }
  for (int i = 0; i < 2; i++) {
...
void MainForm::SaveOpt(void) {
  ...
  for (int i = 0; i < MAXHIST; i++) {
    settings.setValue(QString("tcpopt/history%1").arg(i),
                      tcpOptDialog->History[i]);
  }
  for (int i = 0; i < MAXHIST; i++) {
    settings.setValue(QString("tcpopt/mntphist%1").arg(i),
                      tcpOptDialog->MntpHist[i]);
  }
