...
PntDialog::PntDialog(QWidget *parent) : QDialog(parent) {
  ...
  connect(PntList, SIGNAL(itemChanged(QTableWidgetItem *)), this,
          SLOT(PntListSetEditText()));
  ...
  for (int i = PntList->column(sel); i < PntList->rowCount(); i++) {
    for (int j = 0; j < PntList->columnCount(); j++) {
      if (i + 1 >= PntList->rowCount())
        ...
        PntList->setItem(i, j,
                         new QTableWidgetItem(PntList->item(i + 1, j)->text()));
    }
  }
  PntList->setRowCount(PntList->rowCount() - 1);
  noUpdate = false;
...
void PntDialog::SetPoint(void) {
  ...
  for (int i = 0; i < plot->NWayPnt; i++) {
    PntList->setItem(
        i, 0,
        new QTableWidgetItem(QString::number(plot->PntPos[i][0], 'f', 9)));
    PntList->setItem(
        i, 1,
        new QTableWidgetItem(QString::number(plot->PntPos[i][1], 'f', 9)));
