...
ConnectDialog::ConnectDialog(QWidget *parent) : QDialog(parent) {
  ...
  connect(SolFormat1, SIGNAL(currentIndexChanged(int)), this,
          SLOT(SolFormat1Change()));
  connect(SolFormat2, SIGNAL(currentIndexChanged(int)), this,
          SLOT(SolFormat2Change()));
  connect(SelStream1, SIGNAL(currentIndexChanged(int)), this,
          SLOT(SelStream1Change()));
  connect(SelStream2, SIGNAL(currentIndexChanged(int)), this,
          SLOT(SelStream2Change()));
}
//---------------------------------------------------------------------------
void ConnectDialog::showEvent(QShowEvent *event) {
  int str[] = {STR_NONE,   STR_SERIAL,   STR_TCPCLI,
               STR_TCPSVR, STR_NTRIPCLI, STR_FILE};
...
void ConnectDialog::UpdateEnable(void) {
  ...
  TimeFormS->setEnabled(SolFormat1->currentIndex() != 3 ||
                        SolFormat2->currentIndex() != 3);
  DegFormS->setEnabled(SolFormat1->currentIndex() == 0 ||
                       SolFormat2->currentIndex() == 0);
  FieldSepS->setEnabled(SolFormat1->currentIndex() != 3 ||
                        SolFormat2->currentIndex() != 3);
  Label5->setEnabled(SolFormat1->currentIndex() != 3 ||
                     SolFormat2->currentIndex() != 3);
  Label6->setEnabled(SolFormat1->currentIndex() == 0 ||
                     SolFormat2->currentIndex() == 0);
  Label7->setEnabled(SolFormat1->currentIndex() != 3 ||
                     SolFormat2->currentIndex() != 3);
  Label8->setEnabled(
      (2 <= SelStream1->currentIndex() && SelStream1->currentIndex() <= 4) ||
      (2 <= SelStream2->currentIndex() && SelStream2->currentIndex() <= 4));
  TimeOutTimeE->setEnabled(
      (2 <= SelStream1->currentIndex() && SelStream1->currentIndex() <= 4) ||
      (2 <= SelStream2->currentIndex() && SelStream2->currentIndex() <= 4));
  ReConnTimeE->setEnabled(
      (2 <= SelStream1->currentIndex() && SelStream1->currentIndex() <= 4) ||
      (2 <= SelStream2->currentIndex() && SelStream2->currentIndex() <= 4));
}
//---------------------------------------------------------------------------
