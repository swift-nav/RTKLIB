...
void Plot::ReadSol(const QStringList &files, int sel) {
  ...
  for (i = 0; i < 2; i++) {
    if (SolFiles[i].length() == 0)
      continue;
    setWindowTitle(windowTitle() + SolFiles[i].at(0) +
                   (SolFiles[i].count() > 1 ? "... " : " "));
  }
...
void Plot::ReadNav(const QStringList &files) {
  ...
  if (Nav.n <= 0 && Nav.ng <= 0 && Nav.ns <= 0) {
    ShowMsg(QString(tr("no nav message: %1..."))
                .arg(QDir::toNativeSeparators(files.at(i))));
    ReadWaitEnd();
    return;
  }
  if (NavFiles != files) {
  ...
  for (i = 0; i < NavFiles.size(); i++)
    NavFiles[i] = QDir::toNativeSeparators(NavFiles.at(i));
  ...
  for (time = ts; timediff(time, te) <= 0.0; time = timeadd(time, tint)) {
    for (i = 0; i < MAXSAT; i++) {
      ...
      if (Obs.n >= Obs.nmax) {
        ...
        Obs.data =
            static_cast<obsd_t *>(realloc(Obs.data, sizeof(obsd_t) * Obs.nmax));
        if (!Obs.data) {
          Obs.n = Obs.nmax = 0;
          break;
        }
...
#define ResPixelBL(img1, x, y, b1, pix)                                        \
  {                                                                            \
    ...
    pix = qRgb(static_cast<quint8>(a1 * qRed(p1) + a2 * qRed(p2) +             \
                                   a3 * qRed(p1) + a4 * qRed(p2)),             \
               static_cast<quint8>(a1 * qRed(p1) + a2 * qGreen(p2) +           \
                                   a3 * qGreen(p1) + a4 * qGreen(p2)),         \
               static_cast<quint8>(a1 * qRed(p1) + a2 * qBlue(p2) +            \
                                   a3 * qBlue(p1) + a4 * qBlue(p2)));          \
  }
...
static void YCrCb(const QRgb *pix, double *Y) {
  ...
  Y[0] = (0.299 * qRed(*pix) + 0.587 * qGreen(*pix) + 0.114 * qBlue(*pix)) /
         255; // Y  (0-1)
  Y[1] = (0.500 * qRed(*pix) - 0.419 * qGreen(*pix) + 0.081 * qBlue(*pix)) /
         255; // Cr (-.5-.5)
  Y[2] = (-0.169 * qRed(*pix) - 0.331 * qGreen(*pix) + 0.500 * qBlue(*pix)) /
         255; // Cb (-.5-.5)
}
// update sky image ---------------------------------------------------------
void Plot::UpdateSky(void) {
  ...
  QRgb pix;
  double x, y, xp, yp, r, a, p[3], q[3], R[9] = {0}, dr, dist, Yz[3] = {0},
                                         Y[3];
...
void Plot::ReadSkyTag(const QString &file) {
  ...
  while (!fp.atEnd()) {
    buff = fp.readLine();
    if (buff.at(0) == '\0' || buff.at(0) == '%' || buff.at(0) == '#')
      continue;
    QList<QByteArray> tokens = buff.split('=');
    if (tokens.size() < 2)
      continue;
...
void Plot::ReadStaPos(const QString &file, const QString &sta, double *rr) {
  ...
  while (!fp.atEnd()) {
    ...
    if (sinex) {
      ...
      if ((buff.length() < 68) || (buff.mid(14, 4) != sta))
        continue;
      if (buff.mid(7, 4) == "STAX")
        rr[0] = buff.mid(47, 21).toDouble();
      if (buff.mid(7, 4) == "STAY")
        rr[1] = buff.mid(47, 21).toDouble();
...
void Plot::SaveDop(const QString &file) {
  ...
  tlabel = TimeLabel <= 1
               ? tr("TIME (GPST)")
               : (TimeLabel <= 2 ? tr("TIME (UTC)") : tr("TIME (JST))"));
  ...
  for (i = 0; i < MAXSAT; i++) {
    ...
    for (j = 0; j < Obs.n; j++) {
      ...
      data = QString("%1 %2 %3 %4 %5 %6f\n")
                 ...
                 .arg(Az[j] * R2D, 8, 'f', 1)
                 .arg(El[j] * R2D, 8, 'f', 1)
                 .arg(Obs.data[j].SNR[k] * 0.25, 9, 'f', 2)
...
void Plot::SaveElMask(const QString &file) {
  ...
  for (az = 0; az <= 360; az++) {
    ...
    if (qFuzzyCompare(el, el0))
      continue;
    data = QString("%1 %2\n")
               .arg(static_cast<double>(az), 9, 'f', 1)
               .arg(el, 6, 'f', 1);
    fp.write(data.toLatin1());
    el0 = el;
  }
...
void Plot::Connect(void) {
  ...
  if (Title != "")
    ...
    setWindowTitle(QString(tr("CONNECT %1 %2")).arg(name[0]).arg(name[1]));
...
int Plot::CheckObs(const QString &file) {
  ...
  if (file.indexOf(".z") || file.indexOf(".gz") || file.indexOf(".zip") ||
    ...
    return file.at(p - 1) == 'o' || file.at(p - 1) == 'O' ||
           file.at(p - 1) == 'd' || file.at(p - 1) == 'D';
  }
...
void Plot::UpdateObs(int nobs) {
  ...
  for (i = 0; i < Obs.n; i = j) {
    ...
    if (RcvPos == 0) {
      ...
      for (k = 0; k < j - i; k++) {
        ...
        if (!satpos(time, time, Obs.data[i + k].sat, EPHOPT_BRDC, &Nav, rs, dts,
          ...
          continue;
        if (geodist(rs, rr, e) > 0.0)
          satazel(pos, e, azel + k * 2);
      }
    }
    // satellite azel by tle data
    for (k = 0; k < j - i; k++) {
      if (azel[k * 2] != 0.0 || azel[1 + k * 2] != 0.0)
        continue;
    ...
    if (per != per_) {
      ShowMsg(
          QString(tr("updating azimuth/elevation... (%1%)")).arg(per_ = per));
      qApp->processEvents();
    }
  }
  IndexObs[NObs] = Obs.n;
