...
void Plot::UpdateTimeObs(void) {
  QString msgs1[] = {" OBS=L1/2 ", " L1 ", " L2 ", " L1/2/5 ",
                     " L1/5 ",     "",     " L5 "};
  QString msgs2[] = {" SNR=...45.", "..40.", "..35.", "..30.",
                     "..25 ",       "",      " <25 "};
  QString msgs3[] = {" SYS=GPS ", "GLO ", "GAL ", "QZS ", "BDS ", "SBS ", ""};
  QString msgs4[] = {" MP=..0.6", "..0.3", "..0.0..", "-0.3..",
                     "-0.6..",    "",      ""};
  QString msgs[8];
  QString msg;
...
void Plot::UpdateTimeSol(void) {
  ...
  int sel = BtnSol1->isChecked() || !BtnSol2->isChecked() ? 0 : 1,
      ind = SolIndex[sel];
  QString tstr;
  ...
  if ((BtnSol1->isChecked() || BtnSol2->isChecked() || BtnSol12->isChecked()) &&
    ...
    if (PLOT_SOLP <= PlotType && PlotType <= PLOT_SOLA) {
      ...
      r = norm(data->rr, 3);
      az = norm(data->rr, 2) <= 1E-12 ? 0.0
                                      : atan2(data->rr[0], data->rr[1]) * R2D;
    ...
    if (1 <= data->stat && data->stat <= 6) {
      msgs[data->stat - 1] =
          QString("%1:%2").arg(data->stat).arg(sol[data->stat]);
    }
  }
  ShowMsg(msg);
  ShowLegend(msgs);
...
void Plot::UpdateInfoObs(void) {
  ...
  if (BtnSol1->isChecked()) {
    for (i = 0; i < Obs.n; i++) {
      ...
      if (ts.time == 0)
        ts = t;
      te = t;
      if (tp.time == 0 || timediff(t, tp) > TTOL)
        ne++;
      n++;
      tp = t;
    }
  ...
  if (n > 0) {
    TimeStr(ts, 0, 0, s1);
    TimeStr(te, 0, 1, s2);
    msg = QString("[1]%1-%2 : EP=%3 N=%4")
              .arg(s1)
              .arg(s2.mid(TimeLabel ? 5 : 0))
              .arg(ne)
              .arg(n);
...
void Plot::UpdateInfoSol(void) {
  ...
  int i, j, n = 0, nq[8] = {0},
            sel = BtnSol1->isChecked() || !BtnSol2->isChecked() ? 0 : 1;
  QString s1, s2;
  ...
  for (i = 0; (data = getsol(SolData + sel, i)) != NULL; i++) {
    if (data->type) {
      ...
      for (j = 0; j < 3; j++)
        r[j] = data->rr[j] - SolData[sel].rb[j];
      b = norm(r, 3);
    } else
      b = 0.0;
    if (b < bl[0])
  ...
  if (n > 0) {
    ...
    TimeStr(te, 0, 1, s2);
    msg +=
        QString("%1-%2 : N=%3").arg(s1).arg(s2.mid(TimeLabel ? 5 : 0)).arg(n);
...
void Plot::UpdatePlotType(void) {
  ...
  for (i = 0; i < PlotTypeS->count(); i++) {
    if (PlotTypeS->itemText(i) != PTypes[PlotType])
      continue;
...
void Plot::UpdateSatList(void) {
  ...
  for (sat = 1; sat <= MAXSAT; sat++) {
    if (SatMask[sat - 1] || !smask[sat - 1])
      continue;
    if ((sys = satsys(sat, NULL)) == sysp)
      continue;
    switch ((sysp = sys)) {
    case SYS_GPS:
      strcpy(s, "G");
