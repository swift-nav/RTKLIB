...
Plot::Plot(QWidget *parent) : QMainWindow(parent) {
  ...
  setlocale(LC_NUMERIC,
            "C"); // use point as decimal separator in formated output
  ...
  RangeList->setWindowFlags(Qt::Window | Qt::WindowStaysOnTopHint |
  ...
  connect(BtnSol1, SIGNAL(clicked(bool)), this,
          SLOT(BtnSol1Click())); // FIXME Double Click
  ...
  connect(MenuAnimStart, SIGNAL(triggered(bool)), this,
          SLOT(MenuAnimStartClick()));
  connect(MenuAnimStop, SIGNAL(triggered(bool)), this,
          SLOT(MenuAnimStopClick()));
  connect(MenuCenterOri, SIGNAL(triggered(bool)), this,
          SLOT(MenuCenterOriClick()));
  ...
  connect(MenuDisconnect, SIGNAL(triggered(bool)), this,
          SLOT(MenuDisconnectClick()));
  connect(MenuFileSel, SIGNAL(triggered(bool)), this, SLOT(MenuFileSelClick()));
  connect(MenuFitHoriz, SIGNAL(triggered(bool)), this,
          SLOT(MenuFitHorizClick()));
  ...
  connect(MenuFixHoriz, SIGNAL(triggered(bool)), this,
          SLOT(MenuFixHorizClick()));
  ...
  connect(MenuMonitor1, SIGNAL(triggered(bool)), this,
          SLOT(MenuMonitor1Click()));
  connect(MenuMonitor2, SIGNAL(triggered(bool)), this,
          SLOT(MenuMonitor2Click()));
  connect(MenuOpenElevMask, SIGNAL(triggered(bool)), this,
          SLOT(MenuOpenElevMaskClick()));
  connect(MenuOpenMapImage, SIGNAL(triggered(bool)), this,
          SLOT(MenuOpenMapImageClick()));
  connect(MenuOpenShape, SIGNAL(triggered(bool)), this,
          SLOT(MenuOpenShapeClick()));
  ...
  connect(MenuOpenSkyImage, SIGNAL(triggered(bool)), this,
          SLOT(MenuOpenSkyImageClick()));
  connect(MenuOpenSol1, SIGNAL(triggered(bool)), this,
          SLOT(MenuOpenSol1Click()));
  connect(MenuOpenSol2, SIGNAL(triggered(bool)), this,
          SLOT(MenuOpenSol2Click()));
  ...
  connect(MenuPlotGEGM, SIGNAL(triggered(bool)), this,
          SLOT(MenuPlotGEGMClick()));
  ...
  connect(MenuSaveElMask, SIGNAL(triggered(bool)), this,
          SLOT(MenuSaveElMaskClick()));
  connect(MenuSaveImage, SIGNAL(triggered(bool)), this,
          SLOT(MenuSaveImageClick()));
  connect(MenuSaveSnrMp, SIGNAL(triggered(bool)), this,
          SLOT(MenuSaveSnrMpClick()));
  ...
  connect(MenuShowSkyplot, SIGNAL(triggered(bool)), this,
          SLOT(MenuShowSkyplotClick()));
  connect(MenuShowTrack, SIGNAL(triggered(bool)), this,
          SLOT(MenuShowTrackClick()));
  ...
  connect(MenuStatusBar, SIGNAL(triggered(bool)), this,
          SLOT(MenuStatusBarClick()));
  ...
  connect(MenuMapLayer, SIGNAL(triggered(bool)), this,
          SLOT(MenuMapLayerClick()));
  connect(MenuOpenWaypoint, SIGNAL(triggered(bool)), this,
          SLOT(MenuOpenWaypointClick()));
  connect(MenuSaveWaypoint, SIGNAL(triggered(bool)), this,
          SLOT(MenuSaveWaypointClick()));
  connect(BtnMessage2, SIGNAL(clicked(bool)), this, SLOT(BtnMessage2Click()));
  connect(RangeList, SIGNAL(clicked(QModelIndex)), this,
          SLOT(RangeListClick()));
  connect(&Timer, SIGNAL(timeout()), this, SLOT(TimerTimer()));
  connect(PlotTypeS, SIGNAL(currentIndexChanged(int)), this,
          SLOT(PlotTypeSChange()));
  connect(QFlag, SIGNAL(currentIndexChanged(int)), this, SLOT(QFlagChange()));
  connect(TimeScroll, SIGNAL(valueChanged(int)), this,
          SLOT(TimeScrollChange()));
  connect(SatList, SIGNAL(currentIndexChanged(int)), this,
          SLOT(SatListChange()));
  connect(DopType, SIGNAL(currentIndexChanged(int)), this,
          SLOT(DopTypeChange()));
  connect(ObsType, SIGNAL(currentIndexChanged(int)), this,
          SLOT(ObsTypeChange()));
  connect(ObsType2, SIGNAL(currentIndexChanged(int)), this,
          SLOT(ObsTypeChange()));
  connect(FrqType, SIGNAL(currentIndexChanged(int)), this,
          SLOT(ObsTypeChange()));
...
void Plot::showEvent(QShowEvent *event) {
  ...
  parser.addPositionalArgument(
      "file", QCoreApplication::translate("main", "Read files."));
...
void Plot::MenuOpenSol1Click() {
  ...
  ReadSol(
      QStringList(QDir::toNativeSeparators(QFileDialog::getOpenFileName(
...
void Plot::MenuOpenMapImageClick() {
  ...
  ReadMapData(QDir::toNativeSeparators(
      QFileDialog::getOpenFileName(this, tr("Open Map Image"), MapImageFile,
                                   tr("JPEG File (*.jpg *.jpeg);;All (*.*)"))));
}
...
void Plot::MenuOpenShapeClick() {
  ...
  QStringList files =
      QFileDialog::getOpenFileNames(this, tr("Open Shape File"), QString(),
...
void Plot::MenuOpenSkyImageClick() {
  ...
  ReadSkyData(QDir::toNativeSeparators(
      QFileDialog::getOpenFileName(this, tr("Open Sky Image"), SkyImageFile,
                                   tr("JPEG File (*.jpg *.jpeg);;All (*.*)"))));
}
...
void Plot::MenuOpenWaypointClick() {
  ...
  ReadWaypoint(QDir::toNativeSeparators(
      QFileDialog::getOpenFileName(this, tr("Open Waypoint"), SkyImageFile,
                                   tr("GPX File (*.gpx);;All (*.*)"))));
}
...
void Plot::MenuOpenObsClick() {
  ...
  ReadObs(QFileDialog::getOpenFileNames(this, tr("Open Obs/Nav Data"),
                                        QString(),
                                        tr("RINEX OBS (*.obs *.*o *.*d *.*o.gz "
                                           "*.*o.Z *.d.gz *.d.Z);;All (*.*)")));
}
...
void Plot::MenuOpenNavClick() {
  ...
  ReadNav(QFileDialog::getOpenFileNames(
      ...
      tr("RINEX NAV (*.nav *.gnav *.hnav *.qnav *.*n *.*g *.*h *.*q *.*p);;All "
         "(*.*)")));
}
...
void Plot::MenuOpenElevMaskClick() {
  ...
  ReadElMaskData(QDir::toNativeSeparators(QDir::toNativeSeparators(
      QFileDialog::getOpenFileName(this, tr("Opene Elevation Mask"), QString(),
...
void Plot::MenuSaveImageClick() {
  Buff.save(QDir::toNativeSeparators(QFileDialog::getSaveFileName(
      ...
      tr("JPEG  (*.jpg);;Windows Bitmap (*.bmp)"))));
}
// callback on menu-save-waypoint -------------------------------------------
void Plot::MenuSaveWaypointClick() {
  trace(3, "MenuSaveWaypointClick\n");

  SaveWaypoint(QDir::toNativeSeparators(
      QFileDialog::getSaveFileName(this, tr("Save Waypoint"), QString(),
                                   tr("GPX File (*.gpx);;All (*.*)"))));
}
...
void Plot::MenuPortClick() {
  ...
  for (i = 0; i < 10; i++)
    connectDialog->TcpHistory[i] = StrHistory[i];
  for (i = 0; i < 10; i++)
    connectDialog->TcpMntpHist[i] = StrMntpHist[i];

  connectDialog->exec();

  if (connectDialog->result() != QDialog::Accepted)
    return;
  ...
  for (i = 0; i < 10; i++)
    StrHistory[i] = connectDialog->TcpHistory[i];
  for (i = 0; i < 10; i++)
    StrMntpHist[i] = connectDialog->TcpMntpHist[i];
}
...
void Plot::MenuTimeClick() {
  ...
  if (TimeEna[0] != spanDialog->TimeEna[0] ||
    ...
    for (i = 0; i < 3; i++)
      TimeEna[i] = spanDialog->TimeEna[i];
...
void Plot::MenuMapLayerClick() {
  ...
  vecMapDialog->exec();
  if (vecMapDialog->result() != QDialog::Accepted)
    return;
...
void Plot::MenuQcObsClick() {
  ...
  if (ObsFiles.count() <= 0 || cmd == "")
    return;

  for (i = 0; i < ObsFiles.count(); i++)
    cmd += " \"" + ObsFiles.at(i) + "\"";
  for (i = 0; i < NavFiles.count(); i++)
    cmd += " \"" + NavFiles.at(i) + "\"";
...
void Plot::MenuOptionsClick() {
  ...
  if (plotOptDialog->result() != QDialog::Accepted)
    return;
...
void Plot::MenuGEClick() {
  ...
  googleEarthView->setWindowTitle(QString(tr("%1 ver.%2 %3: Google Earth View"))
                                      .arg(PRGNAME)
                                      .arg(VER_RTKLIB)
                                      .arg(PATCH_LEVEL));
...
void Plot::MenuGMClick() {
  googleMapView->setWindowTitle(QString(tr("%1 ver.%2 %3: Google Map View"))
                                    .arg(PRGNAME)
                                    .arg(VER_RTKLIB)
                                    .arg(PATCH_LEVEL));
  googleMapView->show();
...
void Plot::MenuMaxClick() {
  ...
  this->resize(rect.width() - thisDecoration.width(),
               rect.height() - thisDecoration.height());
}
...
void Plot::MenuPlotGEClick() {
  ...
  QSize thisDecoration = this->frameSize() - this->size();
  this->move(rect.x(), rect.y());
  this->resize(rect.width() / 2 - thisDecoration.width(),
               rect.height() - thisDecoration.height());

  QSize GEDecoration = googleEarthView->frameSize() - googleEarthView->size();
  googleEarthView->move(rect.x(), rect.y());
  googleEarthView->resize(rect.width() - GEDecoration.width(),
                          rect.height() - GEDecoration.height());
...
void Plot::MenuPlotGMClick() {
  ...
  googleMapView->resize(rect.width() / 2 - GMDecoration.width(),
                        rect.height() - GMDecoration.height());
...
void Plot::MenuPlotGEGMClick() {
  ...
  googleEarthView->resize(rect.width() / 2 - GEDecoration.width(),
                          rect.height() / 2 - GEDecoration.height());
  googleMapView->move(rect.x() + rect.width() / 2,
                      rect.y() + rect.height() / 2);
  googleMapView->resize(rect.width() / 2 - GMDecoration.width(),
                        rect.height() / 2 - GMDecoration.height());
}
//---------------------------------------------------------------------------
#if 0
...
void Plot::RangeListClick() {
  ...
  RangeList->setVisible(false);
  if ((i = RangeList->currentItem()) == NULL)
    return;
...
void Plot::PlotTypeSChange() {
  ...
  for (i = 0; PTypes[i] != NULL; i++) {
    if (PlotTypeS->currentText() == PTypes[i])
      UpdateType(i);
  }
  UpdateTime();
  UpdatePlot();
...
void Plot::mouseMoveEvent(QMouseEvent *event) {
  ...
  if ((abs(event->globalX() - Xn) < 1) && (abs(event->globalY() - Yn) < 1))
    return;
...
void Plot::MouseDownSol(int X, int Y) {
  ...
  if (PlotType == PLOT_SNR) {
    ...
    if ((data = getsol(SolData + sel, SolIndex[sel])))
      time = data->time;
  }
  ...
  for (i = 0; i < 3; i++) {
    if (!btn[i]->isChecked() || (i != 1 && PlotType == PLOT_SNR))
      continue;
...
void Plot::MouseMoveTrk(int X, int Y, double dx, double dy, double dxs,
                        double dys) {
  ...
  if (updateTime.elapsed() < RefreshTime)
    return;
  updateTime.restart();
...
void Plot::MouseMoveSol(int X, int Y, double dx, double dy, double dxs,
                        double dys) {
  QPoint p1, p2, p(X, Y);
  double x, y, xs, ys;
  int i, sel = !BtnSol1->isChecked() && BtnSol2->isChecked() ? 1 : 0;
  ...
  if (Drag <= 4) {
    ...
    if (PlotType == PLOT_SNR) {
      for (i = 0; i < NObs; i++) {
        if (TimePos(Obs.data[IndexObs[i]].time) >= x)
          break;
      }
      ObsIndex = i < NObs ? i : NObs - 1;
    } else {
      for (i = 0; i < SolData[sel].n; i++) {
        if (TimePos(SolData[sel].data[i].time) >= x)
          break;
      }
...
void Plot::MouseMoveObs(int X, int Y, double dx, double dy, double dxs,
                        double dys) {
  QPoint p1, p2, p(X, Y);
  double x, y, xs, ys;
  int i;
...
void Plot::keyPressEvent(QKeyEvent *event) {
  double sfact = 1.05,
         fact = event->modifiers().testFlag(Qt::ShiftModifier) ? 1.0 : 10.0;
...
void Plot::TimerTimer() {
  const QColor color[] = {Qt::red, Qt::gray, QColor(0x00, 0xAA, 0xFF),
                          Qt::green, QColor(0x00, 0xff, 0x00)};
  ...
  if (ConnectState) { // real-time input mode
    for (i = 0; i < 2; i++) {
      ...
      strstatus[i]->setStyleSheet(
          QStringLiteral("QLabel {color %1;}")
              .arg(color2String(color[stat < 3 ? stat + 1 : 3])));
    ...
    if (nmsg[0] <= 0 && nmsg[1] <= 0)
      return;
  } else if (BtnAnimate->isEnabled() &&
             BtnAnimate->isChecked()) { // animation mode
...
void Plot::UpdateTime(void) {
  ...
  if (PlotType <= PLOT_NSAT || PlotType <= PLOT_RES) {
    ...
    if (!(sol = getsol(SolData + sel, SolIndex[sel])))
      return;
    time = sol->time;
  ...
  if (i <= NObs - 1) {
    ...
    if (tt < -DTTOL) {
      for (; i < NObs; i++) {
        if (timediff(Obs.data[IndexObs[i]].time, time) >= -DTTOL)
          break;
      }
    } else if (tt > DTTOL) {
      for (; i >= 0; i--) {
...
void Plot::UpdateOrigin(void) {
  ...
  if (Origin == ORG_STARTPOS) {
    if (!(sol = getsol(SolData, 0)) || sol->type != 0)
      return;
    for (i = 0; i < 3; i++)
      opos[i] = sol->rr[i];
  } else if (Origin == ORG_ENDPOS) {
    if (!(sol = getsol(SolData, SolData[0].n - 1)) || sol->type != 0)
      return;
    for (i = 0; i < 3; i++)
      opos[i] = sol->rr[i];
    ...
    for (i = 0; i < 3; i++)
      opos[i] = PntPos[Origin - ORG_PNTPOS][i];
  }
  if (norm(opos, 3) <= 0.0) {
...
void Plot::UpdateSatSel(void) {
  ...
  for (i = 0; i < MAXSAT; i++) {
    satno2id(i + 1, id);
    SatSel[i] =
        SatListText == "ALL" || SatListText == id || satsys(i + 1, NULL) == sys;
  }
}
...
void Plot::UpdateEnable(void) {
  ...
  BtnSol12->setEnabled(!ConnectState && PlotType <= PLOT_SOLA &&
                       SolData[0].n > 0 && SolData[1].n > 0);
  ...
  ObsType2->setVisible(PlotType == PLOT_SNR || PlotType == PLOT_SNRE ||
                       PlotType == PLOT_MPS);
  ...
  Panel102->setVisible(PlotType == PLOT_SOLP || PlotType == PLOT_SOLV ||
                       ...
                       PlotType == PLOT_RES || PlotType == PLOT_SNR ||
                       PlotType == PLOT_SNRE);
  BtnFitHoriz->setVisible(
      PlotType == PLOT_SOLP || PlotType == PLOT_SOLV || PlotType == PLOT_SOLA ||
  ...
  BtnFixHoriz->setVisible(
      PlotType == PLOT_SOLP || PlotType == PLOT_SOLV || PlotType == PLOT_SOLA ||
      PlotType == PLOT_NSAT || PlotType == PLOT_RES || PlotType == PLOT_OBS ||
      PlotType == PLOT_DOP || PlotType == PLOT_RES || PlotType == PLOT_SNR);
  ...
  BtnShowImg->setVisible(PlotType == PLOT_TRK || PlotType == PLOT_SKY ||
                         PlotType == PLOT_MPS);
  MenuAnimStart->setEnabled(!ConnectState && BtnAnimate->isEnabled() &&
                            !BtnAnimate->isChecked());
  MenuAnimStop->setEnabled(!ConnectState && BtnAnimate->isEnabled() &&
                           BtnAnimate->isChecked());
...
void Plot::FitTime(void) {
  ...
  if (TimeEna[0])
    tl[0] = TimePos(TimeStart);
  if (TimeEna[1])
    tl[1] = TimePos(TimeEnd);
...
void Plot::FitRange(int all) {
  ...
  sol_t *data;
  double xs, ys, xp[2], tl[2], xl[] = {1E8, -1E8}, yl[2] = {1E8, -1E8},
                               zl[2] = {1E8, -1E8};
...
void Plot::LoadOpt(void) {
  ...
  MColor[0][0] =
      settings.value("plot/mcolor0", QColor(0xc0, 0xc0, 0xc0)).value<QColor>();
  MColor[0][1] =
      settings.value("plot/mcolor1", QColor(Qt::green)).value<QColor>();
  MColor[0][2] =
      settings.value("plot/mcolor2", QColor(0x00, 0xAA, 0xFF)).value<QColor>();
  MColor[0][3] =
      settings.value("plot/mcolor3", QColor(0xff, 0x00, 0xff)).value<QColor>();
  MColor[0][4] =
      settings.value("plot/mcolor4", QColor(Qt::blue)).value<QColor>();
  MColor[0][5] =
      settings.value("plot/mcolor5", QColor(Qt::red)).value<QColor>();
  MColor[0][6] =
      settings.value("plot/mcolor6", QColor(0x80, 0x80, 0x00)).value<QColor>();
  MColor[0][7] =
      settings.value("plot/mcolor7", QColor(Qt::gray)).value<QColor>();
  MColor[1][0] =
      settings.value("plot/mcolor8", QColor(0xc0, 0xc0, 0xc0)).value<QColor>();
  MColor[1][1] =
      settings.value("plot/mcolor9", QColor(0x80, 0x40, 0x00)).value<QColor>();
  MColor[1][2] =
      settings.value("plot/mcolor10", QColor(0x00, 0x80, 0x80)).value<QColor>();
  MColor[1][3] =
      settings.value("plot/mcolor11", QColor(0xFF, 0x00, 0x80)).value<QColor>();
  MColor[1][4] =
      settings.value("plot/mcolor12", QColor(0xFF, 0x80, 0x00)).value<QColor>();
  MColor[1][5] =
      settings.value("plot/mcolor13", QColor(0x80, 0x80, 0xFF)).value<QColor>();
  MColor[1][6] =
      settings.value("plot/mcolor14", QColor(0xFF, 0x80, 0x80)).value<QColor>();
  MColor[1][7] =
      settings.value("plot/mcolor15", QColor(Qt::gray)).value<QColor>();
  MapColor[0] = settings.value("plot/mapcolor1", QColor(0xc0, 0xc0, 0xc0))
                    .value<QColor>();
  MapColor[1] = settings.value("plot/mapcolor2", QColor(0xc0, 0xc0, 0xc0))
                    .value<QColor>();
  MapColor[2] = settings.value("plot/mapcolor3", QColor(0xf0, 0xd0, 0xd0))
                    .value<QColor>();
  MapColor[3] = settings.value("plot/mapcolor4", QColor(0xd0, 0xf0, 0xd0))
                    .value<QColor>();
  MapColor[4] = settings.value("plot/mapcolor5", QColor(0xd0, 0xd0, 0xf0))
                    .value<QColor>();
  MapColor[5] = settings.value("plot/mapcolor6", QColor(0xd0, 0xf0, 0xf0))
                    .value<QColor>();
  MapColor[6] = settings.value("plot/mapcolor7", QColor(0xf8, 0xf8, 0xd0))
                    .value<QColor>();
  MapColor[7] = settings.value("plot/mapcolor8", QColor(0xf0, 0xf0, 0xf0))
                    .value<QColor>();
  MapColor[8] = settings.value("plot/mapcolor9", QColor(0xf0, 0xf0, 0xf0))
                    .value<QColor>();
  MapColor[9] = settings.value("plot/mapcolor10", QColor(0xf0, 0xf0, 0xf0))
                    .value<QColor>();
  MapColor[10] = settings.value("plot/mapcolor11", QColor(0xf0, 0xf0, 0xf0))
                     .value<QColor>();
  MapColor[11] = settings.value("plot/mapcolor12", QColor(0xf0, 0xf0, 0xf0))
                     .value<QColor>();
  CColor[0] = settings.value("plot/color1", QColor(Qt::white)).value<QColor>();
  CColor[1] =
      settings.value("plot/color2", QColor(0xc0, 0xc0, 0xc0)).value<QColor>();
  CColor[2] = settings.value("plot/color3", QColor(Qt::black)).value<QColor>();
  CColor[3] =
      settings.value("plot/color4", QColor(0xc0, 0xc0, 0xc0)).value<QColor>();

  plotOptDialog->refDialog->StaPosFile =
      settings.value("plot/staposfile", "").toString();
  plotOptDialog->refDialog->Format =
      settings.value("plot/staposformat", 0).toInt();
  ...
  QcCmd =
      settings.value("plot/qccmd", "teqc +qc +sym +l -rep -plot").toString();
  ...
  for (i = 0; i < 2; i++) {
    StrCmds[0][i] =
        settings.value(QString("str/strcmd1_%1").arg(i), "").toString();
    StrCmds[1][i] =
        settings.value(QString("str/strcmd2_%1").arg(i), "").toString();
    StrCmdEna[0][i] =
        settings.value(QString("str/strcmdena1_%1").arg(i), 0).toInt();
    StrCmdEna[1][i] =
        settings.value(QString("str/strcmdena2_%1").arg(i), 0).toInt();
  }
  for (i = 0; i < 3; i++) {
    StrPaths[0][i] =
        settings.value(QString("str/strpath1_%1").arg(i), "").toString();
    StrPaths[1][i] =
        settings.value(QString("str/strpath2_%1").arg(i), "").toString();
  }
  for (i = 0; i < 10; i++) {
    StrHistory[i] =
        settings.value(QString("str/strhistry_%1").arg(i), "").toString();
    StrMntpHist[i] =
        settings.value(QString("str/strmntphist_%1").arg(i), "").toString();
  }

  TextViewer::Color1 =
      settings.value("viewer/color1", QColor(Qt::black)).value<QColor>();
  TextViewer::Color2 =
      settings.value("viewer/color2", QColor(Qt::white)).value<QColor>();
  TextViewer::FontD.setFamily(
      settings.value("viewer/fontname", "Courier New").toString());
