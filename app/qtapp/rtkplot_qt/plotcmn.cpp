...
//---------------------------------------------------------------------------
const QString PTypes[] = {QT_TR_NOOP("Gnd Trk"),
                          QT_TR_NOOP("Position"),
                          QT_TR_NOOP("Velocity"),
                          QT_TR_NOOP("Accel"),
                          QT_TR_NOOP("NSat"),
                          QT_TR_NOOP("Residuals"),
                          QT_TR_NOOP("Sat Vis"),
                          QT_TR_NOOP("Skyplot"),
                          QT_TR_NOOP("DOP/NSat"),
                          QT_TR_NOOP("SNR/MP/EL"),
                          QT_TR_NOOP("SNR/MP-EL"),
...
// execute command ----------------------------------------------------------
int Plot::ExecCmd(const QString &cmd) { return QProcess::startDetached(cmd); }
...
void Plot::ShowLegend(QString msgs[]) {
  ...
  for (i = 0; i < 7; i++) {
    if (!msgs || msgs[i] == NULL) {
      ...
      ql[i]->adjustSize();
      ql[i]->setStyleSheet(
          QString("QLabel {color: %1;}").arg(color2String(MColor[sel][i + 1])));
    }
  }
  Panel21->updateGeometry();
}
...
void Plot::CalcStats(const double *x, int n, double ref, double &ave,
...
QColor Plot::ObsColor(const obsd_t *obs, double az, double el) {
  ...
  if (SimObs) {
    ...
    for (i = 0; i < NFREQ + NEXOBS; i++) {
      if (!strstr(code2obs(obs->code[i], NULL), code))
        continue;
    ...
    if (obs->L[0] != 0.0 && obs->L[1] != 0.0 && obs->L[2] != 0.0)
      color = MColor[0][4];
    else if (obs->L[0] != 0.0 && obs->L[1] != 0.0)
      color = MColor[0][1];
    else if (obs->L[0] != 0.0 && obs->L[2] != 0.0)
      color = MColor[0][5];
    else if (obs->L[0] != 0.0)
      color = MColor[0][2];
    else if (obs->P[1] != 0.0)
  ...
  if (el < ElMask * D2R ||
      (ElMaskP && el < ElMaskData[static_cast<int>(az * R2D + 0.5)])) {
...
int Plot::SearchPos(int x, int y) {
  ...
  for (i = 0; (data = getsol(SolData + sel, i)) != NULL; i++) {
    if (QFlag->currentIndex() && data->stat != QFlag->currentIndex())
      continue;
...
void Plot::TimeStr(gtime_t time, int n, int tsys, QString &str) {
  ...
  if (TimeLabel == 0) { // www/ssss
    tow = time2gpst(time, &week);
    strcpy(tstr, qPrintable(QString("%1/%2").arg(week, 4).arg(
                     tow, (n > 0 ? 6 : 5) + n, 'f', n)));
