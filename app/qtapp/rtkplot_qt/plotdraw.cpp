...
void Plot::DrawTrk(QPainter &c, int level) {
  ...
  if (norm(OPos, 3) > 0.0) {
    ecef2pos(OPos, opos);
    header =
        "ORI=" + LatLonStr(opos, 9) + QString(" %1m").arg(opos[2], 0, 'f', 4);
  }
  ...
  if (BtnShowTrack->isChecked() && BtnSol1->isChecked()) {
    ...
    if ((sol = getsol(SolData, SolIndex[0])))
      time1 = sol->time;
  ...
  if (BtnShowTrack->isChecked() && BtnSol2->isChecked()) {
    ...
    if ((sol = getsol(SolData + 1, SolIndex[1])))
      time2 = sol->time;
  ...
  for (i = MAXMAPLAYER - 1; i >= 0; i--) {
    ...
    for (data = Gis.data[i]; data; data = data->next) {
      ...
      if (data->type == 1) { // point
        ...
        p = new QPoint[n];
        for (j = m = 0; j < n; j++) {
          PosToXyz(time, (static_cast<gis_poly_t *>(data->data))->pos + j * 3,
                   0, xyz);
          GraphT->ToPoint(xyz[0], xyz[1], p1);
        ...
        for (j = m = 0; j < n; j++) {
          PosToXyz(time,
                   (static_cast<gis_polygon_t *>(data->data))->pos + j * 3, 0,
                   xyz);
          GraphT->ToPoint(xyz[0], xyz[1], p1);
          if (m == 0 || p1.x() != p[m - 1].x() || p1.y() != p[m - 1].y()) {
            p[m++] = p1;
        ...
        for (j = 0, S = 0.0; j < m - 1; j++) {
          S += static_cast<double>(p[j].x() * p[j + 1].y() -
                                   p[j + 1].x() * p[j].y());
        }
...
void Plot::DrawTrkStat(QPainter &c, const TIMEPOS *pos, const QString &header,
                       int p) {
  QString s[6];
  QPoint p1, p2;
...
void Plot::DrawTrkArrow(QPainter &c, const TIMEPOS *pos) {
  ...
  for (i = 1; i < pos->n - 1; i++) {
    ...
    if (vel < 0.5 || fmod(tt + 0.005, INTARROW) >= 0.01)
      continue;

    GraphT->ToPoint(pos->x[i], pos->y[i], p);
    p.rx() -= static_cast<int>(off * d[1] / dist);
    p.ry() -= static_cast<int>(off * d[0] / dist);
    DrawMark(GraphT, c, p, 10, CColor[3], 15,
             static_cast<int>(ATAN2(d[1], d[0]) * R2D));
  }
}
...
void Plot::DrawTrkVel(QPainter &c, const TIMEPOS *vel) {
  ...
  p1.ry() -= SIZE_VELC / 2;
  if (v >= 1.0)
    DrawMark(GraphT, c, p1, 10, CColor[2], SIZE_VELC,
             90 - static_cast<int>(dir));
...
void Plot::DrawSol(QPainter &c, int level, int type) {
  ...
  if (BtnShowTrack->isChecked() &&
      (BtnFixHoriz->isChecked() || BtnFixVert->isChecked())) {
  ...
  if (BtnShowTrack->isChecked() &&
    ...
    for (j = k = 0; j < 3 && pos->n > 0; j++) {
      ...
      if (BtnSol2->isChecked() && pos2->n > 0 &&
        ...
        GraphG[j]->DrawMark(c, xl[0], y, 0, MColor[1][pos->q[0]], MarkSize * 2,
                            0);
        if (BtnSol1->isChecked() && pos1->n > 0 &&
            GraphG[j]->ToPoint(xl[0], y, p1)) {
          p1.rx() += MarkSize + 4;
      ...
      if (BtnSol1->isChecked() && pos1->n > 0) {
        ...
        GraphG[j]->DrawMark(c, xl[0], y, 1, CColor[2], MarkSize * 2 + 2, 0);
        GraphG[j]->DrawMark(c, xl[0], y, 0, MColor[0][pos->q[0]], MarkSize * 2,
                            0);
        if (BtnSol2->isChecked() && pos2->n > 0 &&
            GraphG[j]->ToPoint(xl[0], y, p1)) {
          p1.rx() += MarkSize + 4;
...
void Plot::DrawSolPnt(QPainter &c, const TIMEPOS *pos, int level, int style) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (level && ShowErr && PlotType <= PLOT_SOLA && PlotStyle < 2) {
      ...
      if (ShowErr == 1) {
        for (j = 0; j < pos->n; j++) {
          GraphG[i]->DrawMark(c, x[j], y[j], 12, CColor[1],
                              static_cast<int>(SQRT(s[j]) * 2.0 / ys), 0);
        }
      } else {
        yy = new double[pos->n];
    ...
    if (level && PlotStyle < 2) {
      QVector<QColor> color;
      for (j = 0; j < pos->n; j++)
        color.append(MColor[style][pos->q[j]]);
...
void Plot::DrawSolStat(QPainter &c, const TIMEPOS *pos, const QString &unit,
                       int p) {
  QPushButton *btn[] = {BtnOn1, BtnOn2, BtnOn3};
  QPoint p1, p2;
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (j == 0 && p == 0) {
      ...
      if (norm(OPos, 3) > 0.0) {
        ecef2pos(OPos, opos);
        label = "ORI=" + LatLonStr(opos, 9) +
                QString(" %1m").arg(opos[2], 0, 'f', 4);
...
void Plot::DrawNsat(QPainter &c, int level) {
  ...
  if (BtnShowTrack->isChecked() &&
    ...
    for (j = k = 0; j < 3 && ns->n > 0; j++) {
      ...
      GraphG[j]->DrawMark(c, xl[0], y, 1, CColor[2], MarkSize * 2 + 6, 0);
      GraphG[j]->DrawMark(c, xl[0], y, 1, CColor[2], MarkSize * 2 + 2, 0);
      GraphG[j]->DrawMark(c, xl[0], y, 0, MColor[sel][ns->q[0]], MarkSize * 2,
                          0);
...
void Plot::DrawObs(QPainter &c, int level) {
  ...
  gtime_t time;
  obsd_t *obs;
  double xs, ys, xt, xl[2], yl[2], tt[MAXSAT] = {0}, xp, xc, yc,
                                   yp[MAXSAT] = {0};
  ...
  if (level && PlotStyle <= 2) {
    ...
    for (i = 0; i < Obs.n; i++) {
      ...
      if (fabs(xt - tt[obs->sat - 1]) / xs > 0.9) {
        GraphR->DrawMark(c, xt, yp[obs->sat - 1], 0,
                         PlotStyle < 2 ? col : CColor[3],
...
void Plot::DrawObsSlip(QPainter &c, double *yp) {
  ...
  for (i = 0; i < Obs.n; i++) {
    if (El[i] < ElMask * D2R)
      continue;
    if (ElMaskP && El[i] < ElMaskData[static_cast<int>(Az[i] * R2D + 0.5)])
      continue;
    obs = &Obs.data[i];
    if (!SatSel[obs->sat - 1])
      continue;
...
void Plot::DrawObsEphem(QPainter &c, double *yp) {
  ...
  for (i = 0; i < MAXSAT; i++) {
    ...
    for (j = 0; j < Nav.n; j++) {
      ...
      for (k = 0; k < 3; k++)
        ps[k].ry() += MarkSize + 2 + off[Nav.eph[j].sat - 1];
      ps[0].ry() -= 2;
      ...
      if (in)
        GraphR->DrawMark(c, ps[2], 0, svh ? MColor[0][5] : CColor[1],
                         svh ? 4 : 3, 0);
    }
    for (j = 0; j < Nav.ng; j++) {
      if (Nav.geph[j].sat != i + 1)
        continue;
      ...
      for (k = 0; k < 3; k++)
        ps[k].ry() += MarkSize + 2 + off[Nav.geph[j].sat - 1];
      ps[0].ry() -= 2;

      GraphR->DrawPoly(c, ps, 3, Nav.geph[j].svh ? MColor[0][5] : CColor[1], 0);

      if (in)
        GraphR->DrawMark(c, ps[2], 0,
                         Nav.geph[j].svh ? MColor[0][5] : CColor[1],
                         Nav.geph[j].svh ? 4 : 3, 0);
    }
    for (j = 0; j < Nav.ns; j++) {
      if (Nav.seph[j].sat != i + 1)
        continue;
      ...
      for (k = 0; k < 3; k++)
        ps[k].ry() += MarkSize + 2 + off[Nav.seph[j].sat - 1];
      ps[0].ry() -= 2;
      ...
      if (in)
        GraphR->DrawMark(c, ps[2], 0,
                         Nav.seph[j].svh ? MColor[0][5] : CColor[1],
...
void Plot::DrawSky(QPainter &c, int level) {
  ...
  if (PlotStyle <= 2) {
    for (i = 0; i < Obs.n; i++) {
      obs = &Obs.data[i];
      if (SatMask[obs->sat - 1] || !SatSel[obs->sat - 1] || El[i] <= 0.0)
        continue;
      QColor col = ObsColor(obs, Az[i], El[i]);
      if (col == Qt::black)
        continue;
  ...
  // show statistics
  if (ShowStats && BtnShowTrack->isChecked() && 0 <= ind && ind < NObs &&
      !SimObs) {
    ...
    for (i = IndexObs[ind]; i < Obs.n && i < IndexObs[ind + 1]; i++) {
      obs = &Obs.data[i];
      if (SatMask[obs->sat - 1] || !SatSel[obs->sat - 1])
        continue;
      if (HideLowSat && El[i] < ElMask * D2R)
        continue;
      if (HideLowSat && ElMaskP &&
          El[i] < ElMaskData[static_cast<int>(Az[i] * R2D + 0.5)])
        continue;
      ...
      if (!*code) {
        ...
        s += " : ";
        for (j = 0; j < NFREQ; j++)
          s += QString("%1 ").arg(obs->SNR[j] * 0.25, 2, 'f', 0, QChar('0'));
        s += ": ";
        for (j = 0; j < NFREQ; j++)
          s += QString("%1").arg(obs->LLI[j]);
      } else {
        for (j = 0; j < NFREQ + NEXOBS; j++) {
          if (strstr(code2obs(obs->code[j], NULL), code))
            break;
        }
        if (j >= NFREQ + NEXOBS)
          continue;
        ...
        s += QString("%1%2%3 : %4 : %5 : %6")
                 ...
                 .arg(obs->L[j] == 0.0 ? "-" : "L")
                 .arg(obs->D[j] == 0.0 ? "-" : "D")
                 .arg(obs->SNR[j] * 0.25, 4, 'f', 1, QChar('0'))
                 .arg(obs->LLI[j])
                 .arg(code2obs(obs->code[j], NULL));
      }
      QColor col = ObsColor(obs, Az[i], El[i]);
      p2.ry() += hh;
      GraphS->DrawText(c, p2, s, col == Qt::black ? MColor[0][0] : col, 2, 2,
                       0);
    }
  }
...
void Plot::DrawDop(QPainter &c, int level) {
  ...
  p1.setY((p1.y() + p2.y()) / 2);
  if (doptype == 0) {
    label = QString("# OF SATELLITES / DOP (EL>=%1%2)")
                .arg(ElMask, 0, 'f', 0)
                .arg(degreeChar);
  } else if (doptype == 1) {
    label = QString("# OF SATELLITES (EL>=%1%2)")
                .arg(ElMask, 0, 'f', 0)
                .arg(degreeChar);
  } else {
  ...
  for (i = 0; i < NObs; i++) {
    ...
    for (j = IndexObs[i]; j < Obs.n && j < IndexObs[i + 1]; j++) {
      if (SatMask[Obs.data[j].sat - 1] || !SatSel[Obs.data[j].sat - 1])
        continue;
      if (El[j] < ElMask * D2R)
        continue;
      if (ElMaskP && El[j] < ElMaskData[static_cast<int>(Az[j] * R2D + 0.5)])
        continue;
      azel[ns[n] * 2] = Az[j];
      azel[1 + ns[n] * 2] = El[j];
      ns[n]++;
  ...
  if (BtnShowTrack->isChecked() && 0 <= ind && ind < NObs) {
    ...
    ns[0] = 0;
    for (i = IndexObs[ind]; i < Obs.n && i < IndexObs[ind + 1]; i++) {
      if (SatMask[Obs.data[i].sat - 1] || !SatSel[Obs.data[i].sat - 1])
        continue;
      if (El[i] < ElMask * D2R)
        continue;
      if (ElMaskP && El[i] < ElMaskData[static_cast<int>(Az[i] * R2D + 0.5)])
    ...
    for (i = 0; i < 4; i++) {
      if ((doptype != 0 && doptype != i + 2) || dop[i] <= 0.0)
        continue;
      GraphR->DrawMark(c, xl[0], dop[i], 0, MColor[0][i + 2], MarkSize * 2 + 2,
                       0);
    }
    if (doptype == 0 || doptype == 1) {
...
void Plot::DrawDopStat(QPainter &c, double *dop, int *ns, int n) {
  ...
  double ave[4] = {0};
  int i, j, m = 0;
  int ndop[4] = {0}, nsat[MAXOBS] = {0},
      fonth = static_cast<int>(Disp->font().pointSize() * 1.5);
  ...
  for (i = 0; i < 4; i++) {
    for (j = 0; j < n; j++) {
      if (dop[i + j * 4] <= 0.0 || dop[i + j * 4] > MaxDop)
        continue;
      ave[i] += dop[i + j * 4];
      ndop[i]++;
    }
  ...
  for (i = 0; i < m;
       i++, p1.ry() += fonth, p2.ry() += fonth, p3.ry() += fonth) {
...
void Plot::DrawSnr(QPainter &c, int level) {
  ...
  gtime_t time = {0, 0};
  double *x, *y, xl[2], yl[2], off, xc, yc, xp, yp[MAXSAT], ave[3] = {0},
                                                            rms[3] = {0};
  char code[32];
  ...
  if (NObs > 0 && BtnSol1->isChecked()) {
    x = new double[NObs], y = new double[NObs];
    col = new QColor[NObs];
    ...
    for (i = l = 0; i < 3; i++) {
      ...
      for (sat = 1, np = 0; sat <= MAXSAT; sat++) {
        if (SatMask[sat - 1] || !SatSel[sat - 1])
          continue;

        for (j = n = 0; j < Obs.n; j++) {
          if (Obs.data[j].sat != sat)
            continue;

          for (k = 0; k < NFREQ + NEXOBS; k++) {
            if (strstr(code2obs(Obs.data[j].code[k], NULL), code))
              break;
          }
          if (k >= NFREQ + NEXOBS)
            continue;
          ...
          if (i == 0) {
            ...
            y[n] = El[j] * R2D;
            if (SimObs)
              col[n] = SysColor(Obs.data[j].sat);
            else
              col[n] = SnrColor(Obs.data[j].SNR[k] * 0.25);
            if (El[j] > 0.0 && El[j] < ElMask * D2R)
              col[n] = MColor[0][0];
          }
      ...
      if (level && i == 1 && nrms[i] > 0 && ShowStats &&
          !BtnShowTrack->isChecked()) {
        ave[i] = ave[i] / nrms[i];
      ...
      if (BtnShowTrack->isChecked() && 0 <= ind && ind < NObs &&
        ...
        for (k = 0; k < np; k++) {
          if (i != 1 && yp[k] <= 0.0)
            continue;
          GraphG[i]->DrawMark(c, xl[0], yp[k], 0, CColor[0], MarkSize * 2 + 4,
                              0);
          GraphG[i]->DrawMark(c, xl[0], yp[k], 0, colp[k], MarkSize * 2 + 2, 0);
        }
        if (np <= 0 || np > 1 || (i != 1 && yp[0] <= 0.0))
          continue;
        ...
        DrawLabel(
            GraphG[i], c, p1,
            QString("%1 %2").arg(yp[0], 0, 'f', i == 1 ? 4 : 1).arg(unit[i]), 2,
            2);
      }
    }
    delete[] x;
  ...
  for (i = 0; i < 3; i++) {
    ...
    p1.rx() += 5;
    p1.ry() += 3;
    DrawLabel(GraphG[i], c, p1, QString("%1 (%2)").arg(label[i]).arg(unit[i]),
              1, 2);
  }
}
...
void Plot::DrawSnrE(QPainter &c, int level) {
  ...
  gtime_t time = {0, 0};
  double *x[2], *y[2], xl[2] = {-0.001, 90.0},
                       yl[2][2] = {{10.0, 65.0}, {-10.0, 10.0}};
  double xp[2][MAXSAT], yp[2][MAXSAT], ave = 0.0, rms = 0.0;
  char code[32];
  int i, j, k, n[2], np[2] = {0}, sat, ind = ObsIndex,
                     hh = static_cast<int>(Disp->font().pointSize() * 1.5);
  int nrms = 0;
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (i == j) {
      p2.rx() -= 8;
      p2.ry() -= 6;
      GraphE[i]->DrawText(c, p2, tr("Elevation ( %1 )").arg(degreeChar),
                          CColor[2], 2, 1, 0);
    }
  }
  if (0 <= ind && ind < NObs && BtnShowTrack->isChecked()) {
      ...
      for (j = n[0] = n[1] = 0; j < Obs.n; j++) {
        ...
        if (y[0][n[0]] > 0.0) {
          if (timediff(time, Obs.data[j].time) == 0.0) {
            ...
            if (np[0] < MAXSAT && colp[0][np[0]] != Qt::black)
              np[0]++;
          }
          if (n[0] < NObs)
            n[0]++;
        ...
        if (y[1][n[1]] != 0.0) {
          ...
          if (timediff(time, Obs.data[j].time) == 0.0) {
            ...
            if (np[1] < MAXSAT && colp[1][np[1]] != Qt::black)
              np[1]++;
          }
          if (n[1] < NObs)
            n[1]++;
    ...
    if (BtnShowTrack->isChecked() && 0 <= ind && ind < NObs &&
      ...
      for (i = 0; i < 2; i++) {
        if (!btn[i]->isChecked())
          continue;
        for (j = 0; j < np[i]; j++) {
          GraphE[i]->DrawMark(c, xp[i][j], yp[i][j], 0, CColor[0],
                              MarkSize * 2 + 8, 0);
          GraphE[i]->DrawMark(c, xp[i][j], yp[i][j], 1, CColor[2],
                              MarkSize * 2 + 6, 0);
          GraphE[i]->DrawMark(c, xp[i][j], yp[i][j], 0, colp[i][j],
                              MarkSize * 2 + 2, 0);
        }
      }
    }
  ...
  if (ShowStats) {
    ...
    if (btn[1]->isChecked() && nrms > 0 && !BtnShowTrack->isChecked()) {
      ...
      DrawLabel(
          GraphE[1], c, p1,
          QString("AVE=%1m RMS=%2m").arg(ave, 0, 'f', 4).arg(rms, 0, 'f', 4), 2,
          2);
    }
  }
}
...
void Plot::DrawMpS(QPainter &c, int level) {
  ...
  if (!BtnSol1->isChecked() || NObs <= 0 || SimObs)
    return;
  ...
  for (sat = 1; sat <= MAXSAT; sat++) {
    ...
    for (i = 0; i < Obs.n; i++) {
      ...
      for (j = 0; j < NFREQ + NEXOBS; j++) {
        if (strstr(code2obs(Obs.data[i].code[j], NULL), code))
          break;
      }
      if (j >= NFREQ + NEXOBS)
        continue;
  ...
  if (BtnShowTrack->isChecked() && 0 <= ind && ind < NObs) {
    ...
    for (i = IndexObs[ind]; i < Obs.n && i < IndexObs[ind + 1]; i++) {
      ...
      satno2id(obs->sat, id);
      GraphS->DrawMark(c, x, y, 0, col, Disp->font().pointSize() * 2 + 5, 0);
      GraphS->DrawMark(c, x, y, 1, CColor[2], Disp->font().pointSize() * 2 + 5,
                       0);
      GraphS->DrawText(c, x, y, s = id, CColor[0], 0, 0, 0);
    }
  }
...
void Plot::DrawRes(QPainter &c, int level) {
  QString label[] = {tr("Pseudorange Residuals (m)"),
  ...
  int i, j, sel = !BtnSol1->isChecked() && BtnSol2->isChecked() ? 1 : 0,
            ind = SolIndex[sel];
  int frq = FrqType->currentIndex() + 1;

  trace(3, "DrawRes: level=%d\n", level);

  if (0 <= ind && ind < SolData[sel].n && BtnShowTrack->isChecked() &&
      BtnFixHoriz->isChecked()) {
  ...
  if (n > 0 && ((sel == 0 && BtnSol1->isChecked()) ||
                (sel == 1 && BtnSol2->isChecked()))) {

    q = new int[n];
    s = new int[n];
    x = new double[n], y[0] = new double[n];
    y[1] = new double[n];
    y[2] = new double[n];
    y[3] = new double[n];
    ...
    for (int sat = 1; sat <= MAXSAT; sat++) {
      ...
      for (int i = 0; i < n; i++) {
        ...
        if (p->sat != sat || p->frq != frq)
          continue;
        if (p->resp == 0.0 && p->resc == 0.0)
          continue;
        x[m] = TimePos(p->time);
        y[0][m] = p->resp;
        y[1][m] = p->resc;
      ...
      for (int i = 0; i < 3; i++) {
        ...
        if (!level || !(PlotStyle % 2)) {
          ...
          if (i == 2)
            DrawPolyS(GraphG[i], c, x, y[3], m, CColor[3], 0);
        }
        if (level && PlotStyle < 2) {
          QColor color;
          for (int j = 0; j < m; j++) {
            ...
            if (i == 2)
              GraphG[i]->DrawMark(c, x[j], y[3][j], 0, MColor[0][4], MarkSize,
                                  0);
          }
        }
        if (level && i == 1) { /* slip */
    ...
    if (ShowStats) {
      for (int i = 0; i < 2; i++) {
        ...
        std = ns[i] <= 1
                  ? 0.0
                  : SQRT((sum2[i] - 2.0 * sum[i] * ave + ns[i] * ave * ave) /
                         (ns[i] - 1));
...
void Plot::DrawPolyS(Graph *graph, QPainter &c, double *x, double *y, int n,
  ...
  for (i = 0; i < n; i = j) {
    for (j = i + 1; j < n; j++)
      if (fabs(x[j] - x[j - 1]) > TBRK)
        break;
    graph->DrawPoly(c, x + i, y + i, j - i, color, style);
  }
}
// draw label with hemming --------------------------------------------------
void Plot::DrawLabel(Graph *g, QPainter &c, const QPoint &p,
                     const QString &label, int ha, int va) {
  g->DrawText(c, p, label, CColor[2], CColor[0], ha, va, 0);
}
// draw mark with hemming ---------------------------------------------------
void Plot::DrawMark(Graph *g, QPainter &c, const QPoint &p, int mark,
                    const QColor &color, int size, int rot) {
