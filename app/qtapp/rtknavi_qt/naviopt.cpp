...
#define FMTOPT                                                                 \
  ...
  "javad,10:nvs,11:binex,12:rt17,13:sbf,14:cmr,17:sp3"
...
OptDialog::OptDialog(QWidget *parent) : QDialog(parent) {
  QString label;
  int freq[] = {1, 2, 5, 6, 7, 8, 9}, nglo = MAXPRNGLO, ngal = MAXPRNGAL,
      nqzs = MAXPRNQZS;
  ...
  connect(BtnAntPcvFile, SIGNAL(clicked(bool)), this,
          SLOT(BtnAntPcvFileClick()));
  connect(BtnAntPcvView, SIGNAL(clicked(bool)), this,
          SLOT(BtnAntPcvViewClick()));
  ...
  connect(BtnGeoidDataFile, SIGNAL(clicked(bool)), this,
          SLOT(BtnGeoidDataFileClick()));
  ...
  connect(BtnSatPcvFile, SIGNAL(clicked(bool)), this,
          SLOT(BtnSatPcvFileClick()));
  connect(BtnSatPcvView, SIGNAL(clicked(bool)), this,
          SLOT(BtnSatPcvViewClick()));
  ...
  connect(BtnStaPosFile, SIGNAL(clicked(bool)), this,
          SLOT(BtnStaPosFileClick()));
  connect(BtnStaPosView, SIGNAL(clicked(bool)), this,
          SLOT(BtnStaPosViewClick()));
  connect(BtnTLEFile, SIGNAL(clicked(bool)), this, SLOT(BtnTLEFileClick()));
  connect(BtnTLESatFile, SIGNAL(clicked(bool)), this,
          SLOT(BtnTLESatFileClick()));
  connect(PosMode, SIGNAL(currentIndexChanged(int)), this,
          SLOT(PosModeChange(int)));
  connect(SolFormat, SIGNAL(currentIndexChanged(int)), this,
          SLOT(SolFormatChange(int)));
  connect(RefPosTypeP, SIGNAL(currentIndexChanged(int)), this,
          SLOT(RefPosTypePChange(int)));
  connect(RovPosTypeP, SIGNAL(currentIndexChanged(int)), this,
          SLOT(RovPosTypePChange(int)));
  connect(AmbRes, SIGNAL(currentIndexChanged(int)), this,
          SLOT(AmbResChange(int)));
  ...
  connect(OutputHeight, SIGNAL(currentIndexChanged(int)), this,
          SLOT(OutputHeightClick()));
  connect(NavSys2, SIGNAL(clicked(bool)), this, SLOT(NavSys2Click()));
  connect(NavSys6, SIGNAL(clicked(bool)), this, SLOT(NavSys6Click()));
  connect(BaselineConst, SIGNAL(clicked(bool)), this,
          SLOT(BaselineConstClick()));
}
//---------------------------------------------------------------------------
...
void OptDialog::BtnLoadClick() {
  ...
  fileName = QDir::toNativeSeparators(
      QFileDialog::getOpenFileName(this, tr("Load Options..."), QString(),
                                   tr("Options File (*.conf);;All (*.*)")));
...
void OptDialog::BtnSaveClick() {
  ...
  file = QDir::toNativeSeparators(
      QFileDialog::getOpenFileName(this, tr("Save Options..."), QString(),
                                   tr("Options File (*.conf);;All (*.*)")));
...
void OptDialog::BtnStaPosFileClick() {
  ...
  fileName = QDir::toNativeSeparators(
      QFileDialog::getOpenFileName(this, tr("Station Postion File"), QString(),
                                   tr("Position File (*.pos);;All (*.*)")));
...
void OptDialog::BtnGeoidDataFileClick() {
  QString fileName = QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("Geoid Data File"), QString(), tr("All (*.*)")));
  GeoidDataFile->setText(fileName);
}
//---------------------------------------------------------------------------
void OptDialog::BtnDCBFileClick() {
  QString fileName = QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("DCB Data File"), QString(), tr("DCB (*.dcb)")));
  DCBFile->setText(fileName);
}
//---------------------------------------------------------------------------
void OptDialog::BtnEOPFileClick() {
  QString fileName = QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("EOP Data File"), QString(), tr("EOP (*.erp)")));
...
void OptDialog::BtnLocalDirClick() {
  ...
  dir = QDir::toNativeSeparators(QFileDialog::getExistingDirectory(
      this, tr("FTP/HTTP Local Directory"), dir));
...
void OptDialog::BtnFontClick() {
  ...
  FontLabel->setText(FontLabel->font().family() +
                     QString::number(FontLabel->font().pointSize()) + " pt");
}
//---------------------------------------------------------------------------
void OptDialog::BtnTLESatFileClick() {
  QString fileName = QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("TLE Satellite Number File"), QString(), tr("All (*.*)")));
...
void OptDialog::BtnTLEFileClick() {
  QString fileName = QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("TLE Data File"), QString(), tr("All (*.*)")));
...
void OptDialog::GetOpt(void) {
  ...
  ElMask->setCurrentIndex(
      ElMask->findText(QString::number(PrcOpt.elmin * R2D)));
...
void OptDialog::SetOpt(void) {
  ...
  SolOpt.times =
      TimeFormat->currentIndex() == 0 ? 0 : TimeFormat->currentIndex() - 1;
...
void OptDialog::LoadOpt(const QString &file) {
  ...
  if (!loadopts(qPrintable(file), sysopts) ||
      !loadopts(qPrintable(file), rcvopts))
    return;
  ...
  for (int i = 0; i < 8; i++) {
    ...
    for (int j = 0; j < (i < 3 ? 7 : 5); j++) {
      if (strtype[i] != (i < 3 ? itype[j] : otype[j]))
        continue;
      mainForm->Stream[i] = j;
      break;
    }
    if (i < 5)
      mainForm->Format[i] = strfmt[i];
  ...
  ElMask->setCurrentIndex(
      ElMask->findText(QString::number(prcopt.elmin * R2D, 'f', 0)));
  DynamicModel->setCurrentIndex(prcopt.dynamics);
  TideCorr->setCurrentIndex(prcopt.tidecorr);
  IonoOpt->setCurrentIndex(prcopt.ionoopt);
  TropOpt->setCurrentIndex(prcopt.tropopt);
  ...
  for (sat = 1; sat <= MAXSAT; sat++) {
    if (!prcopt.exsats[sat - 1])
      continue;
    satno2id(sat, id);
    buff += QString(buff.isEmpty() ? "" : " ") +
            (prcopt.exsats[sat - 1] == 2 ? "+" : "") + id;
  }
...
void OptDialog::SaveOpt(const QString &file) {
  ...
  QString FieldSep_Text = FieldSep->text();
  QString RovAnt_Text = RovAnt->currentText(),
  ...
  for (int i = 0; i < 8; i++) {
    strtype[i] =
        i < 3 ? itype[mainForm->Stream[i]] : otype[mainForm->Stream[i]];
  ...
  else if (RefPosTypeP->currentIndex() == 5)
    prcopt.refpos = POSOPT_SINGLE;

  if (prcopt.rovpos == POSOPT_POS)
    GetPos(RovPosTypeP->currentIndex(), editu, prcopt.ru);
  if (prcopt.refpos == POSOPT_POS)
...
void OptDialog::UpdateEnable(void) {
  bool rel = PMODE_DGPS <= PosMode->currentIndex() &&
             PosMode->currentIndex() <= PMODE_FIXED;
  bool rtk = PMODE_KINEMA <= PosMode->currentIndex() &&
             PosMode->currentIndex() <= PMODE_FIXED;
  ...
  GloAmbRes->setEnabled(ar && AmbRes->currentIndex() > 0 &&
                        NavSys2->isChecked());
  BdsAmbRes->setEnabled(ar && AmbRes->currentIndex() > 0 &&
                        NavSys6->isChecked());
  ValidThresAR->setEnabled(ar && AmbRes->currentIndex() >= 1 &&
                           AmbRes->currentIndex() < 4);
  ...
  BaselineLen->setEnabled(BaselineConst->isChecked() &&
                          PosMode->currentIndex() == PMODE_MOVEB);
  BaselineSig->setEnabled(BaselineConst->isChecked() &&
                          PosMode->currentIndex() == PMODE_MOVEB);
  ...
  OutputGeoid->setEnabled(SolFormat->currentIndex() == 0 &&
                          OutputHeight->currentIndex() == 1);
  ...
  RovPos1->setEnabled(RovPosTypeP->isEnabled() &&
                      RovPosTypeP->currentIndex() <= 2);
  RovPos2->setEnabled(RovPosTypeP->isEnabled() &&
                      RovPosTypeP->currentIndex() <= 2);
  RovPos3->setEnabled(RovPosTypeP->isEnabled() &&
                      RovPosTypeP->currentIndex() <= 2);
  BtnRovPos->setEnabled(RovPosTypeP->isEnabled() &&
                        RovPosTypeP->currentIndex() <= 2);

  RefPosTypeP->setEnabled(rel && PosMode->currentIndex() != PMODE_MOVEB);
  RefPos1->setEnabled(RefPosTypeP->isEnabled() &&
                      RefPosTypeP->currentIndex() <= 2);
  RefPos2->setEnabled(RefPosTypeP->isEnabled() &&
                      RefPosTypeP->currentIndex() <= 2);
  RefPos3->setEnabled(RefPosTypeP->isEnabled() &&
                      RefPosTypeP->currentIndex() <= 2);
  BtnRefPos->setEnabled(RefPosTypeP->isEnabled() &&
                        RefPosTypeP->currentIndex() <= 2);
...
void OptDialog::GetPos(int type, QLineEdit **edit, double *pos) {
  ...
  if (type == 1) { /* lat/lon/height dms/m */
    ...
    p[0] = (dms1[0] < 0 ? -1 : 1) *
           (fabs(dms1[0]) + dms1[1] / 60 + dms1[2] / 3600) * D2R;
    p[1] = (dms2[0] < 0 ? -1 : 1) *
           (fabs(dms2[0]) + dms2[1] / 60 + dms2[2] / 3600) * D2R;
