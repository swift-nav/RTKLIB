...
InputStrDialog::InputStrDialog(QWidget *parent) : QDialog(parent) {
  ...
  connect(Stream1, SIGNAL(currentIndexChanged(int)), this,
          SLOT(Stream1Change(int)));
  connect(Stream2, SIGNAL(currentIndexChanged(int)), this,
          SLOT(Stream2Change(int)));
  connect(Stream3, SIGNAL(currentIndexChanged(int)), this,
          SLOT(Stream3Change(int)));
  connect(NmeaReqL, SIGNAL(currentIndexChanged(int)), this,
          SLOT(NmeaReqLChange(int)));
...
void InputStrDialog::showEvent(QShowEvent *event) {
  ...
  Format2->setCurrentIndex(Format[1] < NRcv ? Format[1]
                                            : NRcv + Format[1] - STRFMT_SP3);
  Format3->setCurrentIndex(Format[2] < NRcv ? Format[2]
                                            : NRcv + Format[2] - STRFMT_SP3);
...
void InputStrDialog::BtnOkClick() {
  ...
  Format[1] = Format2->currentIndex() < NRcv
                  ? Format2->currentIndex()
                  : STRFMT_SP3 + Format2->currentIndex() - NRcv;
  Format[2] = Format3->currentIndex() < NRcv
                  ? Format3->currentIndex()
