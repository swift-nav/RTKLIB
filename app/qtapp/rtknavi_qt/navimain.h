...
private:
  ...
  void UpdateLog(int stat, gtime_t time, double *rr, float *qr, double *rb,
  ...
  void DrawArrow(QPainter *c, int x, int y, int siz, int ang,
                 const QColor &color);
...
public:
  ...
  int NmeaReq, NmeaCycle, InTimeTag, OutTimeTag, OutAppend, LogTimeTag,
      LogAppend;
