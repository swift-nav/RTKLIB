...
int MainWindow::ExecCmd(const QString &cmd, int show) {
  ...
  return QProcess::startDetached(
      cmd); /* FIXME: show option not yet supported */
}
// constructor --------------------------------------------------------------
...
void MainWindow::showEvent(QShowEvent *event) {
  ...
  MenuStartAction =
      trayMenu->addAction(tr("Start"), this, SLOT(MenuStartClick()));
  ...
  connect(systemTray, SIGNAL(activated(QSystemTrayIcon::ActivationReason)),
          this, SLOT(TrayIconClick(QSystemTrayIcon::ActivationReason)));
...
void MainWindow::BtnPlotClick() {
  ...
  cmd = QString("rtkplot_qt -p tcpcli://localhost:%1 -t \"%2 %3\"")
            .arg(OpenPort)
            .arg(windowTitle())
            .arg(": RTKPLOT");
  if (!ExecCmd(cmd, 1)) {
...
void MainWindow::BtnOptClick() {
  ...
  if (optDialog->result() != QDialog::Accepted)
    return;
  ...
  if (MoniPort != optDialog->MoniPort)
    chgmoni = 1;
...
void MainWindow::BtnInputStrClick() {
  ...
  for (i = 0; i < 3; i++) {
    ...
    for (j = 0; j < 4; j++)
      inputStrDialog->Paths[i][j] = Paths[i][j];
  }
  for (i = 0; i < 3; i++)
    for (j = 0; j < 2; j++) {
  ...
  if (inputStrDialog->result() != QDialog::Accepted)
    return;
  ...
  for (i = 0; i < 3; i++) {
    ...
    for (j = 0; j < 4; j++)
      Paths[i][j] = inputStrDialog->Paths[i][j];
  }
  for (i = 0; i < 3; i++)
    for (j = 0; j < 2; j++) {
...
int MainWindow::ConfOverwrite(const QString &path) {
  ...
  if (!QFile::exists(buff1))
    return 1; // file not exists

  // check overwrite input files
  for (i = 0; i < 3; i++) {
    if (!StreamC[i] || itype[Stream[i]] != STR_FILE)
      continue;
  ...
  return QMessageBox::question(this, tr("File exists"), buff1) ==
         QMessageBox::Yes;
}
...
void MainWindow::BtnOutputStrClick() {
  ...
  for (i = 3; i < 5; i++) {
    ...
    for (j = 0; j < 4; j++)
      outputStrDialog->Paths[i - 3][j] = Paths[i][j];
  }
  for (i = 0; i < 10; i++) {
  ...
  if (outputStrDialog->result() != QDialog::Accepted)
    return;
  ...
  for (i = 3; i < 5; i++) {
    ...
    Format[i] = outputStrDialog->Format[i - 3];
    for (j = 0; j < 4; j++)
      Paths[i][j] = outputStrDialog->Paths[i - 3][j];
  }
  for (i = 0; i < 10; i++) {
  ...
  for (i = 3; i < 5; i++) {
    ...
    str = otype[Stream[i]];
    if (str == STR_SERIAL)
      strncpy(path, qPrintable(Paths[i][0]), 1024);
    else if (str == STR_FILE)
      strncpy(path, qPrintable(Paths[i][2]), 1024);
    else if (str == STR_FTP || str == STR_HTTP)
      strncpy(path, qPrintable(Paths[i][3]), 1024);
    else
      strncpy(path, qPrintable(Paths[i][1]), 1024);
...
void MainWindow::BtnLogStrClick() {
  ...
  for (i = 5; i < 8; i++) {
    ...
    for (j = 0; j < 4; j++)
      logStrDialog->Paths[i - 5][j] = Paths[i][j];
  }
  for (i = 0; i < 10; i++) {
  ...
  if (logStrDialog->result() != QDialog::Accepted)
    return;
  ...
  for (i = 5; i < 8; i++) {
    ...
    for (j = 0; j < 4; j++)
      Paths[i][j] = logStrDialog->Paths[i - 5][j];
  }
  for (i = 0; i < 10; i++) {
...
void MainWindow::SvrStart(void) {
  ...
  if ((RovAntPcvF || RefAntPcvF) && !readpcv(qPrintable(AntPcvFileF), &pcvr)) {
    Message->setText(
        QString(tr("rcv ant file read error %1")).arg(AntPcvFileF));
    return;
  }
  if (RovAntPcvF) {
    if ((pcv = searchpcv(0, qPrintable(RovAntF), time, &pcvr))) {
      ...
      Message->setText(
          QString(tr("no antenna pcv %1")).arg(qPrintable(RovAntF)));
    }
    for (i = 0; i < 3; i++)
      PrcOpt.antdel[0][i] = RovAntDel[i];
  }
  if (RefAntPcvF) {
    if ((pcv = searchpcv(0, qPrintable(RefAntF), time, &pcvr))) {
      ...
      Message->setText(
          QString(tr("no antenna pcv %1")).arg(qPrintable(RefAntF)));
    }
    for (i = 0; i < 3; i++)
      PrcOpt.antdel[1][i] = RefAntDel[i];
  }
  ...
  if (PrcOpt.sateph == EPHOPT_PREC || PrcOpt.sateph == EPHOPT_SSRCOM) {
    if (!readpcv(qPrintable(SatPcvFileF), &pcvs)) {
      Message->setText(
          QString(tr("sat ant file read error %1")).arg(SatPcvFileF));
      return;
    }
    for (i = 0; i < MAXSAT; i++) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (strs[i] == STR_SERIAL) {
      if (CmdEna[i][0])
        strcpy(cmds[i], qPrintable(Cmds[i][0]));
    } else if (strs[i] == STR_TCPCLI || strs[i] == STR_TCPSVR ||
               strs[i] == STR_NTRIPCLI) {
      if (CmdEnaTcp[i][0])
        strcpy(cmds[i], qPrintable(CmdsTcp[i][0]));
    }
  ...
  // start rtk server
  if (!rtksvrstart(&rtksvr, SvrCycle, SvrBuffSize, strs, paths, Format,
                   NavSelect, cmds, cmds_periodic, rcvopts, NmeaCycle, NmeaReq,
                   nmeapos, &PrcOpt, solopt, &monistr, buff)) {
    traceclose();
    for (i = 0; i < 8; i++)
      delete[] paths[i];
    for (i = 0; i < 3; i++)
  ...
  SolStat[0] = Nvsat[0] = 0;
  for (i = 0; i < 3; i++)
    SolRov[i] = SolRef[i] = VelRov[i] = 0.0;
  for (i = 0; i < 9; i++)
    Qr[i] = 0.0;
  Age[0] = Ratio[0] = 0.0;
  Nsat[0] = Nsat[1] = 0;
...
void MainWindow::SvrStop(void) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (str == STR_SERIAL) {
      if (CmdEna[i][1])
        strcpy(cmds[i], qPrintable(Cmds[i][1]));
    } else if (str == STR_TCPCLI || str == STR_TCPSVR || str == STR_NTRIPCLI) {
      if (CmdEnaTcp[i][1])
        strcpy(cmds[i], qPrintable(CmdsTcp[i][1]));
    }
  }
...
void MainWindow::TimerTimer() {
  ...
  if (update) {
    ...
    if (++inactive * Timer.interval() > TIMEOUT)
      SolCurrentStat = 0;
  }
  if (SolCurrentStat) {
...
// update log ---------------------------------------------------------------
void MainWindow::UpdateLog(int stat, gtime_t time, double *rr, float *qr,
  ...
  PSol = PSolE;
  if (++PSolE >= SolBuffSize)
    PSolE = 0;
  if (PSolE == PSolS && ++PSolS >= SolBuffSize)
    PSolS = 0;
}
// update font --------------------------------------------------------------
...
void MainWindow::UpdatePos(void) {
  QLabel *label[] = {Plabel1, Plabel2, Plabel3,  Pos1,
                     Pos2,    Pos3,    LabelStd, LabelNSat};
  ...
  double *rr = SolRov + PSol * 3, *rb = SolRef + PSol * 3, *qr = Qr + PSol * 9,
         pos[3] = {0}, Qe[9] = {0};
  double dms1[3] = {0}, dms2[3] = {0}, bl[3] = {0}, enu[3] = {0}, pitch = 0.0,
         yaw = 0.0, len;
  int i, stat = SolStat[PSol];

  trace(4, "UpdatePos\n");

  if (rtksvr.rtk.opt.mode == PMODE_STATIC ||
      rtksvr.rtk.opt.mode == PMODE_PPP_STATIC) {
    ext = " (S)";
  } else if (rtksvr.rtk.opt.mode == PMODE_FIXED ||
             rtksvr.rtk.opt.mode == PMODE_PPP_FIXED) {
    ext = " (F)";
  }
  ...
  Solution->setStyleSheet(
      QString("QLabel {color: %1;}").arg(rtksvr.state ? color[stat] : "gray"));
  ...
  for (i = 0; i < 8; i++)
    label[i]->setText(s[i]);
  for (i = 3; i < 6; i++) {
    label[i]->setStyleSheet(QString("QLabel {color: %1;}")
                                .arg(PrcOpt.mode == PMODE_MOVEB && SolType <= 2
                                         ? "grey"
                                         : "black"));
  }
...
void MainWindow::UpdateStr(void) {
  ...
  for (i = 0; i < MAXSTRRTK; i++) {
    ind[i]->setStyleSheet(
        QString("QLabel {background-color: %1}").arg(color[sstat[i] + 1]));
    if (sstat[i]) {
      Message->setText(msg);
    }
  }
...
QColor MainWindow::SnrColor(int snr) {
  ...
  g1 =
      static_cast<unsigned int>(a * c1.green() + (1.0 - a) * c2.green()) & 0xFF;
  b1 = static_cast<unsigned int>(a * c1.blue() + (1.0 - a) * c2.blue()) & 0xFF;

  return QColor(r1, g1, b1);
}
// draw snr plot ------------------------------------------------------------
void MainWindow::DrawSnr(QPainter *c, int w, int h, int x0, int y0, int index,
                         int freq) {
  static const QColor color[] = {QColor(0, 128, 0),     QColor(0, 128, 128),
                                 QColor(0xA0, 0, 0xA0), QColor(128, 0, 0),
                                 QColor(0, 0, 128),     QColor(128, 128, 128)};
  static const QColor color_sys[] = {
      Qt::green, QColor(0, 0xAA, 0xFF), QColor(255, 0, 255), Qt::blue, Qt::red,
      Qt::gray};
  QString s;
  int i, j, k, l, n, x1, y1, y2, y3, hh = h - 15, ww, www, snr[NFREQ + 1],
                                     mask[6] = {0};
  ...
  for (i = 0; i < Nsat[index] && i < MAXSAT; i++) {
    ...
    for (j = 0; j < NFREQ + 2; j++) {
      ...
      y2 = y1 - y3;
      if (snr[k] > 0)
        y2 -= (snr[k] - MINSNR) * hh / (MAXSNR - MINSNR) - y3;
      y2 = y2 < 2 ? 2 : (y1 < y2 ? y1 : y2);

      QRect r1(x1, y1, www, y2 - y1);
      if (j == 0) {
        c->setBrush(QBrush(freq < NFREQ ? SnrColor(snr[k]) : color_sys[l],
                           Qt::SolidPattern));
        if (!Vsat[index][i])
...
void MainWindow::DrawSat(QPainter *c, int w, int h, int x0, int y0, int index,
                         int freq) {
  ...
  for (i = 0, k = Nsat[index] - 1; i < Nsat[index] && i < MAXSAT; i++, k--) {
    ...
    l = (q = strchr(sys, id[0])) ? (int)(q - sys) : 5;
    x[i] = static_cast<int>(p.x() + r * (90 - El[index][k] * R2D) / 90 *
                                        sin(Az[index][k])) +
           x0;
    y[i] = static_cast<int>(p.y() - r * (90 - El[index][k] * R2D) / 90 *
                                        cos(Az[index][k])) +
           y0;
    c->setPen(Qt::gray);
    d = SATSIZE / 2;
    c->setBrush(!Vsat[index][k] ? QColor(0xc0, 0xc0, 0xc0)
                                : (freq < NFREQ ? SnrColor(Snr[index][k][f])
                                                : color_sys[l]));
    c->drawEllipse(x[i] - d, y[i] - d, 2 * d + 1, 2 * d + 1);
    DrawText(c, x[i], y[i], s = id, Qt::white, 1);
  }
  dops(ns, azel, 0.0, dop);
  DrawText(c, x0 + 3, y0 + h - 15, QString("# Sat:%1").arg(Nsat[index], 2),
           Qt::gray, 0);
  DrawText(c, x0 + w - 3, y0 + h - 15,
           QString("GDOP:%1").arg(dop[0], 0, 'f', 1), Qt::gray, 2);
}
// draw baseline plot -------------------------------------------------------
void MainWindow::DrawBL(QPainter *c, QLabel *disp, int w, int h) {
  QColor color[] = {
      QColor(0xc0, 0xc0, 0xc0), Qt::green, QColor(0x00, 0xAA, 0xFF),
  ...
  double cp, q, az = 0.0;
  QColor col = Qt::white;
  int i, d1 = 10, d2 = 16, d3 = 10, cy = 0, sy = 0, cya = 0, sya = 0, a, x1, x2,
         y1, y2, r1, digit, mode;
  ...
  if (PMODE_DGPS <= PrcOpt.mode && PrcOpt.mode <= PMODE_FIXED) {
    col = rtksvr.state && SolStat[PSol] && SolCurrentStat ? color[SolStat[PSol]]
                                                          : Qt::white;
  ...
  if (len >= MINBLLEN) {
    ...
    cya =
        static_cast<int>(((r - d1 - d2 / 2) * cp - d2 / 2 - 4) * cos(yaw - az));
    sya =
        static_cast<int>(((r - d1 - d2 / 2) * cp - d2 / 2 - 4) * sin(yaw - az));
  }
  p1.setX(p.x() - sy);
  p1.setY(p.y() + cy); // base
  p2.setX(p.x() + sy);
  ...
  if (pitch < 0.0) {
    ...
    DrawArrow(c, p.x() + sya, p.y() - cya, d3,
              static_cast<int>((yaw - az) * R2D), QColor(0xc0, 0xc0, 0xc0));
  }
  c->setBrush(col);
  ...
  for (a = 0; a < 360; a += 5) {
    q = a % 90 == 0
            ? 0
            : (a % 30 == 0 ? r - d1 * 3 : (a % 10 == 0 ? r - d1 * 2 : r - d1));
  ...
  if (pitch >= 0.0) {
    ...
    DrawArrow(c, p.x() + sya, p.y() - cya, d3,
              static_cast<int>((yaw - az) * R2D), Qt::gray);
  }
  c->setBrush(col);
  c->drawEllipse(pp.x() - d2 / 2 + 2, pp.y() - d2 / 2 + 2, d2 - 4, d2 - 4);
  c->setBrush(Qt::white);
  digit = len < 10.0 ? 3 : (len < 100.0 ? 2 : (len < 1000.0 ? 1 : 0));
  DrawText(c, p.x(), p.y(), QString("%1 m").arg(len, 0, 'f', digit), Qt::gray,
           1);
  DrawText(c, 5, h - 15,
           QString("Y: %1%2").arg(yaw * R2D, 0, 'f', 1).arg(degreeChar),
           Qt::gray, 0);
  DrawText(c, w - 3, h - 15,
           QString("P: %1%2").arg(pitch * R2D, 0, 'f', 1).arg(degreeChar),
           Qt::gray, 2);
}
// draw track plot ----------------------------------------------------------
void MainWindow::DrawTrk(QPainter *c, QLabel *disp, QPixmap &buff) {
  ...
  QPoint p1, p2;
  QString label;
  double scale[] = {0.00021, 0.00047, 0.001,  0.0021, 0.0047, 0.01,
                    0.021,   0.047,   0.1,    0.21,   0.47,   1.0,
                    2.1,     4.7,     10.0,   21.0,   47.0,   100.0,
                    210.0,   470.0,   1000.0, 2100.0, 4700.0, 10000.0};
  ...
  if (norm(ref, 3) > 1E-6) {
    ...
    p1.ry() = p2.y() + 12;
    label =
        QString("%1 %2").arg(pos[0] * R2D, 0, 'f').arg(pos[1] * R2D, 0, 'f', 9);
...
void MainWindow::DrawSky(QPainter *c, int w, int h, int x0, int y0) {
  ...
  for (a = 0; a < 360; a += 45) {
    ...
    if (a % 90 == 0)
      DrawText(c, p.x() + x, p.y() - y, label[a / 90], Qt::gray, 1);
  }
}
// draw text ----------------------------------------------------------------
...
// draw arrow ---------------------------------------------------------------
void MainWindow::DrawArrow(QPainter *c, int x, int y, int siz, int ang,
                           const QColor &color) {
  QPoint p1[4], p2[4];
  int i;
  ...
  for (i = 0; i < 4; i++) {
    p2[i].setX(x + static_cast<int>(p1[i].x() * cos(-ang * D2R) -
                                    p1[i].y() * sin(-ang * D2R) + 0.5));
    p2[i].setY(y - static_cast<int>(p1[i].x() * sin(-ang * D2R) +
                                    p1[i].y() * cos(-ang * D2R) + 0.5));
  }
...
void MainWindow::OpenMoniPort(int port) {
  ...
  for (i = 0; i <= MAXPORTOFF; i++) {
    ...
    if (stropen(&monistr, STR_TCPSVR, STR_MODE_RW, path)) {
      ...
      if (i > 0)
        setWindowTitle(QString(tr("%1 ver.%2 (%3)"))
                           .arg(PRGNAME)
                           .arg(VER_RTKLIB)
                           .arg(i + 1));
      OpenPort = MoniPort + i;
  ...
  QMessageBox::critical(this, tr("Error"),
                        QString(tr("monitor port %1-%2 open error"))
                            .arg(port)
                            .arg(port + MAXPORTOFF));
  OpenPort = 0;
}
...
void MainWindow::InitSolBuff(void) {
  ...
  for (i = 0; i < SolBuffSize; i++) {
    ...
    SolStat[i] = Nvsat[i] = 0;
    for (j = 0; j < 3; j++)
      SolRov[j + i * 3] = SolRef[j + i * 3] = VelRov[j + i * 3] = 0.0;
    for (j = 0; j < 9; j++)
      Qr[j + i * 9] = 0.0;
    Age[i] = Ratio[i] = 0.0;
  }
...
void MainWindow::SaveLog(void) {
  ...
  fileName = QDir::toNativeSeparators(
      QFileDialog::getSaveFileName(this, QString(), fileTemplate));
  QFile file(fileName);
...
void MainWindow::LoadNav(nav_t *nav) {
  ...
  for (i = 0; i < MAXSAT; i++) {
    if ((str = settings.value(QString("navi/eph_%1").arg(i, 2)).toString())
            .isEmpty())
      continue;
    nav->eph[i] = eph0;
    ...
    if (!(p = strchr(buff, ',')))
      continue;
    *p = '\0';
    if (!(nav->eph[i].sat = satid2no(buff)))
      continue;
    sscanf(p + 1,
           "%d,%d,%d,%d,%ld,%ld,%ld,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%"
           "lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%d,%d",
           &nav->eph[i].iode, &nav->eph[i].iodc, &nav->eph[i].sva,
           &nav->eph[i].svh, &nav->eph[i].toe.time, &nav->eph[i].toc.time,
           &nav->eph[i].ttr.time, &nav->eph[i].A, &nav->eph[i].e,
           &nav->eph[i].i0, &nav->eph[i].OMG0, &nav->eph[i].omg,
           &nav->eph[i].M0, &nav->eph[i].deln, &nav->eph[i].OMGd,
           &nav->eph[i].idot, &nav->eph[i].crc, &nav->eph[i].crs,
           &nav->eph[i].cuc, &nav->eph[i].cus, &nav->eph[i].cic,
           &nav->eph[i].cis, &nav->eph[i].toes, &nav->eph[i].fit,
           &nav->eph[i].f0, &nav->eph[i].f1, &nav->eph[i].f2,
  ...
  for (i = 0; i < 8; i++)
    nav->ion_gps[i] = 0.0;
  for (i = 0; (i < 8) && (i < tokens.size()); i++)
    nav->ion_gps[i] = tokens.at(i).toDouble();
  ...
  for (i = 0; (i < 4) && (i < tokens.size()); i++)
    nav->utc_gps[i] = tokens.at(i).toDouble();
...
void MainWindow::SaveNav(nav_t *nav) {
  ...
  str = "";
  for (i = 0; i < 8; i++)
    str = str + QString("%1,").arg(nav->ion_gps[i], 0, 'E', 14);
  settings.setValue("navi/ion", str);

  str = "";
  for (i = 0; i < 4; i++)
    str = str + QString("%1,").arg(nav->utc_gps[i], 0, 'E', 14);
...
void MainWindow::LoadOpt(void) {
  ...
  for (i = 0; i < 8; i++) {
    ...
    for (j = 0; j < 4; j++) {
      Paths[i][j] =
          settings.value(QString("stream/path_%1_%2").arg(no).arg(j), "")
              .toString();
    }
  }
  for (i = 0; i < 3; i++) {
    RcvOpt[i] =
        settings.value(QString("stream/rcvopt%1").arg(i + 1), "").toString();
  }
  for (i = 0; i < 3; i++)
    for (j = 0; j < 2; j++) {
      Cmds[i][j] = settings.value(QString("serial/cmd_%1_%2").arg(i).arg(j), "")
                       .toString();
      CmdEna[i][j] =
          settings.value(QString("serial/cmdena_%1_%2").arg(i).arg(j), 0)
              .toInt();
      Cmds[i][j].replace("@@", "\r\n");
    }
  for (i = 0; i < 3; i++)
    for (j = 0; j < 2; j++) {
      CmdsTcp[i][j] =
          settings.value(QString("tcpip/cmd_%1_%2").arg(i).arg(j), "")
              .toString();
      CmdEnaTcp[i][j] =
          settings.value(QString("tcpip/cmdena_%1_%2").arg(i).arg(j), 0)
              .toInt();
      CmdsTcp[i][j].replace("@@", "\r\n");
    }
  ...
  for (i = 0; i < NFREQ; i++)
    for (j = 0; j < 9; j++) {
      PrcOpt.snrmask.mask[i][j] =
          settings
              .value(QString("prcopt/snrmask_%1_%2").arg(i + 1).arg(j + 1), 0.0)
              .toInt();
    }
  ...
  LocalDirectory =
      settings.value("setting/localdirectory", "C:\\Temp").toString();
  ...
  for (i = 0; i < 3; i++) {
    RovAntDel[i] =
        settings.value(QString("setting/rovantdel_%1").arg(i), 0.0).toDouble();
    RefAntDel[i] =
        settings.value(QString("setting/refantdel_%1").arg(i), 0.0).toDouble();
    RovPos[i] =
        settings.value(QString("setting/rovpos_%1").arg(i), 0.0).toDouble();
    RefPos[i] =
        settings.value(QString("setting/refpos_%1").arg(i), 0.0).toDouble();
  }
  for (i = 0; i < 10; i++) {
    History[i] =
        settings.value(QString("tcpopt/history%1").arg(i), "").toString();
  }
  for (i = 0; i < 10; i++) {
    MntpHist[i] =
        settings.value(QString("tcpopt/mntphist%1").arg(i), "").toString();
  }
  NMapPnt = settings.value("mapopt/nmappnt", 0).toInt();
  for (i = 0; i < NMapPnt; i++) {
    PntName[i] =
        settings.value(QString("mapopt/pntname%1").arg(i + 1), "").toString();
    QString pos = settings.value(QString("mapopt/pntpos%1").arg(i + 1), "0,0,0")
                      .toString();
    PntPos[i][0] = PntPos[i][1] = PntPos[i][2] = 0.0;
    sscanf(qPrintable(pos), "%lf,%lf,%lf", PntPos[i], PntPos[i] + 1,
           PntPos[i] + 2);
  }
  PosFont.setFamily(
      settings.value("setting/posfontname", POSFONTNAME).toString());
  PosFont.setPointSize(
      settings.value("setting/posfontsize", POSFONTSIZE).toInt());
  //    PosFont.setStyle(QColor(settings.value("setting/posfontcolor",(int)clBlack).toInt());
  if (settings.value("setting/posfontbold", 0).toInt())
    PosFont.setBold(true);
  if (settings.value("setting/posfontitalic", 0).toInt())
    PosFont.setItalic(true);
  ;

  TextViewer::Color1 = QColor(static_cast<QRgb>(
      settings.value("viewer/color1", static_cast<int>(Qt::black)).toInt()));
  TextViewer::Color2 = QColor(static_cast<QRgb>(
      settings.value("viewer/color2", static_cast<int>(Qt::white)).toInt()));
...
void MainWindow::SaveOpt(void) {
  ...
  for (i = 0; i < 8; i++) {
    ...
    for (j = 0; j < 4; j++) {
      settings.setValue(QString("stream/path_%1_%2").arg(no).arg(j),
                        Paths[i][j]);
    }
  }
  for (i = 0; i < 3; i++) {
  ...
  for (i = 0; i < 3; i++)
    for (j = 0; j < 2; j++) {
      ...
      settings.setValue(QString("serial/cmdena_%1_%2").arg(i).arg(j),
                        CmdEna[i][j]);
    }
  for (i = 0; i < 3; i++)
    for (j = 0; j < 2; j++) {
      CmdsTcp[i][j].replace("\r\n", "@@");
      settings.setValue(QString("tcpip/cmd_%1_%2").arg(i).arg(j),
                        CmdsTcp[i][j]);
      settings.setValue(QString("tcpip/cmdena_%1_%2").arg(i).arg(j),
                        CmdEnaTcp[i][j]);
    }
  ...
  settings.setValue("viewer/color1",
                    static_cast<int>(TextViewer::Color1.rgb()));
  settings.setValue("viewer/color2",
                    static_cast<int>(TextViewer::Color2.rgb()));
