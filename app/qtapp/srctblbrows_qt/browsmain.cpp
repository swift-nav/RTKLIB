...
static char *getsrctbl(const QString addr) {
  ...
  while (p < buff + MAXSRCTBL - 1) {
    int ns = strread(&str, (unsigned char *)p, (buff + MAXSRCTBL - p - 1));
    *(p + ns) = '\0';
    if (p - len - 3 > buff && strstr(p - len - 3, ENDSRCTBL))
      break;
    p += ns;
    qApp->processEvents();
...
MainForm::MainForm(QWidget *parent) : QMainWindow(parent) {
  ...
  connect(Address, SIGNAL(currentTextChanged(QString)), this,
          SLOT(AddressChange()));
  ...
  connect(MenuUpdateCaster, SIGNAL(triggered(bool)), this,
          SLOT(MenuUpdateCasterClick()));
  connect(MenuUpdateTable, SIGNAL(triggered(bool)), this,
          SLOT(MenuUpdateTableClick()));
  ...
  connect(Table0, SIGNAL(cellClicked(int, int)), this,
          SLOT(Table0SelectCell(int, int)));
...
void MainForm::showEvent(QShowEvent *event) {
  ...
  parser.addPositionalArgument(
      "URL",
      QCoreApplication::translate("main", "file with list of Ntrip sources"));
...
void MainForm::MenuOpenClick() {
  QString OpenDialog_FileName =
      QDir::toNativeSeparators(QFileDialog::getOpenFileName(
          this, tr("Open"), QString(), tr("All File (*.*)")));
...
void MainForm::MenuSaveClick() {
  QString SaveDialog_FileName =
      QDir::toNativeSeparators(QFileDialog::getSaveFileName(
          this, tr("Save File"), QString(), tr("All File (*.*)")));
...
void MainForm::Table0SelectCell(int ACol, int ARow) {
  ...
  if (0 < ARow && ARow < Table0->rowCount()) {
    ...
    googleMapView->setWindowTitle(QString(tr("NTRIP STR Map: %1/%2"))
...
void MainForm::UpdateCaster() {
  ...
  if ((srctbl = CasterWatcher.result()).isEmpty())
    return;
...
void MainForm::ShowTable(void) {
  const QString ti[3][18] = {
      {tr("Mountpoint"), tr("ID"), tr("Format"), tr("Format-Details"),
       tr("Carrier"), tr("Nav-System"), tr("Network"), tr("Country"),
       tr("Latitude"), tr("Longitude"), tr("NMEA"), tr("Solution"),
       tr("Generator"), "Compr-Encrp", "Authentication", "Fee", "Bitrate", ""},
      {tr("Host"), tr("Port"), tr("ID"), tr("Operator"), tr("NMEA"),
       tr("Country"), tr("Latitude"), tr("Longitude"), tr("Fallback_Host"),
       tr("Fallback_Port"), "", "", "", "", "", "", ""},
      {tr("ID"), tr("Operator"), tr("Authentication"), tr("Fee"), tr("Web-Net"),
  ...
  if (type == 3) {
    ...
    for (i = 0; i < 18; i++) {
      table[type]->setHorizontalHeaderItem(i,
                                           new QTableWidgetItem(ti[type][i]));
      table[type]->setItem(0, i, new QTableWidgetItem(""));
    }
  }
  if (AddrCaster != Address->currentText())
    return;
  if (type == 3) {
...
void MainForm::UpdateMap(void) {
  ...
  if (Address->currentText() == "") {
    ...
    googleMapView->setWindowTitle(
        QString(tr("NTRIP STR Map: %1")).arg(Address->currentText()));
  }
  googleMapView->ClearMark();

  for (int i = 0; i < Table0->rowCount(); i++) {
    if (Table0->item(i, 8)->text() == "")
      continue;
    ...
    msg = "<b>" + Table0->item(i, 0)->text() +
          "</b>: " + Table0->item(i, 1)->text() + " (" +
          Table0->item(i, 7)->text() + "), POS: " + Table0->item(i, 8)->text() +
          ", " + Table0->item(i, 9)->text() + "<br>" +
          Table0->item(i, 2)->text() + ": " + Table0->item(i, 3)->text() +
          "<br>" + Table0->item(i, 5)->text() + ", " +
          Table0->item(i, 6)->text() + ", " + Table0->item(i, 12)->text();
