...
extern void settime(gtime_t time) {
  double tt;
  if (tend_.time != 0 && tstart_.time != 0 &&
      (tt = timediff(tend_, tstart_)) > 0.0) {
    QMetaObject::invokeMethod(
        mainForm->Progress, "setValue", Qt::QueuedConnection,
...
void ProcessingThread::run() {
  if ((stat = postpos(ts, te, ti, tu, &prcopt, &solopt, &filopt, infile, n,
                      outfile, rov, base)) == 1) {
    showmsg("aborted");
  };
  emit done(stat);
...
MainForm::MainForm(QWidget *parent) : QDialog(parent) {
  ...
  connect(BtnInputFile1, SIGNAL(clicked(bool)), this,
          SLOT(BtnInputFile1Click()));
  connect(BtnInputFile2, SIGNAL(clicked(bool)), this,
          SLOT(BtnInputFile2Click()));
  connect(BtnInputFile3, SIGNAL(clicked(bool)), this,
          SLOT(BtnInputFile3Click()));
  connect(BtnInputFile4, SIGNAL(clicked(bool)), this,
          SLOT(BtnInputFile4Click()));
  connect(BtnInputFile5, SIGNAL(clicked(bool)), this,
          SLOT(BtnInputFile5Click()));
  connect(BtnInputFile6, SIGNAL(clicked(bool)), this,
          SLOT(BtnInputFile6Click()));
  connect(BtnOutputFile, SIGNAL(clicked(bool)), this,
          SLOT(BtnOutputFileClick()));
  connect(BtnInputView1, SIGNAL(clicked(bool)), this,
          SLOT(BtnInputView1Click()));
  connect(BtnInputView2, SIGNAL(clicked(bool)), this,
          SLOT(BtnInputView2Click()));
  connect(BtnInputView3, SIGNAL(clicked(bool)), this,
          SLOT(BtnInputView3Click()));
  connect(BtnInputView4, SIGNAL(clicked(bool)), this,
          SLOT(BtnInputView4Click()));
  connect(BtnInputView5, SIGNAL(clicked(bool)), this,
          SLOT(BtnInputView5Click()));
  connect(BtnInputView6, SIGNAL(clicked(bool)), this,
          SLOT(BtnInputView6Click()));
  connect(BtnOutputView1, SIGNAL(clicked(bool)), this,
          SLOT(BtnOutputView1Click()));
  connect(BtnOutputView2, SIGNAL(clicked(bool)), this,
          SLOT(BtnOutputView2Click()));
  connect(BtnInputPlot1, SIGNAL(clicked(bool)), this,
          SLOT(BtnInputPlot1Click()));
  connect(BtnInputPlot2, SIGNAL(clicked(bool)), this,
          SLOT(BtnInputPlot2Click()));
  ...
  connect(InputFile1, SIGNAL(currentIndexChanged(int)), this,
          SLOT(InputFile1Change()));
...
void MainForm::FormCreate() {
  setWindowTitle(
      QString("%1 ver.%2 %3").arg(PRGNAME).arg(VER_RTKLIB).arg(PATCH_LEVEL));
}
...
void MainForm::showEvent(QShowEvent *event) {
  ...
  QCommandLineOption roverOption(
      QStringList() << "r",
  ...
  QCommandLineOption navFileOption(
      ...
      QCoreApplication::translate("main",
                                  "rinex nav/clk, sp3, ionex or sp3 <file>"),
      QCoreApplication::translate("main", "file"));
  ...
  QCommandLineOption timeStartOption(
      QStringList() << "ts", QCoreApplication::translate("main", "time start"),
      QCoreApplication::translate("main", "yyyy/mm/dd hh:mm:ss"));
  parser.addOption(timeStartOption);

  QCommandLineOption timeEndOption(
      QStringList() << "ts", QCoreApplication::translate("main", "time end"),
      QCoreApplication::translate("main", "yyyy/mm/dd hh:mm:ss"));
  parser.addOption(timeEndOption);

  QCommandLineOption timeIntervalOption(
      QStringList() << "ti",
      QCoreApplication::translate("main", "time interval (s)"),
      QCoreApplication::translate("main", "time"));
  parser.addOption(timeIntervalOption);

  QCommandLineOption timeUnitOption(
      QStringList() << "tu",
...
void MainForm::BtnExecClick() {
  ...
  if (InputFile2->currentText() == "" && PMODE_DGPS <= PosMode &&
      PosMode <= PMODE_FIXED) {
  ...
  if (OutputFile_Text.contains(".obs", Qt::CaseInsensitive) ||
    ...
    showmsg("error : invalid extension of output file (%s)",
            qPrintable(OutputFile_Text));
    return;
  }
  showmsg("");
...
void MainForm::BtnInputFile1Click() {
  InputFile1->setCurrentText(
      QDir::toNativeSeparators(QFileDialog::getOpenFileName(
          this, tr("RINEX OBS (Rover) File"), InputFile1->currentText(),
          tr("All (*.*);;RINEX OBS (*.obs *.*O *.*D)"))));
  SetOutFile();
}
// callback on button-inputfile-2 -------------------------------------------
void MainForm::BtnInputFile2Click() {
  InputFile2->setCurrentText(
      QDir::toNativeSeparators(QFileDialog::getOpenFileName(
...
void MainForm::BtnInputFile3Click() {
  InputFile3->setCurrentText(
      QDir::toNativeSeparators(QFileDialog::getOpenFileName(
          this, tr("RINEX NAV/CLK,SP3,FCB,IONEX or SBAS/EMS File"),
          InputFile3->currentText(),
          tr("All (*.*);;RINEX NAV (*.*nav *.*N *.*P *.*G *.*H *.*Q)"))));
}
// callback on button-inputfile-4 -------------------------------------------
void MainForm::BtnInputFile4Click() {
  InputFile4->setCurrentText(
      QDir::toNativeSeparators(QFileDialog::getOpenFileName(
          this, tr("RINEX NAV/CLK,SP3,FCB,IONEX or SBAS/EMS File"),
          InputFile4->currentText(),
          tr("All (*.*);;Precise Ephemeris/Clock (*.sp3 *.eph* *.clk*)"))));
}
// callback on button-inputfile-5 -------------------------------------------
void MainForm::BtnInputFile5Click() {
  InputFile5->setCurrentText(
      QDir::toNativeSeparators(QFileDialog::getOpenFileName(
          this, tr("RINEX NAV/CLK,SP3,FCB,IONEX or SBAS/EMS File"),
          InputFile5->currentText(),
          tr("All (*.*);;Precise Ephemeris/Clock (*.sp3 *.eph* *.clk*)"))));
}
// callback on button-inputfile-6 -------------------------------------------
void MainForm::BtnInputFile6Click() {
  InputFile6->setCurrentText(
      QDir::toNativeSeparators(QFileDialog::getOpenFileName(
          this, tr("RINEX NAV/CLK,SP3,FCB,IONEX or SBAS/EMS File"),
          InputFile6->currentText(),
          tr("All (*.*);;FCB (*.fcb),IONEX (*.*i *.ionex),SBAS (*.sbs "
             "*.ems)"))));
}
...
void MainForm::BtnOutDirClick() {
  ...
  if (!SelectDirectory("Output Directory", "", dir))
    return;
  OutDir->Text = dir;
#else
  OutDir->setText(QDir::toNativeSeparators(QFileDialog::getExistingDirectory(
      this, tr("Output Directory"), OutDir->text())));
#endif
}
...
void MainForm::ExecProc(void) {
  QString InputFile1_Text = InputFile1->currentText(),
          InputFile2_Text = InputFile2->currentText();
  QString InputFile3_Text = InputFile3->currentText(),
          InputFile4_Text = InputFile4->currentText();
  QString InputFile5_Text = InputFile5->currentText(),
          InputFile6_Text = InputFile6->currentText();
  QString OutputFile_Text = OutputFile->currentText();
  QString temp;
  ...
  if (TimeStart->isChecked())
    thread->ts = GetTime1();
  if (TimeEnd->isChecked())
    thread->te = GetTime2();
  if (TimeIntF->isChecked())
    thread->ti = TimeInt->currentText().toDouble();
  if (TimeUnitF->isChecked())
    thread->tu = TimeUnit->text().toDouble() * 3600.0;
  ...
  if (!TimeStart->isChecked() || !TimeEnd->isChecked()) {
    if (QFileInfo::exists(thread->outfile)) {
      if (QMessageBox::question(this, tr("Overwrite"),
                                QString(tr("Overwrite existing file %1."))
                                    .arg(thread->outfile)) !=
          QMessageBox::Yes) {
        ProcessingFinished(0);
        return;
...
int MainForm::GetOption(prcopt_t &prcopt, solopt_t &solopt, filopt_t &filopt) {
  ...
  strcpy(solopt.prog, qPrintable(QString("%1 ver.%2 %3")
                                     .arg(PRGNAME)
                                     .arg(VER_RTKLIB)
                                     .arg(PATCH_LEVEL)));
...
int MainForm::ObsToNav(const QString &obsfile, QString &navfile) {
  ...
  if ((suffix.length() == 3) && (suffix.at(2).toLower() == 'o'))
    ...
    suffix = "*nav";
  else if (((p = suffix.indexOf("gz")) != -1) ||
           ((p = suffix.indexOf('Z')) != -1)) {
    if (p < 1)
      return 0;
...
QString MainForm::FilePath(const QString &file) {
  ...
  while ((p = RovList.indexOf("\n", p)) != -1) {
    if ((p < RovList.count()) && (RovList.at(p) != '#'))
      break;
  }
  if (p != -1)
  ...
  while ((p = BaseList.indexOf("\n", p)) != -1) {
    if ((p < BaseList.count()) && (BaseList.at(p) != '#'))
      break;
  }
  if (p != -1)
...
// write history ------------------------------------------------------------
void MainForm::WriteList(QSettings *ini, const QString &key,
...
void MainForm::AddHist(QComboBox *combo) {
  ...
  for (int i = combo->count() - 1; i >= MAXHIST; i--)
    combo->removeItem(i);
...
void MainForm::SetTime1(gtime_t time) {
  QDateTime t = QDateTime::fromTime_t(time.time);
  t = t.addSecs(time.sec);
...
void MainForm::LoadOpt(void) {
  ...
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 9; j++) {
      SnrMask.mask[i][j] =
          ini.value(QString("opt/snrmask_%1_%2").arg(i + 1).arg(j + 1), 0.0)
              .toDouble();
    }
  ...
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 6; j++) {
      ExtErr.cerr[i][j] =
          ini.value(QString("opt/exterr_cerr%1%2").arg(i).arg(j), 0.3)
              .toDouble();
    }
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 6; j++) {
      ExtErr.perr[i][j] =
          ini.value(QString("exterr_perr%1%2").arg(i).arg(j), 0.003).toDouble();
    }
  ...
  convDialog->TrackColor->setCurrentIndex(
      ini.value("conv/trackcolor", 5).toInt());
  convDialog->PointColor->setCurrentIndex(
      ini.value("conv/pointcolor", 5).toInt());
  convDialog->OutputAlt->setCurrentIndex(
      ini.value("conv/outputalt", 0).toInt());
  convDialog->OutputTime->setCurrentIndex(
      ini.value("conv/outputtime", 0).toInt());
  ...
  textViewer->Color1 =
      ini.value("viewer/color1", QColor(Qt::black)).value<QColor>();
  textViewer->Color2 =
      ini.value("viewer/color2", QColor(Qt::white)).value<QColor>();
  textViewer->FontD.setFamily(
      ini.value("viewer/fontname", "Courier New").toString());
...
void MainForm::SaveOpt(void) {
  ...
  RovList.replace("\n", "@@");
  for (int i = 0; i < 10; i++) {
    ini.setValue(QString("opt/rovlist%1").arg(i + 1),
                 RovList.mid(i * 2000, 2000));
  }

  BaseList.replace("\n", "@@");
  for (int i = 0; i < 10; i++) {
    ini.setValue(QString("opt/baselist%1").arg(i + 1),
                 BaseList.mid(i * 2000, 2000));
  }
  ...
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 6; j++) {
      ini.setValue(QString("opt/exterr_cerr%1%2").arg(i).arg(j),
                   ExtErr.cerr[i][j]);
    }
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 6; j++) {
