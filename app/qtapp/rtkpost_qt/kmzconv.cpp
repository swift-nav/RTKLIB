...
ConvDialog::ConvDialog(QWidget *parent) : QDialog(parent) {
  ...
  connect(BtnGoogleEarthFile, SIGNAL(clicked(bool)), this,
          SLOT(BtnGoogleEarthFileClick()));
  ...
  connect(GoogleEarthFile, SIGNAL(editingFinished()), this,
          SLOT(GoogleEarthFileChange()));
...
void ConvDialog::BtnInputFileClick() {
  InputFile->setText(QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("Open..."), QString(), tr("All (*.*)"))));
}
//---------------------------------------------------------------------------
void ConvDialog::BtnConvertClick() {
  QString InputFile_Text = InputFile->text(),
          OutputFile_Text = OutputFile->text();
  int stat;
  QString cmd;
  ...
  if (FormatKML->isChecked()) {
    ...
    stat = convkml(
        file, Compress->isChecked() ? kmlfile : qPrintable(OutputFile_Text), ts,
        te, tint, QFlags->currentIndex(), offset, TrackColor->currentIndex(),
        PointColor->currentIndex(), OutputAlt->currentIndex(),
        OutputTime->currentIndex());
  } else {
    stat = convgpx(
        file, Compress->isChecked() ? kmlfile : qPrintable(OutputFile_Text), ts,
        te, tint, QFlags->currentIndex(), offset, TrackColor->currentIndex(),
...
#ifdef Q_OS_WIN
    cmd = QString("zip.exe -j -m %1 %2")
              .arg(OutputFile->text())
              .arg(kmlfile); // TODO: zip for other platforms
#endif
#ifdef Q_OS_LINUX
...
void ConvDialog::UpdateOutFile(void) {
  ...
  if (fi.suffix() == NULL)
    return;
  InputFile_Text =
      QDir::toNativeSeparators(fi.absolutePath() + "/" + fi.baseName());
  InputFile_Text += FormatGPX->isChecked()
                        ? ".gpx"
                        : (Compress->isChecked() ? ".kmz" : ".kml");
...
void ConvDialog::BtnGoogleEarthFileClick() {
  GoogleEarthFile->setText(QDir::toNativeSeparators(
      QFileDialog::getOpenFileName(this, tr("Google Earth Exe File"))));
}
//---------------------------------------------------------------------------
