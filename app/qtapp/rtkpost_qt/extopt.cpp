...
#include <QShowEvent>
//---------------------------------------------------------------------------
ExtOptDialog::ExtOptDialog(QWidget *parent) : QDialog(parent) {
  setupUi(this);
...
//---------------------------------------------------------------------------
void ExtOptDialog::BtnOkClick() { SetExtErrOpt(); }
//---------------------------------------------------------------------------
void ExtOptDialog::ExtEna0Click() { UpdateEnable(); }
//---------------------------------------------------------------------------
void ExtOptDialog::ExtEna1Click() { UpdateEnable(); }
//---------------------------------------------------------------------------
void ExtOptDialog::ExtEna3Click() { UpdateEnable(); }
//---------------------------------------------------------------------------
void ExtOptDialog::ExtEna2Click() { UpdateEnable(); }
//---------------------------------------------------------------------------
void ExtOptDialog::GetExtErrOpt(void) {
  ...
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 6; j++) {
      editc[i][j]->setText(
          QString::number(optDialog->ExtErr.cerr[i][j], 'f', 3));
      editp[i][j]->setText(
          QString::number(optDialog->ExtErr.perr[i][j], 'f', 3));
    }
