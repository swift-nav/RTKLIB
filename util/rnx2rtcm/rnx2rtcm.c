...
static void gen_rtcm_obs(rtcm_t *rtcm, const int *type, int n, FILE *fp) {
  ...
  for (i = 0; i < n; i++) {
    if (is_nav(type[i]) || is_gnav(type[i]) || is_ant(type[i]))
      continue;
...
static void gen_rtcm_nav(gtime_t time, rtcm_t *rtcm, const nav_t *nav,
  ...
  for (i = index[0]; i < nav->n; i++) {
    ...
    if (time.time && timediff(nav->eph[i].ttr, time) > -0.1)
      continue;
    sat = nav->eph[i].sat;
  ...
  for (i = index[1]; i < nav->ng; i++) {
    ...
    if (time.time && timediff(nav->geph[i].tof, time) > -0.1)
      continue;
    sat = nav->geph[i].sat;
...
static int conv_rtcm(const int *type, int n, const char *outfile,
  ...
  for (i = 0; i < nav->ng; i++) {
    if (satsys(nav->geph[i].sat, &prn) != SYS_GLO)
      continue;
    rtcm.nav.geph[prn - 1] = nav->geph[i];
  }
...
int main(int argc, char **argv) {
  ...
  /* convert to rtcm messages */
  if (!conv_rtcm(type, m, outfile, &obs, &nav, &sta, staid))
    ret = -1;
