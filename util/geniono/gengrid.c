...
static void interp_grid(const nav_t *nav, gtime_t ts, gtime_t te, double tint,
  ...
  /* search grid */
  if (!(n = stec_grid(nav, pos, MAX_NGRID, index, dist)))
    return;
  ...
  for (i = 0;; i++, t0 = t1) {
    ...
    for (sat = 1; sat <= MAXSAT; sat++) {
      ...
      if (!fp) {
        sprintf(file, "%s/%c%4.0f_%c%5.0f.stec", dir, pos[0] < 0.0 ? 'S' : 'N',
                fabs(pos[0]) * R2D * 100.0, pos[1] < 0.0 ? 'W' : 'E',
                fabs(pos[1]) * R2D * 100.0);
...
static void interp_stec(const nav_t *nav, gtime_t ts, gtime_t te, double tint,
  ...
  for (i = 0;; i++) { /* N -> S */
    if ((lat = ns[0] - gint * i) < ns[1] - 1E-6)
      break;

    for (j = 0;; j++) { /* W -> E */
      if ((lon = ew[1] + gint * j) > ew[0] + 1E-6)
        break;
...
int main(int argc, char **argv) {
  nav_t nav = {0};
  gtime_t ts = {0}, te = {0};
  double eps[6] = {2011, 1, 1, 0, 0, 0}, epe[6] = {2011, 1, 3, 23, 59, 59},
         tint = 30.0;
