...
static ekf_t *ekf_new(int nx) {
  ekf_t *ekf;
  if (!(ekf = (ekf_t *)malloc(sizeof(ekf_t))))
    return NULL;
  ekf->nx = nx;
  ekf->x = zeros(nx, 1);
  ekf->P = zeros(nx, nx);
...
static void ekf_init(ekf_t *ekf, const double *xi, double *Pi, int ix, int nx) {
  ...
  for (i = 0; i < nx; i++)
    for (j = 0; j < ekf->nx; j++) {
      ekf->P[ix + i + ekf->nx * j] = ekf->P[j + ekf->nx * (ix + i)] =
          j == ix + i ? Pi[i] : 0.0;
    }
}
...
static void ud_state(const obsd_t *obs, int n, const nav_t *nav,
  ...
  for (i = 0; i < n; i++) {
    ...
    if (!raw_obs(obs + i, nav, &P1, &P2, &L1, &L2) || azel[i * 2 + 1] < MIN_EL)
      continue;
...
static void out_iono(gtime_t time, const ekf_t *ekf, const sstat_t *sstat,
  ...
  for (sat = 1; sat <= MAXSAT; sat++) {
    if (sstat[sat - 1].time.time == 0 ||
        timediff(time, sstat[sat - 1].time) > MAXGAP_IONO)
      continue;
    satno2id(sat, id);
    fprintf(fp,
            "%s %4d %6.0f %-3s %d %8.4f %9.6f %7.4f %6.1f %5.1f %7.3f %11.3f\n",
            STEC_RID, week, tow, id, sstat[sat - 1].slip, ekf->x[II(sat)],
            ekf->x[II(sat) + 1], sqrt(ekf->P[II(sat) * (ekf->nx + 1)]),
            sstat[sat - 1].azel[0] * R2D, sstat[sat - 1].azel[1] * R2D,
            sstat[sat - 1].PG, sstat[sat - 1].LG);
  }
}
/* estimate ionosphere -------------------------------------------------------*/
static int est_iono(obs_t *obs, nav_t *nav, const pcv_t *pcv, double *rr,
  ...
  ekf_t *ekf;
  gtime_t time;
  double pos[3], rs[MAXOBS * 6], dts[MAXOBS * 2], var[MAXOBS], e[3],
      azel[MAXOBS * 2];
  ...
  for (i = 0; i < obs->n; i += n) {
    for (n = 1; i + n < obs->n; n++) {
      if (timediff(obs->data[i + n].time, obs->data[i].time) > 1E-3)
        break;
    }
    ...
    for (j = 0; j < n; j++) {
      if (geodist(rs + j * 6, rr, e) > 0.0)
        satazel(pos, e, azel + j * 2);
      else
    ...
    /* ionosphere residuals */
    if ((nv = res_iono(obs->data + i, n, nav, rs, rr, pos, azel, pcv, ekf, phw,
                       v, H, R)) <= 0) {
      continue;
    }
    /* filter */
