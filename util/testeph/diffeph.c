...
static const char *usage[] = {
    ...
    "-1 r        compared ephemeris type "
    "(0:broadcast,1:precise,2:sbas,3:ssrapc,5:lex)",
...
static void updatertcm(gtime_t time, rtcm_t *rtcm, nav_t *nav, FILE *fp) {
  ...
  while (input_rtcm3f(rtcm, fp) >= 0) {
    ...
    if (timediff(rtcm->time, time) >= 5.0)
      break;
  }
  for (i = 0; i < MAXSAT; i++)
...
static int updatelex(int index, gtime_t time, lex_t *lex, nav_t *nav) {
  ...
  for (; index < lex->n; index++) {
    if (!lexupdatecorr(lex->msgs + index, nav, &tof))
      continue;
    if (timediff(tof, time) >= 0.0)
      break;
  }
  return index;
}
...
static void printephdiff(gtime_t time, int sat, int eph1, int eph2,
  ...
  if (!satpos(time, time, sat, eph1, nav1, rs1, dts1, &var1, &svh1))
    return;
  if (!satpos(time, time, sat, eph2, nav2, rs2, dts2, &var2, &svh2))
    return;
  if (svh1 || svh2)
    return;
  ...
  printf("%8.3f ",
         dts1[0] == 0.0 || dts2[0] == 0.0 ? 0.0 : (dts1[0] - dts2[0]) * CLIGHT);
  if (mopt) {
    printf(" %13.3f %13.3f %13.3f %12.3f", rs1[0], rs1[1], rs1[2],
           dts1[0] * CLIGHT);
  }
  if (norm(pos, 3) > 0.0) {
    pos2ecef(pos, rr);
    ...
    for (i = 0; i < 3; i++)
      e[i] = rr2[i] / r2;
    satazel(pos, e, azel);
    printf(" %8.3f %5.1f", (r1 - r2) - (dts1[0] - dts2[0]) * CLIGHT,
           azel[1] * R2D);
  }
  printf("\n");
}
...
int main(int argc, char **argv) {
  ...
  int i, s, n = 0, nx = 0, sat = 0, topt = 0, eopt = 0, mopt = 0, trl = 0,
            index = 0;
  ...
  for (i = 1; i < argc; i++) {
    if (!strcmp(argv[i], "-s") && i + 1 < argc)
      sat = atoi(argv[++i]);
    else if (!strcmp(argv[i], "-1") && i + 1 < argc)
      eph1 = atoi(argv[++i]);
  ...
  for (i = 0; i < n; i++) {
    if (!(ext = strrchr(files[i], '.')))
      ext = "";
    if (!strcmp(ext, ".sp3") || !strcmp(ext, ".SP3") || !strcmp(ext, ".eph") ||
        !strcmp(ext, ".EPH")) {
      if (nav.ne > 0) {
  ...
  for (i = 0; i < (int)(tspan * 3600.0 / tint); i++) {
    ...
    for (s = 1; s <= MAXSAT; s++) {
      ...
      printephdiff(time, s, eph1, eph2, &nav,
                   eph1 == 1 && eph2 == 1 ? &nav2 : &nav, topt, eopt, mopt,
                   pos);
    }
  }
  if (fp)
