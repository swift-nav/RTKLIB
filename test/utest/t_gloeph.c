...
static void dumpgeph(geph_t *geph, int n) {
  ...
  for (i = 0; i < n; i++) {
    ...
    printf("(%3d) sat=%2d frq=%2d svh=%d age=%d toe=%s %s "
           ...
           geph[i].pos[0], geph[i].pos[1], geph[i].pos[2], geph[i].vel[0],
           geph[i].vel[1], geph[i].vel[2], geph[i].acc[0], geph[i].acc[1],
...
void utest5(void) {
  ...
  for (i = 0; i < MAXSAT; i++) {
    satno2id(i + 1, id);
    printf("%2d %-4s %8.3f %8.3f %8.3f\n", i + 1, id, nav.pcvs[i].off[0][0],
           nav.pcvs[i].off[0][1], nav.pcvs[i].off[0][2]);
  }
  printf("\n");
...
void utest6(void) {
  ...
  */
  ...
  for (i = 0; i < tspan / tint; i++) {
    ...
    if (norm(rs1, 3) <= 0.0 || norm(rs2, 3) <= 0.0)
      continue;
