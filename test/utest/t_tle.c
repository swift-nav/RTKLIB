...
static void utest2(void) {
  ...
  for (i = 0; i < 5; i++) {
    ...
    stat =
        tle_pos(timeadd(epoch, min * 60.0), "TEST_SAT", "", "", &tle, NULL, rs);
    assert(stat);

    fprintf(OUT, "%4.0f: %14.8f %14.8f %14.8f  %11.8f %11.8f %11.8f\n", min,
            rs[0] / 1e3, rs[1] / 1e3, rs[2] / 1e3, rs[3] / 1e3, rs[4] / 1e3,
            rs[5] / 1e3);
  }
...
static void utest3(void) {
  ...
  for (i = 0; i < MAXSAT; i++) {
    ...
    for (j = 0; j < 96; j++) {
      ...
      if (!satpos(time, time, i + 1, EPHOPT_BRDC, &nav, rs1, dts, &var, &svh))
        continue;
