...
static void dumpobs(obs_t *obs) {
  ...
  for (i = 0; i < obs->n; i++) {
    ...
    printf("%s : %2d %2d %13.3f %13.3f %13.3f %13.3f  %d %d\n", str,
           obs->data[i].sat, obs->data[i].rcv, obs->data[i].L[0],
           obs->data[i].L[1], obs->data[i].P[0], obs->data[i].P[1],
           obs->data[i].LLI[0], obs->data[i].LLI[1]);
...
static rnxopt_t opt2 = {
    ...
    {"OBSERVER90123456789012345678901", "AGENCY7890123456789012345678901"},
    {"RCV1567890123456789012345678901", "RCV2567890123456789012345678901",
     "RCV3567890123456789012345678901"},
    {"ANT1567890123456789012345678901", "ANT2567890123456789012345678901",
...
void utest4(void) {
  ...
  for (i = j = 0; i < obs.n; i = j) {
    while (j < obs.n && timediff(obs.data[j].time, obs.data[i].time) <= 0.0)
      j++;
