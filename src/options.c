...
#define IONOPT \
  "0:off,1:brdc,2:sbas,3:dual-freq,4:est-stec,5:ionex-tec,6:qzs-brdc"
...
#define ARMOPT                                                              \
  "0:off,1:continuous,2:instantaneous,3:fix-and-hold,4:WLNL,5:TCAR,6:wide-" \
  "lane"
#define POSOPT \
  "0:llh,1:xyz,2:single,3:posfile,4:rinexhead,5:rtcm,6:raw,7:rinexdynamic"
 ...
 * return : status (1:ok,0:error)
 *-----------------------------------------------------------------------------*/
extern int saveopts(const char *file,
                    const char *mode,
                    const char *comment,
                    const opt_t *opts) {
  FILE *fp;
...
static void sysopts2buff(void) {
  ...
  exsats_[0] = '\0';
  for (sat = 1, p = exsats_;
       sat <= MAXSAT && p - exsats_ < (int)sizeof(exsats_) - 32;
       sat++) {
 ...
 *-----------------------------------------------------------------------------*/
extern void setsysopts(const prcopt_t *prcopt,
