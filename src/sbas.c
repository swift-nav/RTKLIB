...
/* search igps ---------------------------------------------------------------*/
static void searchigp(gtime_t time,
                      const double *pos,
                      const sbsion_t *ion,
                      const sbsigp_t **igp,
                      double *x,
...
extern int sbsioncorr(gtime_t time,
  ...
  trace(4,
        "sbsioncorr: pos=%.3f %.3f azel=%.3f %.3f\n",
        pos[0] * R2D,
        pos[1] * R2D,
        azel[0] * R2D,
        azel[1] * R2D);
  ...
  *delay = *var = 0.0;
  ...
  if (err) {
    trace(2,
          "no sbas iono correction: lat=%3.0f lon=%4.0f\n",
          posp[0] * R2D,
          posp[1] * R2D);
    return 0;
  }
 ...
 *-----------------------------------------------------------------------------*/
extern double sbstropcorr(gtime_t time,
                          const double *pos,
                          const double *azel,
                          double *var) {
  ...
  trace(4,
        "sbstropcorr: pos=%.3f %.3f azel=%.3f %.3f\n",
        pos[0] * R2D,
        pos[1] * R2D,
        azel[0] * R2D,
        azel[1] * R2D);
...
static int sbslongcorr(
    gtime_t time, int sat, const sbssat_t *sbssat, double *drs, double *ddts) {
  const sbssatp_t *p;
  double t;
  int i;
  ...
  for (p = sbssat->sat; p < sbssat->sat + sbssat->nsat; p++) {
    ...
    if (fabs(t) > MAXSBSAGEL) {
      trace(2,
            "sbas long-term correction expired: %s sat=%2d t=%5.0f\n",
            time_str(time, 0),
            sat,
            t);
      return 0;
    ...
    *ddts = p->lcorr.daf0 + p->lcorr.daf1 * t;
    ...
    trace(5,
          "sbslongcorr: sat=%2d drs=%7.2f%7.2f%7.2f ddts=%7.2f\n",
          sat,
          drs[0],
          drs[1],
          drs[2],
  ...
  trace(
      2, "no sbas long-term correction: %s sat=%2d\n", time_str(time, 0), sat);
  return 0;
}
/* fast correction -----------------------------------------------------------*/
static int sbsfastcorr(
    gtime_t time, int sat, const sbssat_t *sbssat, double *prc, double *var) {
  const sbssatp_t *p;
  double t;
  ...
  for (p = sbssat->sat; p < sbssat->sat + sbssat->nsat; p++) {
    ...
    *var = varfcorr(p->fcorr.udre) + degfcorr(p->fcorr.ai) * t * t / 2.0;
    ...
    trace(5,
          "sbsfastcorr: sat=%3d prc=%7.2f sig=%7.2f t=%5.0f\n",
          sat,
          *prc,
          sqrt(*var),
          t);
 ...
 *-----------------------------------------------------------------------------*/
extern int sbssatcorr(gtime_t time,
                      int sat,
                      const nav_t *nav,
                      double *rs,
                      double *dts,
 ...
 *-----------------------------------------------------------------------------*/
extern int sbsdecodemsg(gtime_t time,
                        int prn,
