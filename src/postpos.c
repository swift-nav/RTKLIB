...
static void outrpos(FILE *fp, const double *r, const solopt_t *opt) {
  ...
  if (opt->posf == SOLF_LLH || opt->posf == SOLF_ENU) {
    ...
    if (opt->degf) {
      ...
      fprintf(fp,
              "%3.0f%s%02.0f%s%08.5f%s%4.0f%s%02.0f%s%08.5f%s%10.4f",
              dms1[0],
              sep,
              dms1[1],
              sep,
...
static void outheader(
    FILE *fp, char **file, int n, const prcopt_t *popt, const solopt_t *sopt) {
  ...
  if (sopt->outhead) {
    ...
    time2str(te, s3, 1);
    fprintf(fp,
            "%s obs start : %s %s (week%04d %8.1fs)\n",
            COMMENTH,
            s2,
            s1[sopt->times],
            w1,
...
static int valcomb(const sol_t *solf, const sol_t *solb) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    trace(2,
          "degrade fix to float: %s dr=%.3f %.3f %.3f std=%.3f %.3f %.3f\n",
          tstr + 11,
          dr[0],
          dr[1],
          dr[2],
  ...
  for (i = 0, j = isolb - 1; i < isolf && j >= 0; i++, j--) {
    if ((tt = timediff(solf[i].time, solb[j].time)) < -DTTOL) {
      ...
      if (popt->dynamics) {
        ...
        if (smoother(
                solf[i].rr + 3, Qf, solb[j].rr + 3, Qb, 3, sols.rr + 3, Qs))
          continue;
...
static void readpreceph(
    char **infile, int n, const prcopt_t *prcopt, nav_t *nav, sbs_t *sbs) {
  seph_t seph0 = {0};
  int i;
  char *ext;
...
static int readobsnav(gtime_t ts,
                      gtime_t te,
                      double ti,
                      char **infile,
                      const int *index,
  ...
  for (i = 0; i < n; i++) {
    ...
    if (readrnxt(infile[i],
                 rcv,
                 ts,
                 te,
                 ti,
                 prcopt->rnxopt[rcv <= 1 ? 0 : 1],
                 obs,
                 nav,
...
static int antpos(prcopt_t *opt,
                  int rcvno,
                  const obs_t *obs,
                  const nav_t *nav,
                  const sta_t *sta,
...
static int openses(const prcopt_t *popt,
                   const solopt_t *sopt,
                   const filopt_t *fopt,
                   nav_t *nav,
                   pcvs_t *pcvs,
...
static void setpcv(gtime_t time,
                   prcopt_t *popt,
                   nav_t *nav,
                   const pcvs_t *pcvs,
                   const pcvs_t *pcvr,
...
static int outhead(const char *outfile,
                   char **infile,
                   int n,
                   const prcopt_t *popt,
                   const solopt_t *sopt) {
...
static int execses_r(gtime_t ts,
                     gtime_t te,
                     double ti,
                     const prcopt_t *popt,
                     const solopt_t *sopt,
  ...
  if (i < n) { /* include rover keywords */
    ...
    for (p = rov_;; p = q + 1) { /* for each rover */
      ...
      if (*p) {
        ...
        /* execute processing session */
        stat =
            execses(ts, te, ti, popt, sopt, fopt, flag, ifile, index, n, ofile);
      }
      if (stat == 1 || !q) break;
    }
    free(rov_);
    for (i = 0; i < n; i++) free(ifile[i]);
  } else {
    /* execute processing session */
    stat =
        execses(ts, te, ti, popt, sopt, fopt, flag, infile, index, n, outfile);
  }
  return stat;
}
/* execute processing session for each base station --------------------------*/
static int execses_b(gtime_t ts,
                     gtime_t te,
                     double ti,
                     const prcopt_t *popt,
                     const solopt_t *sopt,
  ...
  if (i < n) { /* include base station keywords */
    ...
    for (p = base_;; p = q + 1) { /* for each base station */
      ...
      if (*p) {
        ...
        stat = execses_r(
            ts, te, ti, popt, sopt, fopt, flag, ifile, index, n, ofile, rov);
      }
      if (stat == 1 || !q) break;
    }
    free(base_);
    ...
    stat = execses_r(
        ts, te, ti, popt, sopt, fopt, flag, infile, index, n, outfile, rov);
  }
  /* free prec ephemeris and sbas data */
  freepreceph(&navs, &sbss);
...
extern int postpos(gtime_t ts,
  ...
  if (ts.time != 0 && te.time != 0 && tu >= 0.0) {
    ...
    for (i = 0;; i++) { /* for each periods */
      ...
      for (j = k = nf = 0; j < n; j++) {
        ...
        if (ext && (!strcmp(ext, ".rtcm3") || !strcmp(ext, ".RTCM3"))) {
          ...
          nf += reppaths(
              infile[j], ifile + nf, MAXINFILE - nf, tts, ttte, "", "");
        }
    ...
    /* execute processing session */
    stat = execses_b(
        ts, te, ti, popt, sopt, fopt, 1, ifile, index, n, ofile, rov, base);
    ...
    /* execute processing session */
    stat = execses_b(
        ts, te, ti, popt, sopt, fopt, 1, infile, index, n, outfile, rov, base);
  }
