                      ...
                      const solopt_t *sopt)
{
    ...
    if (sopt->outhead) {
        if (!*sopt->prog) {
            fprintf(fp,"%c program   : RTKLIB ver.%s\n",COMMENTH,VER_RTKLIB);
        }
        else {
            fprintf(fp,"%c program   : %s\n",COMMENTH,sopt->prog);
        }
        for (i=0;i<n;i++) {
            fprintf(fp,"%c inp file  : %s\n",COMMENTH,file[i]);
        }
