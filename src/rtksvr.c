...
/* update ephemeris ----------------------------------------------------------*/
static void update_eph(
    rtksvr_t *svr, nav_t *nav, int ephsat, int ephset, int index) {
...
static void update_svr(rtksvr_t *svr,
                       int ret,
                       obs_t *obs,
                       nav_t *nav,
                       int ephsat,
                       ...
                       int iobs) {
  tracet(4,
         "updatesvr: ret=%d ephsat=%d ephset=%d index=%d\n",
         ret,
         ephsat,
         ephset,
         index);
...
static void send_nmea(rtksvr_t *svr, uint32_t *tickreset) {
  ...
  if (svr->nmeareq == 1) { /* lat-lon-hgt mode */
    ...
    if (bl >= svr->bl_reset && (int)(tick - *tickreset) > MIN_INT_RESET) {
      ...
      tracet(2,
             "send reset: bl=%.3f rr=%.3f %.3f %.3f rb=%.3f %.3f %.3f\n",
             bl,
             svr->rtk.sol.rr[0],
             svr->rtk.sol.rr[1],
             svr->rtk.sol.rr[2],
 ...
 * return : status (1:ok 0:error)
 *-----------------------------------------------------------------------------*/
extern int rtksvropenstr(rtksvr_t *svr,
                         int index,
                         int str,
                         const char *path,
 ...
 *-----------------------------------------------------------------------------*/
extern int rtksvrostat(rtksvr_t *svr,
                       int rcv,
                       gtime_t *time,
                       int *sat,
                       double *az,
...
extern int rtksvrmark(rtksvr_t *svr, const char *name, const char *comment) {
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (svr->solopt[i].posf == SOLF_STAT) {
      ...
      p += sprintf(p,
                   "$GPTXT,01,01,02,MARK:%s,%s,%.9f,%.9f,%.4f,%d,%s",
                   name,
                   tstr,
                   pos[0] * R2D,
                   pos[1] * R2D,
