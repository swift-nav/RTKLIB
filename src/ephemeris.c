 ...
 *Document Navigational radiosignal In bands L1, L2, (Version 5.1), 2008 [3]
 *RTCA/DO-229C, Minimum operational performance standards for global positioning
 ...
 *Interface Control Document, Issue 1.3, December, 2016 [8] Quasi-Zenith
 *Satellite System Interface Specification Satellite Positioning, Navigation and
 *Timing Service (IS-QZSS-PNT-003), Cabinet Office, November 5, 2018 [9] BeiDou
 *navigation satellite system signal in space interface control document open
 *service signal B1I (version 3.0), China Satellite Navigation office, February,
 *2019 [10] RTCM Standard 10403.3, Differential GNSS (Global Navigation
...
extern void eph2pos(
  ...
  trace(4,
        "kepler: sat=%2d e=%8.5f n=%2d del=%10.3e\n",
        eph->sat,
        eph->e,
        n,
        E - Ek);
...
static eph_t *seleph(gtime_t time, int sat, int iode, const nav_t *nav) {
  ...
  if (iode >= 0 || j < 0) {
    trace(3,
          "no broadcast ephemeris: %s sat=%2d iode=%3d\n",
          time_str(time, 0),
          sat,
          iode);
    return NULL;
...
static geph_t *selgeph(gtime_t time, int sat, int iode, const nav_t *nav) {
  ...
  trace(
      4, "selgeph : time=%s sat=%2d iode=%2d\n", time_str(time, 3), sat, iode);
  ...
  if (iode >= 0 || j < 0) {
    trace(3,
          "no glonass ephemeris  : %s sat=%2d iode=%2d\n",
          time_str(time, 0),
          sat,
          iode);
    return NULL;
...
static int ephclk(
    gtime_t time, gtime_t teph, int sat, const nav_t *nav, double *dts) {
  eph_t *eph;
  geph_t *geph;
  seph_t *seph;
  int sys;
...
static int ephpos(gtime_t time,
                  gtime_t teph,
                  int sat,
                  const nav_t *nav,
                  int iode,
...
static int satpos_sbas(gtime_t time,
                       gtime_t teph,
                       int sat,
                       const nav_t *nav,
                       double *rs,
...
static int satpos_ssr(gtime_t time,
                      gtime_t teph,
                      int sat,
                      const nav_t *nav,
                      int opt,
  ...
  if (ssr->iod[0] != ssr->iod[1]) {
    trace(2,
          "inconsist ssr correction: %s sat=%2d iod=%d %d\n",
          time_str(time, 0),
          sat,
          ssr->iod[0],
          ssr->iod[1]);
  ...
  if (fabs(t1) > MAXAGESSR || fabs(t2) > MAXAGESSR) {
    trace(2,
          "age of ssr error: %s sat=%2d t=%.0f %.0f\n",
          time_str(time, 0),
          sat,
          t1,
          t2);
  ...
  if (norm(deph, 3) > MAXECORSSR || fabs(dclk) > MAXCCORSSR) {
    trace(3,
          "invalid ssr correction: %s deph=%.1f dclk=%.1f\n",
          time_str(time, 0),
          norm(deph, 3),
          dclk);
    *svh = -1;
 ...
 *-----------------------------------------------------------------------------*/
extern void satposs(gtime_t teph,
                    const obsd_t *obs,
                    int n,
                    const nav_t *nav,
                    int ephopt,
