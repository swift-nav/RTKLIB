...
/* output track --------------------------------------------------------------*/
static void outtrack(FILE *f,
                     const solbuf_t *solbuf,
                     const char *color,
                     int outalt,
                     int outtime) {
  double pos[3];
...
static void outpoint(FILE *fp,
  ...
  if (outtime) {
    ...
    sprintf(str,
            "%04.0f-%02.0f-%02.0fT%02.0f:%02.0f:%05.2fZ",
            ep[0],
            ep[1],
            ep[2],
            ep[3],
  ...
  fprintf(fp,
          "<coordinates>%13.9f,%12.9f,%5.3f</coordinates>\n",
          pos[1] * R2D,
          pos[0] * R2D,
          alt);
  fprintf(fp, "</Point>\n");
...
static void outcircle(FILE *fp,
  ...
  for (k = 0; k < num_points; k++) {
    ...
    xyz[2] += pos_ecef_m[2];
    ecef2pos(xyz, llh);
    fprintf(fp, "%13.9f,%12.9f,%5.3f\n", llh[1] * R2D, llh[0] * R2D, 0.0);
  }
  fprintf(fp, "        </coordinates>\n");
  fprintf(fp, "      </LinearRing>\n");
  fprintf(fp, "    </outerBoundaryIs>\n");
...
/* save kml file -------------------------------------------------------------*/
static int savekml(const char *file,
                   const solbuf_t *solbuf,
                   int tcolor,
                   int pcolor,
                   int outalt,
  ...
  if (pcolor > 0) {
    ...
    for (i = 0; i < solbuf->n; i++) {
      ...
      /* printf("solbuf->data[i].qr %.1f %.1f %.1f  %.1f %.1f %.1f radius_m
        %.3f\n", solbuf->data[i].qr[0], solbuf->data[i].qr[1],
        solbuf->data[i].qr[2], solbuf->data[i].qr[3], solbuf->data[i].qr[4],
        solbuf->data[i].qr[5], radius_m); */
      outcircle(fp,
                solbuf->data[i].rr,
                radius_m,
                pcolor == 5 ? qcolor[solbuf->data[i].stat] : pcolor - 1);
 ...
 * args   : char   *infile   I   input solutions file (wild-card (*) is
 *expanded) char   *outfile  I   output google earth kml file ("":<infile>.kml)
