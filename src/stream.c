...
static serial_t *openserial(const char *path, int mode, char *msg) {
  ...
  sprintf(
      dcb, "baud=%d parity=%c data=%d stop=%d", brate, parity, bsize, stopb);
...
static void decodetcppath(const char *path,
                          char *addr,
                          char *port,
                          char *user,
                          char *passwd,
...
static int setsock(socket_t sock, char *msg) {
  ...
  if (setsockopt(
          sock, SOL_SOCKET, SO_RCVTIMEO, (const char *)&tv, sizeof(tv)) == -1 ||
      setsockopt(
          sock, SOL_SOCKET, SO_SNDTIMEO, (const char *)&tv, sizeof(tv)) == -1) {
  ...
  if (setsockopt(sock, SOL_SOCKET, SO_RCVBUF, (const char *)&bs, sizeof(bs)) ==
      ...
      setsockopt(sock, SOL_SOCKET, SO_SNDBUF, (const char *)&bs, sizeof(bs)) ==
          -1) {
    tracet(1,
           "setsock: setsockopt error 2 sock=%d err=%d bs=%d\n",
           sock,
           errsock(),
           bs);
  ...
  if (setsockopt(
          sock, IPPROTO_TCP, TCP_NODELAY, (const char *)&mode, sizeof(mode)) ==
      -1) {
...
/* non-block accept ----------------------------------------------------------*/
static socket_t accept_nb(socket_t sock,
...
static int gentcp(tcp_t *tcp, int type, char *msg) {
  ...
  if (type == 0) { /* server socket */
    ...
    setsockopt(
        tcp->sock, SOL_SOCKET, SO_REUSEADDR, (const char *)&opt, sizeof(opt));
#endif
    ...
    if (!(hp = gethostbyname(tcp->saddr))) {
      ...
      tracet(1,
             "gentcp: gethostbyname error addr=%s err=%d\n",
             tcp->saddr,
             errsock());
      closesocket(tcp->sock);
      tcp->state = 0;
...
static int accsock(tcpsvr_t *tcpsvr, char *msg) {
  ...
  tracet(3,
         "accsock: connected sock=%d addr=%s i=%d\n",
         tcpsvr->cli[i].sock,
         tcpsvr->cli[i].saddr,
         i);
...
static int readtcpsvr(tcpsvr_t *tcpsvr, uint8_t *buff, int n, char *msg) {
  ...
  for (i = 0; i < MAXCLI; i++) {
    ...
    if ((nr = recv_nb(tcpsvr->cli[i].sock, buff, n)) == -1) {
      if ((err = errsock())) {
        tracet(2,
               "readtcpsvr: recv error sock=%d err=%d\n",
               tcpsvr->cli[i].sock,
               err);
      }
...
static int writetcpsvr(tcpsvr_t *tcpsvr, uint8_t *buff, int n, char *msg) {
  ...
  for (i = 0; i < MAXCLI; i++) {
    ...
    if ((ns = send_nb(tcpsvr->cli[i].sock, buff, n)) == -1) {
      if ((err = errsock())) {
        tracet(2,
               "writetcpsvr: send error i=%d sock=%d err=%d\n",
               i,
               tcpsvr->cli[i].sock,
               err);
      }
...
static int consock(tcpcli_t *tcpcli, char *msg) {
  ...
  /* non-block connect */
  if ((stat = connect_nb(tcpcli->svr.sock,
                         (struct sockaddr *)&tcpcli->svr.addr,
  ...
  tracet(3,
         "consock: connected sock=%d addr=%s\n",
         tcpcli->svr.sock,
         tcpcli->svr.saddr);
  tcpcli->svr.state = 2;
...
static int readtcpcli(tcpcli_t *tcpcli, uint8_t *buff, int n, char *msg) {
  ...
  if ((nr = recv_nb(tcpcli->svr.sock, buff, n)) == -1) {
    if ((err = errsock())) {
      tracet(
          2, "readtcpcli: recv error sock=%d err=%d\n", tcpcli->svr.sock, err);
...
static int writetcpcli(tcpcli_t *tcpcli, uint8_t *buff, int n, char *msg) {
  ...
  tracet(3,
         "writetcpcli: sock=%d state=%d n=%d\n",
         tcpcli->svr.sock,
         tcpcli->svr.state,
         n);
...
static int reqntrip_s(ntrip_t *ntrip, char *msg) {
  ...
  tracet(
      3, "reqntrip_s: send request state=%d ns=%d\n", ntrip->state, p - buff);
...
static int reqntrip_c(ntrip_t *ntrip, char *msg) {
  ...
  tracet(
      3, "reqntrip_c: send request state=%d ns=%d\n", ntrip->state, p - buff);
...
static int waitntrip(ntrip_t *ntrip, char *msg) {
  ...
  if (ntrip->state == 1) { /* read response */
    ...
    if ((n = readtcpcli(
             ntrip->tcp, (uint8_t *)p, NTRIP_MAXRSP - ntrip->nb - 1, msg)) ==
        0) {
...
static ntrip_t *openntrip(const char *path, int type, char *msg) {
  ...
  decodetcppath(
      path, addr, port, ntrip->user, ntrip->passwd, ntrip->mntpnt, ntrip->str);
...
static void wait_ntripc(ntripc_t *ntripc, char *msg) {
  ...
  for (i = 0; i < MAXCLI; i++) {
    ...
    if ((n = recv_nb(ntripc->tcp->cli[i].sock, buff, nmax)) == -1) {
      if ((err = errsock())) {
        tracet(2,
               "wait_ntripc: recv error sock=%d err=%d\n",
               ntripc->tcp->cli[i].sock,
               err);
      }
      discon_ntripc(ntripc, i);
      continue;
...
static int readntripc(ntripc_t *ntripc, uint8_t *buff, int n, char *msg) {
  ...
  for (i = 0; i < MAXCLI; i++) {
    ...
    if (nr < 0) {
      if ((err = errsock())) {
        tracet(2,
               "readntripc: recv error i=%d sock=%d err=%d\n",
               i,
               ntripc->tcp->cli[i].sock,
               err);
      }
...
static int writentripc(ntripc_t *ntripc, uint8_t *buff, int n, char *msg) {
  ...
  for (i = 0; i < MAXCLI; i++) {
    ...
    if (ns < n) {
      if ((err = errsock())) {
        tracet(2,
               "writentripc: send error i=%d sock=%d err=%d\n",
               i,
               ntripc->tcp->cli[i].sock,
               err);
      }
...
static udp_t *genudp(int type, int port, const char *saddr, char *msg) {
  ...
  if (setsockopt(
          udp->sock, SOL_SOCKET, SO_RCVBUF, (const char *)&bs, sizeof(bs)) ==
          -1 ||
      setsockopt(
          udp->sock, SOL_SOCKET, SO_SNDBUF, (const char *)&bs, sizeof(bs)) ==
          -1) {
    tracet(2,
           "genudp: setsockopt error sock=%d err=%d bs=%d\n",
           udp->sock,
           errsock(),
           bs);
    sprintf(msg, "sockopt error: bufsiz");
  ...
  if (!udp->type) { /* udp server */
...
#ifdef SVR_REUSEADDR
    setsockopt(
        udp->sock, SOL_SOCKET, SO_REUSEADDR, (const char *)&opt, sizeof(opt));
#endif
    if (bind(udp->sock, (struct sockaddr *)&udp->addr, sizeof(udp->addr)) ==
        -1) {
      tracet(2,
             "genudp: bind error sock=%d port=%d err=%d\n",
             udp->sock,
             port,
             errsock());
    ...
    if (!strcmp(saddr, "255.255.255.255") && setsockopt(udp->sock,
                                                        SOL_SOCKET,
                                                        SO_BROADCAST,
                                                        (const char *)&opt,
                                                        sizeof(opt)) == -1) {
      tracet(
          2, "genudp: setsockopt error sock=%d err=%d\n", udp->sock, errsock());
...
static int writeudpcli(udp_t *udpcli, uint8_t *buff, int n, char *msg) {
  ...
  return (int)sendto(udpcli->sock,
                     (char *)buff,
                     n,
                     0,
                     (struct sockaddr *)&udpcli->addr,
                     sizeof(udpcli->addr));
}
...
/* decode ftp path -----------------------------------------------------------*/
static void decodeftppath(const char *path,
                          char *addr,
                          char *file,
                          char *user,
                          char *passwd,
...
static gtime_t nextdltime(const int *topts, int stat) {
  ...
  tracet(3,
         "nextdltime: topts=%d %d %d %d stat=%d\n",
         topts[0],
         topts[1],
         topts[2],
         topts[3],
...
#endif
{
  ...
  if (ftp->proto == 0) { /* ftp */
    ...
    sprintf(
        opt, "%.32s -t 1 -T %d -O \"%.768s\"", proxyopt, FTP_TIMEOUT, local);
    sprintf(cmd,
            "%s%s %s \"http://%s/%s\" 2> \"%.768s\"\n",
            env,
            FTP_CMD,
            opt,
            ftp->addr,
