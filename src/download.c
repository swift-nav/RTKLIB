...
static void genpath(
    const char *file, const char *name, gtime_t time, int seqno, char *path) {
...
static int gen_path(gtime_t time,
                    gtime_t time_p,
                    int seqnos,
                    int seqnoe,
                    const url_t *url,
...
static int gen_paths(gtime_t time,
                     gtime_t time_p,
                     int seqnos,
                     int seqnoe,
                     const url_t *url,
...
static int get_list(const path_t *path,
                    const char *usr,
                    const char *pwd,
                    const char *proxy) {
  FILE *fp;
...
/* execute download ----------------------------------------------------------*/
static int exec_down(path_t *path,
                     char *remot_p,
                     const char *usr,
                     const char *pwd,
                     const char *proxy,
  ...
  if (proto == 0 || proto == 2) {
    ...
    if (*pwd) {
      sprintf(
          opt + strlen(opt), " --http-user=%s --http-password=%s ", usr, pwd);
    }
    sprintf(cmd,
            "%s%s %s %s-t %d -T %d -O \"%s\" -o \"%s\"%s\n",
            env,
            FTP_CMD,
            path->remot,
            opt,
...
/* test local file -----------------------------------------------------------*/
static int test_local(gtime_t ts,
                      gtime_t te,
                      double ti,
                      const char *path,
                      const char *sta,
...
/* test local files ----------------------------------------------------------*/
static int test_locals(gtime_t ts,
                       gtime_t te,
                       double ti,
                       const url_t *url,
                       char **stas,
  ...
  if (strstr(url->path, "%s") || strstr(url->path, "%S")) {
    ...
    if (test_local(
            ts, te, ti, url->path, "", *dir ? dir : url->dir, nc, nt, fp)) {
      return 1;
    }
  }
  return 0;
...
static int print_total(
    const url_t *url, char **stas, int nsta, int *nc, int *nt, FILE *fp) {
  int i;
...
extern int dl_exec(gtime_t ts,
  ...
  while (timediff(ts, te) < 1E-3) {
    for (i = 0; i < nurl; i++) {
      if (!gen_paths(
              ts, ts_p, seqnos, seqnoe, urls + i, stas, nsta, dir, &paths)) {
        free_path(&paths);
  ...
  sprintf(msg,
          "OK=%d No_File=%d Skip=%d Error=%d (Time=%.1f s)",
          n[0],
          n[1],
          n[2],
          n[3],
  ...
  for (; timediff(ts, te) < 1E-3 && !abort; ts = timeadd(ts, ti * ncol)) {
    ...
    *date_p = '\0';
    ...
    for (i = j = 0; i < nurl && !abort; i++) {
      ...
      /* test local files */
      abort = test_locals(
          ts, time, ti, urls + i, stas, nsta, dir, nc + j, nt + j, fp);
