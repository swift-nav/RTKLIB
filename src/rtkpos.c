...
static void outsolstat(rtk_t *rtk) {
  ...
  for (i = 0; i < MAXSAT; i++) {
    ...
    for (j = 0; j < nfreq; j++) {
      fprintf(
          fp_stat,
          "$SAT,%d,%.3f,%s,%d,%.1f,%.1f,%.4f,%.4f,%d,%.1f,%d,%d,%d,%d,%d,%d\n",
          week,
          tow,
          id,
          j + 1,
...
static int selsat(const obsd_t *obs,
                  double *azel,
                  int nu,
                  int nr,
                  const prcopt_t *opt,
...
static void detslp_ll(rtk_t *rtk, const obsd_t *obs, int i, int rcv) {
  ...
  for (f = 0; f < rtk->opt.nf; f++) {
    if (obs[i].L[f] == 0.0 ||
        fabs(timediff(obs[i].time, rtk->ssat[sat - 1].pt[rcv - 1][f])) <
            DTTOL) {
      continue;
    }
...
static void detslp_gf(
    rtk_t *rtk, const obsd_t *obs, int i, int j, const nav_t *nav) {
  int k, sat = obs[i].sat;
  double g0, g1;
  ...
  for (k = 1; k < rtk->opt.nf; k++) {
    ...
    if (g0 != 0.0 && fabs(g1 - g0) > rtk->opt.thresslip) {
      ...
      errmsg(rtk,
             "slip detected GF jump (sat=%2d L1-L%d GF=%.3f %.3f)\n",
             sat,
             k + 1,
             g0,
             g1);
...
static void detslp_dop(
    rtk_t *rtk, const obsd_t *obs, int i, int rcv, const nav_t *nav) {
...
static void zdres_sat(int base,
                      double r,
                      const obsd_t *obs,
                      const nav_t *nav,
                      const double *azel,
  ...
  if (opt->ionoopt == IONOOPT_IFLC) { /* iono-free linear combination */
    ...
    if (obs->L[0] != 0.0 && obs->L[1] != 0.0) {
      y[0] = C1 * obs->L[0] * CLIGHT / freq1 + C2 * obs->L[1] * CLIGHT / freq2 -
             r - dant_if;
    }
...
static int zdres(int base,
  ...
  for (i = 0; i < n; i++) {
    trace(4,
          "sat=%2d %13.3f %13.3f %13.3f %13.10f %6.1f %5.1f\n",
          obs[i].sat,
          rs[i * 6],
          rs[1 + i * 6],
          rs[2 + i * 6],
...
static void ddcov(const int *nb,
                  int n,
                  const double *Ri,
                  const double *Rj,
                  int nv,
...
static int constbl(rtk_t *rtk,
  ...
  trace(4,
        "baseline len   v=%13.3f R=%8.6f %8.6f\n",
        v[index],
        Ri[index],
        Rj[index]);
...
static double prectrop(gtime_t time,
                       const double *pos,
                       int r,
                       const double *azel,
                       const prcopt_t *opt,
...
static int ddres(rtk_t *rtk,
  ...
  int i, j, k, m, f, nv = 0, nb[NFREQ * 4 * 2 + 2] = {0}, b = 0, sysi, sysj,
                     nf = NF(opt);
  ...
  for (i = 0; i < ns; i++) {
    ...
    if (opt->tropopt >= TROPOPT_EST) {
      tropu[i] = prectrop(
          rtk->sol.time, posu, 0, azel + iu[i] * 2, opt, x, dtdxu + i * 3);
      tropr[i] = prectrop(
          rtk->sol.time, posr, 1, azel + ir[i] * 2, opt, x, dtdxr + i * 3);
    }
  }
    ...
    for (f = opt->mode > PMODE_DGPS ? 0 : nf; f < nf * 2; f++) {
      ...
      for (j = 0; j < ns; j++) {
        ...
        if (opt->maxinno > 0.0 && fabs(v[nv]) > opt->maxinno) {
          ...
          errmsg(rtk,
                 "outlier rejected (sat=%3d-%3d %s%d v=%.3f)\n",
                 sat[i],
                 sat[j],
                 f < nf ? "L" : "P",
                 f % nf + 1,
...
static double intpres(gtime_t time,
                      const obsd_t *obs,
                      int n,
                      const nav_t *nav,
                      rtk_t *rtk,
...
static void restamb(rtk_t *rtk, const double *bias, int nb, double *xa) {
  int i, n, m, f, index[MAXSAT] = {0}, index0[MAXSAT] = {0}, nv = 0,
                  nf = NF(&rtk->opt);
  ...
  for (m = 0; m < 5; m++)
    for (f = 0; f < nf; f++) {
      ...
      if (rtk->opt.modear == ARMODE_WL) {
        for (i = 1; i < n; i++) {
          if (f > 0 && index0[0] != 0 && index0[i] != 0 && index[0] != 0 &&
              index[i] != 0) {
            xa[index[i]] =
                bias[nv++] - (xa[index0[0]] - xa[index0[i]] - xa[index[0]]);
          }
        }
      } else {
...
static void holdamb(rtk_t *rtk, const double *xa) {
  double *v, *H, *R;
  int i, n, m, f, info, index[MAXSAT] = {0}, index0[MAXSAT] = {0},
                        nb = rtk->nx - rtk->na;
...
static int dd_wl_mat(rtk_t *rtk, double *D) {
  ...
  for (m = 0; m < 5; m++) { /* m=0:gps/sbs,1:glo,2:gal,3:bds,4:qzs */
    ...
    f = nf == 2 ? 1 : 2;
    for (i = k; i < k + MAXSAT; i++) {
      if (rtk->x[i] == 0.0 || rtk->x[i + f * MAXSAT] == 0.0 ||
          !test_sys(rtk->ssat[i - k].sys, m) || !rtk->ssat[i - k].vsat[0] ||
          !rtk->ssat[i - k].half[0] || !rtk->ssat[i - k].vsat[f] ||
          !rtk->ssat[i - k].half[f]) {
        continue;
      }
      if (rtk->ssat[i - k].lock[0] > 0 && rtk->ssat[i - k].lock[f] > 0 &&
          !(rtk->ssat[i - k].slip[f] & 2) && !(rtk->ssat[i - k].slip[0] & 2) &&
    ...
    for (j = k; j < k + MAXSAT; j++) {
      if (i == j || rtk->x[j] == 0.0 || rtk->x[j + f * MAXSAT] == 0.0 ||
          !test_sys(rtk->ssat[j - k].sys, m) || !rtk->ssat[j - k].vsat[0] ||
          !rtk->ssat[j - k].vsat[f]) {
        continue;
      }
      if (rtk->ssat[j - k].lock[0] > 0 && rtk->ssat[j - k].lock[f] > 0 &&
          !(rtk->ssat[j - k].slip[0] & 2) && !(rtk->ssat[j - k].slip[f] & 2) &&
...
static int resamb_WL(rtk_t *rtk, double *bias, double *xa) {
  ...
  if (rtk->opt.modear != ARMODE_WL || rtk->opt.thresar[0] < 1.0 ||
      rtk->opt.nf < 2) {
    return 0;
  }
  /* single to double-differenced wide-lane transformation matrix (D') */
...
static int resamb_LAMBDA(rtk_t *rtk, double *bias, double *xa) {
  ...
  for (j = 0; j < nb; j++)
    for (i = 0; i < nb; i++) {
      Qb[i + j * nb] =
          DP[i + (ix[j * 2] - na) * nb] - DP[i + (ix[j * 2 + 1] - na) * nb];
    }
  for (j = 0; j < nb; j++)
    for (i = 0; i < na; i++) {
      Qab[i + j * na] =
          rtk->P[i + ix[j * 2] * nx] - rtk->P[i + ix[j * 2 + 1] * nx];
    }
...
static int relpos(
    rtk_t *rtk, const obsd_t *obs, int nu, int nr, const nav_t *nav) {
  prcopt_t *opt = &rtk->opt;
  ...
  for (i = 0; i < niter; i++) {
    ...
    if (!zdres(
            0, obs, nu, rs, dts, var, svh, nav, xp, opt, 0, y, e, azel, freq)) {
  ...
  /* resolve integer ambiguity by LAMBDA */
    ...
    if (zdres(
            0, obs, nu, rs, dts, var, svh, nav, xa, opt, 0, y, e, azel, freq)) {
 ...
 *                .dtr[1-5]  O   receiver GLO/GAL/BDS/IRN/QZS-GPS time offset
 *(s) .Qr[]      O   rover position covarinace .stat      O   solution status
 *(SOLQ_???) .ns        O   number of valid satellites .age       O   age of
 *differential (s) .ratio     O   ratio factor for ambiguity validation
 ...
 *satellite {s+1} status .sys       O   system (SYS_???) .az   [r]  O   azimuth
 *angle   (rad) (r=0:rover,1:base) .el   [r]  O   elevation angle (rad)
