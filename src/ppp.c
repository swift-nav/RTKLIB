  ...
  if (rtk->opt.ionoopt == IONOOPT_EST) {
    for (i = 0; i < MAXSAT; i++) {
      ...
      p += sprintf(p,
                   ...
                   rtk->sol.stat,
                   id,
                   rtk->ssat[i].azel[0] * R2D,
                   rtk->ssat[i].azel[1] * R2D,
                   x[j],
                   STD(rtk, j));
    }
  }
...
static void testeclipse(const obsd_t *obs,
                        int n,
                        const nav_t *nav,
                        double *rs) {
  ...
  for (i = 0; i < n; i++) {
    ...
    trace(3,
          "eclipsing sat excluded %s sat=%2d\n",
...
extern int yaw_angle(
    int sat, const char *type, int opt, double beta, double mu, double *yaw) {
  *yaw = yaw_nominal(beta, mu);
  return 1;
}
/* satellite attitude model --------------------------------------------------*/
static int sat_yaw(gtime_t time,
                   int sat,
                   const char *type,
                   int opt,
                   const double *rs,
...
/* phase windup model --------------------------------------------------------*/
static int model_phw(gtime_t time,
                     int sat,
                     const char *type,
                     int opt,
                     const double *rs,
...
static double varerr(
    int sat, int sys, double el, int idx, int type, const prcopt_t *opt) {
...
static void corr_meas(const obsd_t *obs,
                      const nav_t *nav,
                      const double *azel,
                      const prcopt_t *opt,
                      const double *dantr,
...
static void detslp_gf(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav) {
  ...
  for (i = 0; i < n && i < MAXOBS; i++) {
    ...
    if (g0 != 0.0 && fabs(g1 - g0) > rtk->opt.thresslip) {
      trace(3,
            "detslip_gf: slip detected sat=%2d gf=%8.3f->%8.3f\n",
            obs[i].sat,
            g0,
            g1);
...
static void detslp_mw(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav) {
  ...
  for (i = 0; i < n && i < MAXOBS; i++) {
    ...
    if (w0 != 0.0 && fabs(w1 - w0) > THRES_MW_JUMP) {
      trace(3,
            "detslip_mw: slip detected sat=%2d mw=%8.3f->%8.3f\n",
            obs[i].sat,
            w0,
            w1);
...
static void udiono_ppp(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav) {
  ...
  for (i = 0; i < n; i++) {
    ...
    if (rtk->x[j] == 0.0) {
      ...
      if (obs[i].P[0] == 0.0 || obs[i].P[1] == 0.0 || freq1 == 0.0 ||
          freq2 == 0.0) {
        continue;
      }
      ion = (obs[i].P[0] - obs[i].P[1]) /
            (SQR(FREQ1 / freq1) - SQR(FREQ1 / freq2));
      ecef2pos(rtk->sol.rr, pos);
...
static void udbias_ppp(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav) {
  ...
  for (f = 0; f < NF(&rtk->opt); f++) {
    ...
    if (k >= 2 && fabs(offset / k) > 0.0005 * CLIGHT) {
      ...
      trace(2,
            "phase-code jump corrected: %s n=%2d dt=%12.9fs\n",
 ...
 * --------------------------------------------------*/
static void udstate_ppp(rtk_t *rtk,
                        const obsd_t *obs,
                        int n,
                        const nav_t *nav) {
...
static void satantpcv(const double *rs,
                      const double *rr,
                      const pcv_t *pcv,
                      double *dant) {
...
static double trop_model_prec(gtime_t time,
                              const double *pos,
                              const double *azel,
                              const double *x,
                              double *dtdx,
...
/* tropospheric model
 * ---------------------------------------------------------*/
static int model_trop(gtime_t time,
                      const double *pos,
                      const double *azel,
                      const prcopt_t *opt,
                      const double *x,
...
/* ionospheric model ---------------------------------------------------------*/
static int model_iono(gtime_t time,
                      const double *pos,
                      const double *azel,
                      const prcopt_t *opt,
                      int sat,
...
static int ppp_res(int post,
  ...
  double y, r, cdtr, bias, C = 0.0, rr[3], pos[3], e[3], dtdx[3], L[NFREQ],
                           P[NFREQ], Lc, Pc;
  ...
  for (i = 0; i < n && i < MAXOBS; i++) {
    ...
    if (!model_trop(
            obs[i].time, pos, azel + i * 2, opt, x, dtdx, nav, &dtrp, &vart) ||
        !model_iono(
            obs[i].time, pos, azel + i * 2, opt, sat, x, nav, &dion, &vari)) {
      continue;
    }
    ...
    for (j = 0; j < 2 * NF(opt); j++) {
      ...
      /* residual */
      v[nv] =
          y - (r + cdtr - CLIGHT * dts[i * 2] + dtrp + C * dion + dcb + bias);
      ...
      trace(4,
            "%s sat=%2d %s%d res=%9.4f sig=%9.4f el=%4.1f\n",
            str,
            sat,
            j % 2 ? "P" : "L",
            j / 2 + 1,
      ...
      if (!post && opt->maxinno > 0.0 && fabs(v[nv]) > opt->maxinno) {
        trace(2,
              "outlier (%d) rejected %s sat=%2d %s%d res=%9.4f el=%4.1f\n",
              post,
              str,
              sat,
              j % 2 ? "P" : "L",
...
extern void pppos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav) {
  ...
  if (stat == SOLQ_PPP) {
    ...
    if (ppp_ar(rtk, obs, n, exc, nav, azel, xp, Pp) && ppp_res(9,
                                                               obs,
                                                               n,
                                                               rs,
                                                               dts,
