 ...
 *                           add quality test for receiver time in
 *decode_trkd5() add half cycle shift correction for BDS GEO delete receiver
 *option -GALFNAV use API code2idx() and code2freq() support QZSS L1S (CODE_L1Z)
...
static int decode_rxmrawx(raw_t *raw) {
  ...
  if (raw->outtype) {
    ...
    sprintf(raw->msgtype,
            "UBX RXM-RAWX  (%4d): time=%s nmeas=%d ver=%d",
            raw->len,
            tstr,
            nmeas,
            ver);
...
extern int gen_ubx(const char *msg, uint8_t *buff) {
  const char *cmd[] = {
      ...
      "VALGET", "VALSET",    ""};
  const uint8_t id[] = {0x00, 0x1B, 0x01, 0x17, 0x08, 0x09, 0x07, 0x1A,
                        0x06, 0x02, 0x04, 0x11, 0x13, 0x0E, 0x16, 0x80,
                        0x10, 0x19, 0x1D, 0x12, 0x3E, 0x39, 0x47, 0x24,
                        0x23, 0x1E, 0x3B, 0x57, 0x34, 0x62, 0x36, 0x71,
                        0x31, 0x53, 0x8c, 0x8b, 0x8a};
  const int prm[][32] = {
