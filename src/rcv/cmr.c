...
| See reference #3.
*/
static const unsigned char SnrTable[][2] = {{0, 0},
                                            {30, 4},
                                            {32, 8},
                                            {34, 12},
                                            {36, 16},
...
static int ReferenceCmrObs(
    raw_t *Raw, gtime_t Time, unsigned char Type, double P0, obsbd_t *Obs);
static gtime_t ReferenceCmrTime(gtime_t CmrTime,
                                gtime_t RoverTime,
                                double WindowSize);
...
static size_t TrimCopy(char *Destination,
                       size_t DestinationLength,
                       char *Source,
                       size_t SourceLength);
static unsigned int ubitn(const unsigned char *Address,
                          int BitPosition,
                          int BitLength);
...
static int CheckStation(raw_t *Raw, unsigned int StationID) {
  ...
  if ((p = strstr(Raw->opt, "-STA=")) && sscanf(p, "-STA=%u", &ID) == 1) {
    if (StationID != ID) {
      tracet(2,
             "CMR: Message with wrong Base Station ID (%d) ignored.\n",
             StationID);
      return 0;
    }
  }
  ...
  */
  if (!Cmr->StationID)
    ...
    tracet(2,
           "CMR: Base Station ID changed from %d to %d.\n",
           Cmr->StationID,
           StationID);
...
static int DecodeCmr(raw_t *Raw) {
  ...
  */
  if ((Version > 3) && !((Version == 4) && (Type == 4))) {
    tracet(
        2, "CMR: Unsupported CMR type %u message version: %u\n", Type, Version);
    return -1;
  }
  ...
  if (((Page == Pages) && Cmr->BufferBytes) || ((Page == 0) && (Pages == 0))) {
    ...
    */
    if ((Page != 0) && ((Cmr->BufferBytes + Length) > BUFFER_LENGTH)) {
      tracet(2,
             "CMR: Buffer would overflow. %d CMR+ messages discarded.\n",
             Pages + 1);
...
static int DecodeBuffer(raw_t *Raw) {
  ...
  while (BufferBytes > 0) {
    ...
    tracet(3,
           "CMR: CMR+ Message type=%u (%s), Length=%u.\n",
           Type,
           Type_s,
           Length);
...
static int ReferenceCmrObs(
    raw_t *Raw, gtime_t Time, unsigned char Type, double P0, obsbd_t *b) {
  cmr_t *Cmr = (cmr_t *)Raw->rcv_data;
  obsbd_t *t4 = (obsbd_t *)Cmr->T4Data;
  rtksvr_t *Svr = Cmr->Svr;
  nav_t *Nav = (Svr) ? &Svr->nav : &Raw->nav;
...
static void SetStationDescription(raw_t *Raw, unsigned char *p, size_t Length) {
  ...
  tracet(3,
         "CMR: Reference station decription received. STATION=\"%s\"\n",
         sta->name);
}
