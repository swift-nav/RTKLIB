 ...
 *                           support NavIC/IRNSS
 *                           support GPS/QZS L1C, GLO L3, GAL E6, QZS L6, BDS
 *B3, B1C, B2a, B2b support message NAVICEPHEMERISB support QZS L1S in RANGEB
 *and RANGECMPB no support message GALALMANACB add receiver option
 *-GL1L,-GL2S,-GL2P,-EL6B,-JL1L, -JL1Z,-CL1P,-CL7D,-GLOBIAS=bias delete receiver
 *option -GL1P,-GL2X,EL2C fix bug on reading SVH in GLOEPHEMERISB add reading of
 *dtaun field in GLOEPHEMERISB output GAL I/NAV or F/NAV to seperated ephem sets
 ...
 * notes  : refer [1][3]
 *-----------------------------------------------------------------------------*/
static int decode_track_stat(uint32_t stat,
                             int *sys,
                             int *code,
                             int *track,
                             int *plock,
...
static int decode_rangecmpb(raw_t *raw) {
  ...
  for (i = 0, p += 4; i < nobs; i++, p += 24) {
    if ((idx = decode_track_stat(
             U4(p), &sys, &code, &track, &plock, &clock, &parity, &halfc)) <
        0) {
      continue;
    }
    prn = U1(p + 17);
    ...
    if (!(sat = satno(sys, prn))) {
      trace(3,
            "oem4 rangecmpb satellite number error: sys=%d,prn=%d\n",
            sys,
            prn);
      continue;
    }
    ...
    if (raw->tobs[sat - 1][idx].time != 0) {
      ...
      lli = (lockt < 65535.968 && lockt - raw->lockt[sat - 1][idx] + 0.05 <= tt)
                ? LLI_SLIP
                : 0;
    } else {
      lli = 0;
...
static int decode_rangeb(raw_t *raw) {
  ...
  for (i = 0, p += 4; i < nobs; i++, p += 44) {
    if ((idx = decode_track_stat(U4(p + 40),
                                 &sys,
                                 &code,
                                 &track,
                                 &plock,
...
static int decode_rawwaasframeb(raw_t *raw) {
  ...
  if ((prn < MINPRNSBS || prn > MAXPRNSBS) &&
      (prn < MINPRNQZS_S || prn > MAXPRNQZS_S)) {
    return 0;
  }
  if (raw->outtype) {
    sprintf(raw->msgtype + strlen(raw->msgtype), " prn=%d", prn);
...
static int decode_galephemerisb(raw_t *raw) {
  ...
  double tow, sqrtA, af0_fnav, af1_fnav, af2_fnav, af0_inav, af1_inav, af2_inav,
      tt;
  int prn, sat, week, rcv_fnav, rcv_inav, svh_e1b, svh_e5a, svh_e5b, dvs_e1b,
      dvs_e5a;
...
static int decode_oem4(raw_t *raw) {
  ...
  if (stat == 20 || week == 0) {
    trace(3,
          "oem4 time error: type=%3d msg=%d stat=%d week=%d\n",
          type,
          msg,
          stat,
          week);
