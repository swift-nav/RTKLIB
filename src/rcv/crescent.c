...
static int chksum(const uint8_t *buff, int len) {
  ...
  trace(4,
        "checksum=%02X%02X %02X%02X:%02X%02X\n",
        sum >> 8,
        sum & 0xFF,
        buff[len - 3],
        buff[len - 4],
...
static int decode_crespos(raw_t *raw) {
  ...
  trace(3,
        "$BIN1 %s %13.9f %14.9f %10.4f %4d %3d %.3f\n",
        tstr,
        pos[0],
        pos[1],
        pos[2],
        mode == 6 ? 1 : (mode > 4 ? 2 : (mode > 1 ? 5 : 0)),
        ns,
        std);
  return 0;
}
  ...
  for (i = 0, p += 16; i < 15 && n < MAXOBS; i++) {
    ...
    for (j = 0; j < NFREQ; j++) {
      if (j == 0 || (j == 1 && i < 12)) {
        ...
        raw->obs.data[n].L[j] =
            cp[j] == 0.0 ? 0.0 : cp[j] - toff * freq[j] / CLIGHT;
...
static int decode_cresgloeph(raw_t *raw) {
  ...
  for (i = 0; i < 5; i++) {
    ...
    if ((no = getbitu(str, 1, 4)) != i + 1) {
      trace(2,
            "creasent bin 65 string no error: sat=%2d no=%d %d\n",
            sat,
            i + 1,
            no);
      return -1;
