...
static int is_meas(char sig) {
  return sig == 'c' || sig == 'C' || sig == '1' || sig == '2' || sig == '3' ||
         sig == '5' || sig == 'l';
}
...
static int decode_RD(raw_t *raw) {
  ...
  if (raw->outtype) {
    msg = raw->msgtype + strlen(raw->msgtype);
    sprintf(
        msg, " %04.0f/%02.0f/%02.0f base=%d", ep[0], ep[1], ep[2], raw->tbase);
  }
  if (raw->tod < 0) {
...
static int decode_eph(raw_t *raw, int sys) {
  ...
  if (raw->outtype) {
    msg = raw->msgtype + strlen(raw->msgtype);
    sprintf(msg,
            " prn=%3d iode=%3d iodc=%3d toes=%6.0f",
            prn,
            eph.iode,
            eph.iodc,
            eph.toes);
  ...
  if (sys == SYS_GPS || sys == SYS_QZS || sys == SYS_IRN) {
    ...
    eph.code =
        (sigtype == 0) ? 1 : ((sigtype == 1) ? 3 : ((sigtype == 2) ? 5 : 0));
    eph.week = week;
...
static int decode_NE(raw_t *raw) {
  ...
  if (raw->nav.geph[prn - 1].toe.time &&
      geph.frq != raw->nav.geph[prn - 1].frq) {
    trace(2,
          "javad NE glonass fcn changed: prn=%2d fcn=%2d->%2d\n",
          prn,
          raw->nav.geph[prn - 1].frq,
          geph.frq);
    return -1;
  }
  if (!strstr(raw->opt, "-EPHALL")) {
...
static int decode_L2nav(uint8_t *buff, int len, int sat, raw_t *raw) {
  ...
  trace(3,
        "L2CNAV: sat=%2d prn=%2d msgid=%2d tow=%6d alert=%d\n",
        sat,
        prn,
        msgid,
        tow,
...
static int decode_L5nav(uint8_t *buff, int len, int sat, raw_t *raw) {
  ...
  trace(3,
        "L5CNAV: sat=%2d prn=%2d msgid=%2d tow=%6d alert=%d\n",
        sat,
        prn,
        msgid,
        tow,
...
static int decode_lD(raw_t *raw) {
  ...
  for (i = 0; i < 4; i++) {
    setbitu(raw->subfrm[sat - 1] + (id - 1) * 10,
            i * 25,
            i < 3 ? 25 : 2,
            U4(p + 4 * i) >> (i < 3 ? 0 : 23));
  }
...
static int decode_Rx(raw_t *raw, char sig) {
  ...
  if (raw->len != raw->obuf.n * 8 + 6) {
    trace(
        2, "javad R%c length error: n=%d len=%d\n", sig, raw->obuf.n, raw->len);
    return -1;
  }
  for (i = 0; i < raw->obuf.n && i < MAXOBS; i++) {
    pr = R8(p);
...
static int decode_xR(raw_t *raw, char sig) {
  ...
  if (raw->len != raw->obuf.n * 4 + 6) {
    trace(
        2, "javad %cR length error: n=%d len=%d\n", sig, raw->obuf.n, raw->len);
    return -1;
  }
  for (i = 0; i < raw->obuf.n && i < MAXOBS; i++) {
    pr = R4(p);
...
static int decode_Px(raw_t *raw, char sig) {
  ...
  if (raw->len != raw->obuf.n * 8 + 6) {
    trace(
        2, "javad P%c length error: n=%d len=%d\n", sig, raw->obuf.n, raw->len);
    return -1;
  }
  for (i = 0; i < raw->obuf.n && i < MAXOBS; i++) {
    cp = R8(p);
...
static int decode_xP(raw_t *raw, char sig) {
  ...
  if (raw->len != raw->obuf.n * 4 + 6) {
    trace(
        2, "javad %cP length error: n=%d len=%d\n", sig, raw->obuf.n, raw->len);
    return -1;
  }
  for (i = 0; i < raw->obuf.n && i < MAXOBS; i++) {
    rcp = R4(p);
...
static int decode_Dx(raw_t *raw, char sig) {
  ...
  if (raw->len != raw->obuf.n * 4 + 6) {
    trace(
        2, "javad D%c length error: n=%d len=%d\n", sig, raw->obuf.n, raw->len);
    return -1;
  }
  for (i = 0; i < raw->obuf.n && i < MAXOBS; i++) {
    dp = I4(p);
...
static int decode_xd(raw_t *raw, char sig) {
  ...
  if (raw->len != raw->obuf.n * 2 + 6) {
    trace(
        2, "javad %cd length error: n=%d len=%d\n", sig, raw->obuf.n, raw->len);
    return -1;
  }
  for (i = 0; i < raw->obuf.n && i < MAXOBS; i++) {
    rdp = I2(p);
...
static int decode_Ex(raw_t *raw, char sig) {
  ...
  if (raw->len != raw->obuf.n + 6) {
    trace(
        2, "javad E%c length error: n=%d len=%d\n", sig, raw->obuf.n, raw->len);
    return -1;
  }
  for (i = 0; i < raw->obuf.n && i < MAXOBS; i++) {
    cnr = U1(p);
...
static int decode_xE(raw_t *raw, char sig) {
  ...
  if (raw->len != raw->obuf.n + 6) {
    trace(
        2, "javad %cE length error: n=%d len=%d\n", sig, raw->obuf.n, raw->len);
    return -1;
  }
  for (i = 0; i < raw->obuf.n && i < MAXOBS; i++) {
    cnr = U1(p);
...
static int decode_Fx(raw_t *raw, char sig) {
  ...
  if (raw->len != raw->obuf.n * 2 + 6) {
    trace(
        2, "javad F%c length error: n=%d len=%d\n", sig, raw->obuf.n, raw->len);
    return -1;
  }
  for (i = 0; i < raw->obuf.n && i < MAXOBS; i++) {
    flags = U2(p);
...
static int decode_TC(raw_t *raw) {
  ...
  for (i = 0; i < raw->obuf.n && i < MAXOBS; i++) {
    ...
    if (tt == 0 || tt < tt_p) {
      trace(3,
            "decode_TC: loss-of-lock detected: t=%s sat=%2d tt=%6d->%6d\n",
            time_str(raw->time, 3),
            sat,
            tt_p,
            tt);
