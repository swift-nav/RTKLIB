...
static int decode_stqtime(raw_t *raw) {
  ...
  if (raw->outtype) {
    sprintf(raw->msgtype,
            "SKYTRAQ EPOCH (%4d): iod=%d week=%d tow=%.3f",
            raw->len,
            raw->iod,
            week,
            tow);
...
static int decode_stqrawx(raw_t *raw) {
  ...
  if (raw->outtype) {
    sprintf(
        raw->msgtype, "SKYTRAQ RAWX  (%4d): nsat=%2d", raw->len, U1(p + 13));
  }
  ver = U1(p + 1);
  raw->iod = U1(p + 2);
  week = U2(p + 3);
...
static int decode_stqbds(raw_t *raw) {
  ...
  if (prn > 5) { /* IGSO/MEO */
    ...
    if (pgn < 1 || 10 < pgn) {
      trace(
          2, "stq bds subframe page number error: prn=%2d pgn=%d\n", prn, pgn);
      return -1;
    }
    word = getbitu(p + 3, j, 26) << 4;
    j += 26;
...
static int decode_stqack(raw_t *raw) {
  ...
  if (raw->outtype) {
    sprintf(
        raw->msgtype, "SKYTRAQ ACK   (%4d): msg=0x%02X", raw->len, U1(p + 1));
  }
  return 0;
}
...
static int decode_stqnack(raw_t *raw) {
  ...
  if (raw->outtype) {
    sprintf(
        raw->msgtype, "SKYTRAQ NACK  (%4d): msg=0x%02X", raw->len, U1(p + 1));
  }
  return 0;
}
...
static int decode_stq(raw_t *raw) {
  ...
  if (cs != *p || *(p + 1) != 0x0D || *(p + 2) != 0x0A) {
    trace(2,
          "stq checksum error: type=%02X cs=%02X tail=%02X%02X%02X\n",
          type,
          cs,
          *p,
          *(p + 1),
