...
static int decode_bnx_00(raw_t *raw, uint8_t *buff, int len) {
  ...
  while (p - buff < len) {
    ...
    if (fid <= 0x0c || (fid >= 0x0f && fid <= 0x1c) ||
        (fid >= 0x20 && fid <= 0x22) || fid == 0x7f) {
...
static int decode_bnx_01_04(raw_t *raw, uint8_t *buff, int len) {
  ...
  if (!strstr(raw->opt, "-EPHALL")) {
    if (raw->nav.eph[sat - 1 + MAXSAT * set].iode == eph.iode &&
        fabs(timediff(raw->nav.eph[sat - 1 + MAXSAT * set].toe, eph.toe)) <
            1.0 &&
        fabs(timediff(raw->nav.eph[sat - 1 + MAXSAT * set].toc, eph.toc)) <
            1.0) {
      return 0;
    }
  }
...
static int decode_bnx_01(raw_t *raw, uint8_t *buff, int len) {
  ...
  if (raw->outtype) {
    prn = srec == 0x01 || srec == 0x02 || srec == 0x04
              ...
              : (srec == 0x00 ? 0 : prn);
    sprintf(
        raw->msgtype + strlen(raw->msgtype), " subrec=%02X prn=%d", srec, prn);
  }
  switch (srec) {
    case 0x00:
...
static uint8_t *decode_bnx_7f_05_obs(
    raw_t *raw, uint8_t *buff, int sat, int nobs, obsd_t *data) {
  ...
  for (i = 0; i < nobs; i++) {
    ...
    trace(5,
          "(%d) CODE=%2d S=%d F=%02X %02X %02X %02X\n",
          i + 1,
          code[i],
          slip[i],
          flags[0],
          ...
          flags[3]);
    trace(5,
          "(%d) P=%13.3f L=%13.3f D=%7.1f SNR=%4.1f SCNT=%2d\n",
          i + 1,
          range[i],
          phase[i],
          dopp[i],
