 ...
 *-----------------------------------------------------------------------------*/
static int decode_trackstat(unsigned int stat,
                            int *sys,
                            int *code,
                            int *track,
                            int *plock,
...
static int decode_rangeb(raw_t *raw) {
  ...
  for (i = 0, p += 4; i < nobs; i++, p += 44) {
    ...
    if ((freq = decode_trackstat(U4(p + 40),
                                 &sys,
                                 &code,
                                 &track,
                                 &plock,
    ...
    if (!(sat = satno(sys, prn))) {
      trace(
          3, "tersus rangeb satellite number error: sys=%d,prn=%d\n", sys, prn);
      continue;
    }
...
static int decode_rangecmpb(raw_t *raw) {
  ...
  for (i = 0, p += 4; i < nobs; i++, p += 24) {
    /* decode tracking status */
    if ((freq = decode_trackstat(
             U4(p), &sys, &code, &track, &plock, &clock, &parity, &halfc)) < 0)
      continue;
    ...
    if (!(sat = satno(sys, prn))) {
      trace(3,
            "tersus rangecmpb satellite number error: sys=%d,prn=%d\n",
            sys,
            prn);
      continue;
    }
...
static int sync_tersus(unsigned char *buff, unsigned char data) {
  ...
  buff[1] = buff[2];
  buff[2] = data;
  return buff[0] == TERSUSSYNC1 && buff[1] == TERSUSSYNC2 &&
         buff[2] == TERSUSSYNC3;
}
...
extern int input_tersus(raw_t *raw, unsigned char data) {
  ...
  if (raw->nbyte == 10 &&
      (raw->len = U2(raw->buff + 8) + TERSUSHLEN) > MAXRAWLEN - 4) {
