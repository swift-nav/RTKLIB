...
const uint8_t ssr_sig_gps[32] = {CODE_L1C,
                                 CODE_L1P,
                                 CODE_L1W,
                                 CODE_L1S,
                                 CODE_L1L,
                                 ...
                                 CODE_L5I,
                                 CODE_L5Q};
const uint8_t ssr_sig_glo[32] = {CODE_L1C,
                                 CODE_L1P,
                                 CODE_L2C,
                                 CODE_L2P,
                                 CODE_L4A,
                                 ...
                                 CODE_L3I,
                                 CODE_L3Q};
const uint8_t ssr_sig_gal[32] = {CODE_L1A,
                                 CODE_L1B,
                                 CODE_L1C,
                                 0,
                                 0,
                                 ...
                                 CODE_L6B,
                                 CODE_L6C};
const uint8_t ssr_sig_qzs[32] = {CODE_L1C,
                                 CODE_L1S,
                                 CODE_L1L,
                                 CODE_L2S,
                                 CODE_L2L,
                                 ...
                                 0,
                                 CODE_L6E};
const uint8_t ssr_sig_cmp[32] = {CODE_L2I,
                                 CODE_L2Q,
                                 0,
                                 CODE_L6I,
                                 CODE_L6Q,
...
static int lossoflock(rtcm_t *rtcm, int sat, int idx, int lock) {
  int lli =
      (!lock && !rtcm->lock[sat - 1][idx]) || lock < rtcm->lock[sat - 1][idx];
...
static int decode_type1010(rtcm_t *rtcm) {
  ...
  int i = 24 + 61, j, index, nsat, sync, prn, sat, code, fcn, ppr1, lock1, amb,
      sys = SYS_GLO;
...
static int decode_type1012(rtcm_t *rtcm) {
  ...
  int i = 24 + 61, j, index, nsat, sync, prn, sat, fcn, code1, code2, pr21,
      ppr1, ppr2;
...
static int decode_type1019(rtcm_t *rtcm) {
  ...
  trace(
      4, "decode_type1019: prn=%d iode=%d toe=%.0f\n", prn, eph.iode, eph.toes);
...
static int decode_type1020(rtcm_t *rtcm) {
  ...
  trace(4,
        "decode_type1020: prn=%d tk=%02.0f:%02.0f:%02.0f\n",
        prn,
        tk_h,
        tk_m,
        tk_s);
  ...
  if (rtcm->outtype) {
    msg = rtcm->msgtype + strlen(rtcm->msgtype);
    sprintf(msg,
            " prn=%2d tk=%02.0f:%02.0f:%02.0f frq=%2d bn=%d tb=%d",
            prn,
            tk_h,
            tk_m,
            tk_s,
...
static int decode_type1041(rtcm_t *rtcm) {
  ...
  trace(
      4, "decode_type1041: prn=%d iode=%d toe=%.0f\n", prn, eph.iode, eph.toes);

  if (rtcm->outtype) {
    msg = rtcm->msgtype + strlen(rtcm->msgtype);
    sprintf(msg,
            " prn=%2d iode=%3d week=%d toe=%6.0f toc=%6.0f svh=%02X",
            prn,
            eph.iode,
            week,
            eph.toes,
...
static int decode_type1044(rtcm_t *rtcm) {
  ...
  trace(
      4, "decode_type1044: prn=%d iode=%d toe=%.0f\n", prn, eph.iode, eph.toes);
...
static int decode_type1045(rtcm_t *rtcm) {
  ...
  trace(
      4, "decode_type1045: prn=%d iode=%d toe=%.0f\n", prn, eph.iode, eph.toes);

  if (rtcm->outtype) {
    msg = rtcm->msgtype + strlen(rtcm->msgtype);
    sprintf(msg,
            " prn=%2d iode=%3d week=%d toe=%6.0f toc=%6.0f hs=%d dvs=%d",
            prn,
            eph.iode,
            week,
            eph.toes,
...
static int decode_type1046(rtcm_t *rtcm) {
  ...
  double toc, sqrtA, tt;
  char *msg;
  int i = 24 + 12, prn, sat, week, e5b_hs, e5b_dvs, e1_hs, e1_dvs,
      sys = SYS_GAL;
  ...
  trace(
      4, "decode_type1046: prn=%d iode=%d toe=%.0f\n", prn, eph.iode, eph.toes);

  if (rtcm->outtype) {
    msg = rtcm->msgtype + strlen(rtcm->msgtype);
    sprintf(msg,
            " prn=%2d iode=%3d week=%d toe=%6.0f toc=%6.0f hs=%d %d dvs=%d %d",
            prn,
            eph.iode,
            week,
            eph.toes,
...
static int decode_type1042(rtcm_t *rtcm) {
  ...
  trace(
      4, "decode_type1042: prn=%d iode=%d toe=%.0f\n", prn, eph.iode, eph.toes);
...
static int decode_ssr1_head(rtcm_t *rtcm,
                            int sys,
                            int subtype,
                            int *sync,
                            int *iod,
  ...
  *udint = ssrudint[udi];
  ...
  if (rtcm->outtype) {
    msg = rtcm->msgtype + strlen(rtcm->msgtype);
    sprintf(msg,
            " %s nsat=%2d iod=%2d udi=%2d sync=%d",
            tstr,
            nsat,
            *iod,
            udi,
...
static int decode_ssr2_head(rtcm_t *rtcm,
                            int sys,
                            int subtype,
                            int *sync,
                            int *iod,
...
static int decode_ssr1(rtcm_t *rtcm, int sys, int subtype) {
  ...
  int i, j, k, type, sync, iod, nsat, prn, sat, iode, iodcrc = 0, refd = 0, np,
                                                      ni, nj, offp;

  type = getbitu(rtcm->buff, 24, 12);

  if ((nsat = decode_ssr1_head(
           rtcm, sys, subtype, &sync, &iod, &udint, &refd, &i)) < 0) {
    trace(2, "rtcm3 %d length error: len=%d\n", type, rtcm->len);
    return -1;
  }
  switch (sys) {
...
static int decode_ssr2(rtcm_t *rtcm, int sys, int subtype) {
  ...
  if ((nsat = decode_ssr2_head(rtcm, sys, subtype, &sync, &iod, &udint, &i)) <
      0) {
    trace(2, "rtcm3 %d length error: len=%d\n", type, rtcm->len);
    return -1;
  }
...
static int decode_ssr4(rtcm_t *rtcm, int sys, int subtype) {
  ...
  int i, j, k, type, nsat, sync, iod, prn, sat, iode, iodcrc = 0, refd = 0, np,
                                                      ni, nj, offp;
...
static int decode_ssr7_head(rtcm_t *rtcm,
                            int sys,
                            int subtype,
                            int *sync,
                            int *iod,
...
static int decode_ssr7(rtcm_t *rtcm, int sys, int subtype) {
  ...
  int i, j, k, type, mode, sync, iod, nsat, prn, sat, nbias, np, mw, offp, sii,
      swl;
  int dispe, sdc, yaw_ang, yaw_rate;

  type = getbitu(rtcm->buff, 24, 12);

  if ((nsat = decode_ssr7_head(
           rtcm, sys, subtype, &sync, &iod, &udint, &dispe, &mw, &i)) < 0) {
    trace(2, "rtcm3 %d length error: len=%d\n", type, rtcm->len);
    return -1;
  }
  switch (sys) {
  ...
  for (j = 0; j < nsat && i + 5 + 17 + np <= rtcm->len * 8; j++) {
    ...
    for (k = 0; k < nbias && i + ((subtype == 0) ? 49 : 32) <= rtcm->len * 8;
         k++) {
      mode = getbitu(rtcm->buff, i, 5);
      i += 5;
...
static void sigindex(
    int sys, const uint8_t *code, int n, const char *opt, int *idx) {
...
static void save_msm_obs(rtcm_t *rtcm,
                         int sys,
                         msm_h_t *h,
                         const double *r,
                         const double *pr,
...
static int decode_msm_head(
  ...
  *h = h0;
  ...
  if (h->nsat * h->nsig > 64) {
    trace(2,
          "rtcm3 %d number of sats and sigs error: nsat=%d nsig=%d\n",
          type,
          h->nsat,
          h->nsig);
    return -1;
  ...
  if (i + h->nsat * h->nsig > rtcm->len * 8) {
    trace(2,
          "rtcm3 %d length error: len=%d nsat=%d nsig=%d\n",
          type,
          rtcm->len,
          h->nsat,
          h->nsig);
  ...
  *hsize = i;
  ...
  if (rtcm->outtype) {
    msg = rtcm->msgtype + strlen(rtcm->msgtype);
    sprintf(msg,
            " staid=%4d %s nsat=%2d nsig=%2d iod=%2d ncell=%2d sync=%d",
            staid,
            tstr,
            h->nsat,
            h->nsig,
...
static int decode_msm4(rtcm_t *rtcm, int sys) {
  ...
  if (i + h.nsat * 18 + ncell * 48 > rtcm->len * 8) {
    trace(2,
          "rtcm3 %d length error: nsat=%d ncell=%d len=%d\n",
          type,
          h.nsat,
          ncell,
          rtcm->len);
