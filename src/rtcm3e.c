...
static void gen_obs_gps(rtcm_t *rtcm,
                        const obsd_t *data,
                        int *code1,
                        int *pr1,
                        int *ppr1,
...
static void gen_obs_glo(rtcm_t *rtcm,
                        const obsd_t *data,
                        int fcn,
                        int *code1,
                        int *pr1,
...
static int encode_head(int type, rtcm_t *rtcm, int sys, int sync, int nsat) {
  ...
  trace(4,
        "encode_head: type=%d sync=%d sys=%d nsat=%d\n",
        type,
        sync,
        sys,
        nsat);
...
static int encode_type1046(rtcm_t *rtcm, int sync) {
  ...
  int i = 24, prn, week, toe, toc, i0, OMG0, omg, M0, deln, idot, OMGd, crs,
      crc;
  int cus, cuc, cis, cic, af0, af1, af2, bgd1, bgd2, oshs1, osdvs1, oshs2,
      osdvs2;
...
/* encode SSR header ---------------------------------------------------------*/
static int encode_ssr_head(int type,
                           rtcm_t *rtcm,
                           int sys,
                           int subtype,
                           int nsat,
...
/* SSR signal and tracking mode IDs ------------------------------------------*/
static const int codes_gps[32] = {CODE_L1C,
                                  CODE_L1P,
                                  CODE_L1W,
                                  CODE_L1S,
                                  CODE_L1L,
                                  ...
                                  CODE_L5I,
                                  CODE_L5Q};
static const int codes_glo[32] = {CODE_L1C,
                                  CODE_L1P,
                                  CODE_L2C,
                                  CODE_L2P,
                                  CODE_L4A,
                                  ...
                                  CODE_L3I,
                                  CODE_L3Q};
static const int codes_gal[32] = {CODE_L1A,
                                  CODE_L1B,
                                  CODE_L1C,
                                  0,
                                  0,
                                  ...
                                  CODE_L6B,
                                  CODE_L6C};
static const int codes_qzs[32] = {CODE_L1C,
                                  CODE_L1S,
                                  CODE_L1L,
                                  CODE_L2S,
                                  CODE_L2L,
                                  ...
                                  0,
                                  CODE_L6E};
static const int codes_bds[32] = {CODE_L2I,
                                  CODE_L2Q,
                                  0,
                                  CODE_L6I,
                                  CODE_L6Q,
...
static int encode_ssr1(rtcm_t *rtcm, int sys, int subtype, int sync) {
  ...
  /* encode SSR header */
  i = encode_ssr_head(
      1, rtcm, sys, subtype, nsat, sync, iod, udint, refd, 0, 0);
...
static int encode_ssr4(rtcm_t *rtcm, int sys, int subtype, int sync) {
  ...
  /* encode SSR header */
  i = encode_ssr_head(
      4, rtcm, sys, subtype, nsat, sync, iod, udint, refd, 0, 0);
...
static void gen_msm_index(rtcm_t *rtcm,
                          int sys,
                          int *nsat,
                          int *nsig,
                          int *ncell,
...
static void gen_msm_sat(rtcm_t *rtcm,
                        int sys,
                        int nsat,
...
static void gen_msm_sig(rtcm_t *rtcm,
                        int sys,
                        int nsat,
                        int nsig,
                        int ncell,
  ...
  for (i = 0; i < rtcm->obs.n; i++) {
    ...
    for (j = 0; j < NFREQ + NEXOBS; j++) {
      ...
      phrng_s = data->L[j] == 0.0 || lambda <= 0.0
                    ? 0.0
                    : data->L[j] * lambda - rrng[k];
      rate_s = data->D[j] == 0.0 || lambda <= 0.0
                   ? 0.0
                   : -data->D[j] * lambda - rrate[k];
...
/* encode MSM header ---------------------------------------------------------*/
static int encode_msm_head(int type,
                           rtcm_t *rtcm,
                           int sys,
                           int sync,
                           int *nsat,
...
static int encode_msm_int_rrng(rtcm_t *rtcm,
                               int i,
                               const double *rrng,
                               int nsat) {
  uint32_t int_ms;
  ...
  for (j = 0; j < nsat; j++) {
    if (rrng[j] == 0.0) {
      ...
      trace(2,
            "msm rough range overflow %s rrng=%.3f\n",
...
static int encode_msm_mod_rrng(rtcm_t *rtcm,
                               int i,
                               const double *rrng,
                               int nsat) {
  uint32_t mod_ms;
...
static int encode_msm_rrate(rtcm_t *rtcm,
                            int i,
                            const double *rrate,
                            int nsat) {
  int j, rrate_val;
  ...
  for (j = 0; j < nsat; j++) {
    if (fabs(rrate[j]) > 8191.0) {
      trace(2,
            "msm rough phase-range-rate overflow %s rrate=%.4f\n",
            time_str(rtcm->time, 0),
            rrate[j]);
      rrate_val = -8192;
    } else {
...
static int encode_msm_psrng(rtcm_t *rtcm,
                            int i,
                            const double *psrng,
                            int ncell) {
  int j, psrng_val;
  ...
  for (j = 0; j < ncell; j++) {
    if (psrng[j] == 0.0) {
      ...
      trace(2,
            "msm fine pseudorange overflow %s psrng=%.3f\n",
            time_str(rtcm->time, 0),
            psrng[j]);
      psrng_val = -16384;
    } else {
...
static int encode_msm_psrng_ex(rtcm_t *rtcm,
                               int i,
                               const double *psrng,
                               int ncell) {
  int j, psrng_val;
  ...
  for (j = 0; j < ncell; j++) {
    if (psrng[j] == 0.0) {
      ...
      trace(2,
            "msm fine pseudorange ext overflow %s psrng=%.3f\n",
            time_str(rtcm->time, 0),
            psrng[j]);
      psrng_val = -524288;
    } else {
...
static int encode_msm_phrng(rtcm_t *rtcm,
                            int i,
                            const double *phrng,
                            int ncell) {
  int j, phrng_val;
  ...
  for (j = 0; j < ncell; j++) {
    if (phrng[j] == 0.0) {
      ...
      trace(2,
            "msm fine phase-range overflow %s phrng=%.3f\n",
            time_str(rtcm->time, 0),
            phrng[j]);
      phrng_val = -2097152;
    } else {
...
static int encode_msm_phrng_ex(rtcm_t *rtcm,
                               int i,
                               const double *phrng,
                               int ncell) {
  int j, phrng_val;
  ...
  for (j = 0; j < ncell; j++) {
    if (phrng[j] == 0.0) {
      ...
      trace(2,
            "msm fine phase-range ext overflow %s phrng=%.3f\n",
            time_str(rtcm->time, 0),
            phrng[j]);
      phrng_val = -8388608;
    } else {
...
static int encode_msm_lock_ex(rtcm_t *rtcm,
                              int i,
                              const double *lock,
                              int ncell) {
  int j, lock_val;
...
static int encode_msm_half_amb(rtcm_t *rtcm,
                               int i,
                               const uint8_t *half,
                               int ncell) {
  int j;
...
static int encode_msm_rate(rtcm_t *rtcm, int i, const double *rate, int ncell) {
  ...
  for (j = 0; j < ncell; j++) {
    if (rate[j] == 0.0) {
      ...
      trace(2,
            "msm fine phase-range-rate overflow %s rate=%.3f\n",
            time_str(rtcm->time, 0),
            rate[j]);
      rate_val = -16384;
    } else {
