...
static void timestr_rnx(char *str) {
  ...
  time2epoch(time, ep);
  sprintf(str,
          "%04.0f%02.0f%02.0f %02.0f%02.0f%02.0f UTC",
          ep[0],
          ep[1],
          ep[2],
          ep[3],
...
static void decode_obsh(FILE *fp,
                        char *buff,
                        double ver,
                        int *tsys,
  ...
  if (strstr(label, "MARKER NAME")) {
    ...
    *tobs[i][nt] = '\0';
    ...
    for (j = 0; j < nt; j++) {
      ...
      trace(2,
            "set default for unknown code: sys=%c code=%s\n",
            buff[0],
            tobs[i][j]);
    }
...
static int decode_obsepoch(
    FILE *fp, char *buff, double ver, gtime_t *time, int *flag, int *sats) {
  int i, j, n;
  char satid[8] = "";
...
static int decode_obsdata(
    FILE *fp, char *buff, double ver, int mask, sigind_t *index, obsd_t *obs) {
  sigind_t *ind;
  ...
  trace(
      4, "decode_obsdata: time=%s sat=%2d\n", time_str(obs->time, 0), obs->sat);
  return 1;
}
/* save cycle slips ----------------------------------------------------------*/
...
/* set signal index ----------------------------------------------------------*/
static void set_index(double ver,
                      int sys,
                      const char *opt,
                      char tobs[MAXOBSTYPE][4],
                      sigind_t *ind) {
  ...
  for (p = opt; p && (p = strchr(p, '-')); p++) {
    ...
    for (i = 0; i < n; i++) {
      ...
      ind->shift[i] = shift;
      trace(
          2, "phase shift: sys=%2d tobs=%s shift=%.3f\n", sys, tobs[i], shift);
    }
  }
  /* assign index for highest priority code */
  for (i = 0; i < NFREQ; i++) {
    for (j = 0, k = -1; j < n; j++) {
      if (ind->idx[j] == i && ind->pri[j] &&
...
static int readrnxobsb(FILE *fp,
                       const char *opt,
                       double ver,
                       int *tsys,
                       char tobs[][MAXOBSTYPE][4],
...
static int readrnxobs(FILE *fp,
  ...
  /* read RINEX observation data body */
  while ((n = readrnxobsb(fp, opt, ver, tsys, tobs, &flag, data, sta)) >= 0 &&
         stat >= 0) {
    for (i = 0; i < n; i++) {
      /* UTC -> GPST */
...
/* decode ephemeris ----------------------------------------------------------*/
static int decode_eph(
    double ver, int sat, gtime_t toc, const double *data, eph_t *eph) {
  eph_t eph0 = {0};
  int sys;
...
static int decode_geph(
    double ver, int sat, gtime_t toc, double *data, geph_t *geph) {
  geph_t geph0 = {0};
  gtime_t tof;
  double tow, tod;
  int week, dow;
...
static int decode_seph(
    double ver, int sat, gtime_t toc, double *data, seph_t *seph) {
  seph_t seph0 = {0};
  int week;
...
static int readrnxnavb(FILE *fp,
                       const char *opt,
                       double ver,
                       int sys,
                       int *type,
    ...
    if (i == 0) {
      ...
      if (ver >= 3.0 || sys == SYS_GAL ||
        ...
        if (ver >= 3.0) {
          ...
          if (!sys) {
            sys =
                (id[0] == 'S') ? SYS_SBS : ((id[0] == 'R') ? SYS_GLO : SYS_GPS);
          }
        }
      } else {
...
static int readrnxnav(
    FILE *fp, const char *opt, double ver, int sys, nav_t *nav) {
  eph_t eph;
  geph_t geph;
  seph_t seph;
  int stat, type;
...
static int readrnxfp(FILE *fp,
  ...
  switch (*type) {
    case 'O':
      return readrnxobs(
          fp, ts, te, tint, opt, index, ver, &tsys, tobs, obs, sta);
    case 'N':
...
static int readrnxfile(const char *file,
                       gtime_t ts,
                       gtime_t te,
                       double tint,
                       const char *opt,
...
extern int readrnx(const char *file,
                   int rcv,
                   const char *opt,
                   obs_t *obs,
                   nav_t *nav,
...
extern int readrnxc(const char *file, nav_t *nav) {
  ...
  for (i = 0; i < n; i++) {
    if (readrnxfile(
            files[i], t, t, 0.0, "", 1, index++, &type, NULL, nav, NULL)) {
      continue;
    }
    stat = 0;
    break;
...
extern int input_rnxctr(rnxctr_t *rnx, FILE *fp) {
  ...
  if ((stat = readrnxnavb(
           fp, rnx->opt, rnx->ver, sys, &type, &eph, &geph, &seph)) <= 0) {
    return stat < 0 ? -2 : 0;
  }
  if (type == 1) { /* GLONASS ephemeris */
    ...
    set = (sys == SYS_GAL && (eph.code & (1 << 9)))
              ? 1
              : 0; /* GAL 0:I/NAV,1:F/NAV */
...
static void outrnx_phase_shift(FILE *fp,
                               const rnxopt_t *opt,
                               const nav_t *nav) {
...
static void outrnx_glo_bias(FILE *fp, const rnxopt_t *opt) {
  ...
  if (opt->navsys & SYS_GLO) {
    fprintf(fp,
            " %s %8.3f %s %8.3f %s %8.3f %s %8.3f%8s%-20s\n",
            tobs[0],
            opt->glo_cp_bias[0],
            tobs[1],
            opt->glo_cp_bias[1],
...
extern int outrnxobsh(FILE *fp, const rnxopt_t *opt, const nav_t *nav) {
  ...
  fprintf(fp,
          "  %04.0f    %02.0f    %02.0f    %02.0f    %02.0f   %010.7f     "
          "%-12s%-20s\n",
          ep[0],
          ep[1],
          ep[2],
...
static int obsindex(int rnxver,
                    int sys,
                    const uint8_t *code,
                    const char *tobs,
                    const char *mask) {
...
extern int outrnxobsb(FILE *fp,
  ...
  if (opt->rnxver <= 299) { /* ver.2 */
    fprintf(fp,
            " %02d %02.0f %02.0f %02.0f %02.0f %010.7f  %d%3d",
            (int)ep[0] % 100,
            ep[1],
            ep[2],
            ep[3],
    ...
    fprintf(fp,
            "> %04.0f %02.0f %02.0f %02.0f %02.0f %010.7f  %d%3d%6s%15.12f\n",
            ep[0],
            ep[1],
            ep[2],
            ep[3],
  ...
  for (i = 0; i < ns; i++) {
    ...
    for (j = 0; j < opt->nobs[m]; j++) {
      ...
      if ((k = obsindex(
               opt->rnxver, sys, obs[ind[i]].code, opt->tobs[m][j], mask)) <
          0) {
        outrnxobsf(fp, 0.0, -1);
        continue;
      }
...
static void out_iono_sys(FILE *fp, const char *sys, const double *ion, int n) {
  const char *label1[] = {"ION ALPHA", "ION BETA"},
...
static void out_leaps(FILE *fp,
  ...
  if (opt->rnxver <= 300) {
    ...
    fprintf(fp,
            "%6.0f%6.0f%6.0f%6.0f%3s%33s%-20s\n",
            leaps[0],
            leaps[3],
            leaps[1],
            leaps[2],
...
extern int outrnxnavb(FILE *fp, const rnxopt_t *opt, const eph_t *eph) {
  ...
  if ((opt->rnxver >= 300 && sys == SYS_GPS) ||
    ...
    fprintf(fp,
            "%2d %02d %02.0f %02.0f %02.0f %02.0f %04.1f",
            prn,
            (int)ep[0] % 100,
            ep[1],
            ep[2],
...
extern int outrnxhnavb(FILE *fp, const rnxopt_t *opt, const seph_t *seph) {
  ...
  if (opt->rnxver <= 299) { /* ver.2 */
    fprintf(fp,
            "%2d %02d %2.0f %2.0f %2.0f %2.0f %4.1f",
            prn - 100,
            (int)ep[0] % 100,
            ep[1],
            ep[2],
    ...
    fprintf(fp,
            "%-3s %04.0f %2.0f %2.0f %2.0f %2.0f %2.0f",
            code,
            ep[0],
            ep[1],
            ep[2],
