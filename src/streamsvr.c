 ...
 *-----------------------------------------------------------------------------*/
extern strconv_t *strconvnew(int itype,
                             int otype,
                             const char *msgs,
                             int staid,
                             int stasel,
...
/* convert stearm ------------------------------------------------------------*/
static void strconv(stream_t *str,
                    strconv_t *conv,
                    unsigned char *buff,
                    int n) {
  int i, ret;
...
extern void strsvrinit(strsvr_t *svr, int nout) {
  ...
  svr->tick = 0;
  for (i = 0; i < nout + 1; i++) strinit(svr->stream + i);
  svr->nstr = nout + 1;
...
extern void strsvrstat(
    strsvr_t *svr, int *stat, int *byte, int *bps, char *msg) {
