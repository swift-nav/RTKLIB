 ...
 *                           fix bug on writing BDS/IRN nav header in
 *closefile() fix bug on screening time in screent_ttol() fix bug on screening
 *QZS L1S messages as SBAS use integer types in stdint.h
 *-----------------------------------------------------------------------------*/
#include "rtklib.h"
...
static const int navsys[] = {/* system codes */
                             ...
                             SYS_IRN,
                             0};
static const char vercode[]
                         [MAXCODE] = {
                             /* supported obs-type by RINEX version */
                             /* 0........1.........2.........3.........4.........5.........6........
                              */
                             /* 11111111111112222222222555777666666688822663331155599991555677788444
                                CODE */
                             /* CPWYMNSLEABXZCDSLXPWYMNIQXIQXABCXZSLIQXIQIQIQXIQABCABCXDDPZEDPZDPABX
                              */
                             "00000000...0.0000000000000......................."
                             "...................", /* GPS */
                             "00...........0....0..........44.4..........222..."
                             "................444", /* GLO */
                             "0........0000..........0000000000...000.........."
                             "...................", /* GAL */
                             "2.....22...22..222.....222......2422............."
                             ".......4444........", /* QZS */
                             "0......................000......................."
                             "...................", /* SBS */
                             ".4...4...4.4.....1.......41114..1.....41111......"
                             "......444..44444...", /* BDS */
                             ".........................3......................"
                             "3333333............." /* IRN */
};
...
static void convcode(int rnxver, int sys, char *type) {
  if (rnxver >= 212 && (sys == SYS_GPS || sys == SYS_QZS || sys == SYS_SBS) &&
    ...
    strcpy(type + 1, "C");
  } else if (rnxver >= 212 && sys == SYS_GLO &&
             !strcmp(type + 1, "1C")) { /* L1C/A */
    strcpy(type + 1, "A");
  } else if (rnxver >= 212 && sys == SYS_GLO &&
    ...
    strcpy(type + 1, "D");
             ...
             (!strcmp(type + 1, "2I") || !strcmp(type + 1, "2Q") ||
...
static void setopt_file(
    int format, char **paths, int n, const int *mask, rnxopt_t *opt) {
  int i, j;
...
static void setopt_obstype(const uint8_t *codes,
                           const uint8_t *types,
                           int sys,
                           rnxopt_t *opt) {
  ...
  for (i = 0; codes[i]; i++) {
    if (!(id = code2obs(codes[i])) ||
        (idx = code2idx(navsys[sys], codes[i])) < 0) {
      continue;
    }
    ...
    if (opt->rnxver >= 300) {
      ...
      if (ver < '0' || ver > '0' + opt->rnxver - 300) {
        trace(2,
              "unsupported obs type: rnxver=%.2f sys=%d code=%s\n",
              opt->rnxver / 100.0,
              sys,
              code2obs(codes[i]));
        continue;
    ...
    for (j = 0; j < opt->nobs[i]; j++) {
      ...
      if (navsys[i] == SYS_GPS) {
        if (code == CODE_L1S || code == CODE_L1L || code == CODE_L1X ||
            code == CODE_L1P || code == CODE_L1W || code == CODE_L1N) {
...
static void dump_stas(const strfile_t *str) {
  ...
  trace(2,
        "# %17s %19s %5s %6s %16s %16s %12s %13s %9s %2s %6s %6s %6s\n",
        "TIME",
        "STAID",
        "MARKER",
        "ANTENNA",
...
static void update_halfc(strfile_t *str, obsd_t *obs) {
  ...
  for (i = 0; i < NFREQ + NEXOBS; i++) {
    ...
    } else if (str->halfc[sat - 1][i]->stat ==
               1) { /* halfcyc unknown -> known */
    ...
    for (j = 0; j < NFREQ + NEXOBS; j++) {
      ...
      for (p = str->halfc[sat - 1][j]; p; p = p->next) {
        ...
        if (timediff(data[i].time, p->ts) < -DTTOL ||
            timediff(data[i].time, p->te) > DTTOL)
          continue;
...
static int scan_file(
    char **files, int nf, rnxopt_t *opt, strfile_t *str, int *mask) {
  eph_t eph0 = {0, -1, -1};
  geph_t geph0 = {0, -1};
  seph_t seph0 = {0};
  ...
  for (m = 0; m < nf && !abort; m++) {
    ...
    while ((type = input_strfile(str)) >= -1) {
      ...
      if (type == 1) { /* observation data */
        ...
        if (str->format == STRFMT_SBP || str->format == STRFMT_SBPJSON) {
          /* update station list */
          if (str->raw.staid >= 0) update_stas(str);
        }
        /* update station info */
        update_stainf(str);
      }
...
/* output RINEX event --------------------------------------------------------*/
static void outrnxevent(FILE *fp,
                        const rnxopt_t *opt,
                        gtime_t time,
                        int event,
                        const stas_t *stas,
  ...
  double ep[6], pos[3] = {0}, enu[3] = {0}, del[3] = {0};
  ...
  if (event == EVENT_STARTMOVE) {
    ...
    time2epoch(time, ep);
    fprintf(fp,
            "%s %02d %02.0f %02.0f %02.0f %02.0f %010.7f  %d%3d\n",
            (opt->rnxver >= 300) ? ">" : "",
...
static int screent_ttol(
    gtime_t time, gtime_t ts, gtime_t te, double tint, double ttol) {
  if (ttol <= 0.0) ttol = DTTOL;

  return (tint <= 0.0 ||
          fmod(time2gpst(time, NULL) + ttol, tint) <= ttol * 2.0) &&
         (ts.time == 0 || timediff(time, ts) >= -ttol) &&
         (te.time == 0 || timediff(time, te) < ttol);
}
/* convert observation data --------------------------------------------------*/
static void convobs(FILE **ofp,
                    rnxopt_t *opt,
                    strfile_t *str,
                    int *n,
                    gtime_t *tend,
...
static void convsbs(
    FILE **ofp, rnxopt_t *opt, strfile_t *str, int *n, gtime_t *tend) {
  gtime_t time;
  ...
  *tend = time;
  ...
  if ((opt->navsys & SYS_SBS) &&
      sbsupdatecorr(&str->raw.sbsmsg, str->nav) == 9) {
    if (ofp[1] && !sep_nav) {
      outrnxhnavb(ofp[1], opt, str->nav->seph + prn - MINPRNSBS);
      n[1]++;
    } else if (ofp[3] && sep_nav) {
...
static int convrnx_s(
    int sess, int format, rnxopt_t *opt, const char *file, char **ofile) {
  FILE *ofp[NOUTFILE] = {NULL};
  strfile_t *str;
  gtime_t tend[3] = {{0}};
  int i, j, nf, type, n[NOUTFILE + 1] = {0}, mask[MAXEXFILE] = {0}, staid = -1,
                                   abort = 0;
...
extern int convrnx(int format, rnxopt_t *opt, const char *file, char **ofile) {
  ...
  trace(3,
        "convrnx: format=%d file=%s ofile=%s %s %s %s %s %s %s %s %s\n",
        format,
        file,
        ofile[0],
        ofile[1],
