...
static double prange(const obsd_t *obs,
                     const nav_t *nav,
                     const prcopt_t *opt,
                     double *var) {
...
extern int ionocorr(gtime_t time,
                    ...
                    double *var) {
  trace(4,
        "ionocorr: time=%s opt=%d sat=%2d pos=%.3f %.3f azel=%.3f %.3f\n",
        time_str(time, 3),
        ionoopt,
        sat,
        pos[0] * R2D,
...
extern int tropcorr(gtime_t time,
                    ...
                    double *var) {
  trace(4,
        "tropcorr: time=%s opt=%d pos=%.3f %.3f azel=%.3f %.3f\n",
        time_str(time, 3),
        tropopt,
        pos[0] * R2D,
        pos[1] * R2D,
...
static int rescode(int iter,
  ...
  for (i = *ns = 0; i < n && i < MAXOBS; i++) {
    ...
    if (iter > 0) {
      ...
      /* ionospheric correction */
      if (!ionocorr(
              time, nav, sat, pos, azel + i * 2, opt->ionoopt, &dion, &vion)) {
        continue;
      }
...
/* validate solution ---------------------------------------------------------*/
static int valsol(const double *azel,
                  const int *vsat,
                  int n,
                  const prcopt_t *opt,
                  const double *v,
  ...
  if (nv > nx && vv > chisqr[nv - nx - 1]) {
    sprintf(msg,
            "chi-square error nv=%d vv=%.1f cs=%.1f",
            nv,
            vv,
            chisqr[nv - nx - 1]);
    return 0;
...
static int estpos(const obsd_t *obs,
                  int n,
                  const double *rs,
                  const double *dts,
                  const double *vare,
...
static int raim_fde(const obsd_t *obs,
                    int n,
                    const double *rs,
                    const double *dts,
                    const double *vare,
  ...
  for (i = 0; i < n; i++) {
    ...
    if (nvsat < 5) {
      trace(3,
            "raim_fde: exsat=%2d lack of satellites nvsat=%2d\n",
            obs[i].sat,
            nvsat);
      continue;
    }
...
static int resdop(const obsd_t *obs,
                  int n,
                  const double *rs,
                  const double *dts,
                  const nav_t *nav,
  ...
  for (i = 0; i < n && i < MAXOBS; i++) {
    ...
    if (obs[i].D[0] == 0.0 || freq == 0.0 || !vsat[i] ||
        norm(rs + 3 + i * 6, 3) <= 0.0) {
      continue;
    }
    ...
    v[nv] = (-obs[i].D[0] * CLIGHT / freq -
             (rate + x[3] - CLIGHT * dts[1 + i * 2])) /
            sig;
...
static void estvel(const obsd_t *obs,
                   int n,
                   const double *rs,
                   const double *dts,
                   const nav_t *nav,
  ...
  for (i = 0; i < MAXITR; i++) {
    ...
    if ((nv = resdop(obs, n, rs, dts, nav, sol->rr, x, azel, vsat, err, v, H)) <
        4) {
      break;
    }
    /* least square estimation */
...
extern int pntpos(const obsd_t *obs,
  ...
  stat = estpos(
      obs, n, rs, dts, var, svh, nav, &opt_, sol, azel_, vsat, resp, msg);
