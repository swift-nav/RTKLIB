...
static int test_nmea(const char *buff) {
  ...
  return !strncmp(buff + 1, "GP", 2) ||
         !strncmp(buff + 1, "GA", 2) || /* NMEA 4.10 [1] */
 ...
 *-----------------------------------------------------------------------------*/
extern int inputsol(uint8_t data,
                    gtime_t ts,
                    gtime_t te,
                    double tint,
                    int qflag,
...
/* read solution data --------------------------------------------------------*/
static int readsoldata(FILE *fp,
                       gtime_t ts,
                       gtime_t te,
                       double tint,
                       int qflag,
 ...
 *-----------------------------------------------------------------------------*/
extern int readsolt(char *files[],
                    int nfile,
                    gtime_t ts,
                    gtime_t te,
                    double tint,
...
static int decode_solstat(char *buff, solstat_t *stat) {
  ...
  n = sscanf(buff,
             "$SAT%d%lf%s%d%lf%lf%lf%lf%d%lf%d%d%d%d%d%d",
             &week,
             &tow,
             id,
             &frq,
...
static int readsolstatdata(
    FILE *fp, gtime_t ts, gtime_t te, double tint, solstatbuf_t *statbuf) {
  solstat_t stat = {{0}};
  char buff[MAXSOLMSG + 1];
 ...
 * return : status (1:ok,0:no data or error)
 *-----------------------------------------------------------------------------*/
extern int readsolstatt(char *files[],
                        int nfile,
                        gtime_t ts,
                        gtime_t te,
                        double tint,
...
static int outecef(uint8_t *buff,
                   const char *s,
                   const sol_t *sol,
                   const solopt_t *opt) {
  const char *sep = opt2sep(opt);
...
static int outpos(uint8_t *buff,
                  const char *s,
                  const sol_t *sol,
                  const solopt_t *opt) {
  ...
  if (opt->degf) {
    ...
    p += sprintf(p,
                 "%s%s%4.0f%s%02.0f%s%08.5f%s%4.0f%s%02.0f%s%08.5f",
                 s,
                 sep,
                 dms1[0],
                 sep,
    ...
    p += sprintf(
        p, "%s%s%14.9f%s%14.9f", s, sep, pos[0] * R2D, sep, pos[1] * R2D);
  }
  p += sprintf(p,
...
static int outenu(uint8_t *buff,
                  const char *s,
                  const sol_t *sol,
                  const double *rb,
                  const solopt_t *opt) {
...
extern int outnmea_gsa(uint8_t *buff, const sol_t *sol, const ssat_t *ssat) {
  ...
  for (i = 0; nmea_sys[i]; i++) {
    ...
    if (nsat > 0) {
      s = p;
      p += sprintf(
          p, "$%sGSA,A,%d", nsys > 1 ? "GN" : nmea_tid[i], sol->stat ? 3 : 1);
      for (j = 0; j < 12; j++) {
      ...
      p += sprintf(
          p, ",%3.1f,%3.1f,%3.1f,%d", dop[1], dop[2], dop[3], nmea_sid[i]);
...
extern int outprcopts(uint8_t *buff, const prcopt_t *opt) {
  ...
  for (i = 0; i < 2; i++) {
    if (opt->mode == PMODE_SINGLE || (i >= 1 && opt->mode > PMODE_FIXED))
      continue;
    p += sprintf(p,
                 "%s antenna%d  : %-21s (%7.4f %7.4f %7.4f)\r\n",
                 COMMENTH,
                 i + 1,
                 opt->anttype[i],
                 opt->antdel[i][0],
...
extern int outsolheads(uint8_t *buff, const solopt_t *opt) {
  ...
  if (opt->posf == SOLF_LLH) { /* lat/lon/hgt */
    ...
    if (opt->outvel) {
      p += sprintf(p,
                   "%s%10s%s%10s%s%10s%s%9s%s%8s%s%8s%s%8s%s%8s%s%8s",
                   sep,
                   "vn(m/s)",
                   sep,
                   "ve(m/s)",
 ...
 * return : number of output bytes
 *-----------------------------------------------------------------------------*/
extern int outsols(uint8_t *buff,
                   const sol_t *sol,
                   const double *rb,
                   const solopt_t *opt) {
  gtime_t time, ts = {0};
 ...
 *-----------------------------------------------------------------------------*/
extern int outsolexs(uint8_t *buff,
                     const sol_t *sol,
                     const ssat_t *ssat,
                     const solopt_t *opt) {
  gtime_t ts = {0};
