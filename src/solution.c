...
static int decode_nmea(char *buff, sol_t *sol)
{
    ...
    if (NULL == val[0]) {
        trace(1,"decode_nmea parser error\n");
        return 0;
    }
...
static int decode_sol(char *buff, const solopt_t *opt, sol_t *sol, double *rb)
{
    ...
    if (buff[0] == COMMENTH) { /* reference position */
...
static void decode_solopt(char *buff, solopt_t *opt)
{
    ...
    if ((buff[0] != COMMENTH) && strncmp(buff,"+",1)) return;
...
extern int outsolheads(unsigned char *buff, const solopt_t *opt)
{
    ...
    p+=sprintf(p,"%c  %-*s%s",COMMENTH,(opt->timef?16:8)+timeu+1,s3[opt->times],sep);
                   ...
                   const solopt_t *opt)
{
    ...
    if (opt->timef) time2str(time,s,timeu);
        ...
        sprintf(s,"%4d%c%*.*f",week,sep[0],6+(timeu<=0?0:timeu+1),timeu,gpst);
    }
    switch (opt->posf) {
